...
/** 应用设置 store（T1.2）：加载 ~/.pi-desktop/settings.json，布局变更防抖写回 */
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  loaded: false,
  async load() {
    const settings = (await window.pi.settingsGet()) as unknown as PiDeskSettings;
    set({ settings: { ...defaults, ...settings }, loaded: true });
  },
  async patch(patch) {
    const settings = (await window.pi.settingsSet(patch)) as unknown as PiDeskSettings;
