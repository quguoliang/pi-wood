...
export interface PiWoodSettings {
  ...
  /**
   * T8.6/T8.11：工作树策略（设置「工作树」页可改；主进程 conversation-registry 读同一段配置）。
   * mode：ask=新建会话时按主树是否脏决定（脏则弹框选「当前分支 / 独立工作树」，缺省）；
   *       worktree=每条对话总是建独立工作树；current=总在主工作树/当前分支跑，不建。
   */
  worktree: { mode: "ask" | "worktree" | "current"; keepAfterClose: boolean };
...
const defaults: PiWoodSettings = {
  ...
  recentProjects: [],
  worktree: { mode: "ask", keepAfterClose: false },
...
const merge = (raw: Partial<PiWoodSettings> | undefined): PiWoodSettings => ({
  ...
  worktree: (() => {
    const w = raw?.worktree as { mode?: string; enabled?: boolean; keepAfterClose?: boolean } | undefined;
    const mode: PiWoodSettings["worktree"]["mode"] =
      w?.mode === "worktree" || w?.mode === "current" || w?.mode === "ask"
        ? w.mode
        : w?.enabled === false
          ? "current"
          : "ask";
    return { mode, keepAfterClose: w?.keepAfterClose === true };
  })(),
