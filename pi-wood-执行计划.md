...
> **✅ 2026-09-02 落地（S3/S4/S5）**：以 **vendored in-process** 方式接线完成，typecheck 全绿、`electron-vite build` 通过、离线探针证明 6 个 `agent_*` 工具经桌面同一条 `attach→session_start` 路径注册成功。**关键偏差（与下方计划清单的差异）**：
> - **不 `pi install`、不补 `package.json` exports**：直接把源码 vendored 到 `apps/desktop/electron/main/subagent/vendor/`，用相对 `.ts` import 打进主进程 bundle（esbuild），无需 `pi` CLI 与磁盘扩展加载；`composition.ts` 裁到**只留 Pi harness**（删 claude/codex，其 `@anthropic-ai/claude-agent-sdk` 不在依赖树）。
> - **`pi-tui` 提为 apps/desktop 直接依赖**（`^0.84.4`）：vendor 的 TUI 渲染层（`render.ts/widget.ts/notification-message.ts`）在 RPC/桌面模式是惰性死代码，但模块级 import 需运行时可解析；externalize 后 `require` 到。已核实 `installRunsWidget`/`renderNotificationMessage` 不在 RPC `session_start` eager 构造 TUI 组件。
> - **child 审批门注入点**：给 vendored `createPiSessionOptions` 加可选 `extraExtensionFactories?: InlineExtension[]` 参数，透传进 child 的 `DefaultResourceLoader.extensionFactories`；`pi-wood-subagent.ts` 用自定义 `sessionOptionsFactory` 把桌面 `permissionGateExtension`（复用父 `getPolicy` + `confirmViaRenderer`）注入 child，杜绝审批旁路。
> - **未抽 `security/approval-io.ts`**：`engine-manager` 本就持有 `getPolicy/confirmViaRenderer`，构造子代理运行时时以**闭包传入**达成同一复用，避免搬动阻塞对话框基础设施（`approval:acceptAll` 同时触达 `pendingApprovals` 与 `pendingUiRequests`）的回归风险。文件命名 `pi-wood-subagent.ts`（非计划的 `pi-wood-session-options.ts`）。
> - **S1 空操作**：本机 macOS、`pi` CLI 不在 PATH、`~/.pi/agent/settings.json` 不存在、无旧 `pi-subagents` → 无需卸载、无 `Could not resolve the Pi CLI on Windows` 报错。
> - **Node ESM 细节**：`pi-wood-subagent.ts` 对 `../security/approval-gate` 补 `.ts` 扩展名，使真实 Node（`--experimental-strip-types`）也能直跑探针；esbuild/tsc 两可。
> **剩余门禁（S6）**：真·密钥 DeepSeek 的多代理端到端（并行 fan-out + 命名续跑 + steer + cancel）与「子代理触发 bash → highRisk/untrusted 弹 ApprovalCard」运行态手验（需重启 dev），及 §7.7 T6.3 状态面板/进度上屏——本 increment 未含。
...
- [x] `engine-manager.ts`：`ensureEngineUnlocked` 里 `subagentHandle = createPiWoodSubagentRuntime({getPolicy:()=>getPolicy(), confirm:(t,m)=>confirmViaRenderer(t,m)})`，把 `subagentHandle.inlineExtension` 追加进 `inlineExtensions`。**偏差**：未抽 `security/approval-io.ts`——engine-manager 本就持有二者，以**闭包传入**达成同一复用（避开搬动 `approval:acceptAll` 跨 `pendingApprovals`/`pendingUiRequests` 的回归）。切项目时先 `disposeSubagent()` 再重建。
- [ ] 验证（运行态，需密钥+重启 dev）：子代理触发 `bash`，highRisk/untrusted 下必须弹 ApprovalCard（或按 policy deny），不得静默执行。child 门的 `permissionGateExtension` 已编译进 bundle（`extraExtensionFactories`/`piwood-permission-gate` 在 out/main 可见），运行态待手验。
...
### S6 门禁与验收
- [ ] **真密钥多代理 e2e**（并行 2-3 子代理 + 命名会话续跑 + steer + cancel + child bash 弹审批卡）——需用户 DeepSeek 密钥 + 重启 dev（主/preload 改动），待手验。
