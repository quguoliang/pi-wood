...
|---|---|---|---|
| 每对话净增 RSS（含引擎子进程 + 其扩展/子进程） | **≤ 300MB/对话、3 路 ≤ 1GB**（T8.0 实测 child 自报 RSS **198~225MB/对话**，含 5 个已装社区包 + MCP 适配器；3 路合计 ≈600~675MB，预算有余量） | 探针轮询 `utilityProcess` 与主进程 RSS，取「起第 N 个对话前后的进程树 RSS 差」→ **实测口径修正**：tasklist 按 PID 取 RSS 在本机解析不出值（探针记 `pidRSS=0` 属口径失效、非无占用），改**以 child 自报 `process.memoryUsage().rss` 为准** | 活跃进程硬上限（默认 3，可配 1~4）+ LRU 关停 |
| 对话冷启动（fork + 载 SDK + 载扩展 + `bindExtensions`） | **≤ 3s**（T8.0 实测 **≈1.1~1.2s**：fork→ready 75~88ms + 装配 1089~1147ms，其中 SDK import ≈590ms、services ≈445ms、session 16ms、bindExtensions 5ms；首帧引擎事件 @1078ms，一轮真对话 settle 2872ms） | 探针打时间戳：fork→`ready` 帧→首个 `agent_start` | 引擎惰性起、worktree 后台预建、启动期显「正在准备引擎…」 |
...
- 验收：
  - [~] 单项目 2 对话 + 跨项目 1 对话并存：三边各自流式跑完、互不打断 → **跨项目 3 对话并存已证**（`--conversation-probe` C1：三路独立 pid、工具集逐个全等、三路装配 3.9~4.1s）；**同项目 2 对话**要等 T8.6 的 worktree（注册表当前按 projectDir 1:1，避免同目录互踩），stub adapter 注入点已就位（`ConversationCapabilities` + `EngineTransport`）
  - [ ] 同项目两对话的 **cwd 不同**（worktree 生效），扩展模块态互不可见 → 归 **T8.6**；本轮已证「每对话一个独立进程 + 独立模块图」（C1.2/C1.3 + child 内 `globalThis.__piwoodSubagentBridge` 天然每进程一份）
...
- 验收：
  ...
  - [~] `pnpm -r typecheck` + `pnpm -r test` + build + 三探针回归 EXIT=0 → typecheck / test（desktop 184/184 + 三包）/ build **实跑全绿**；三探针回归待真机
...
- 验收：
  ...
  - [~] `pnpm -r typecheck` + `pnpm -r test` + build 全绿 → typecheck / test（desktop **208/208**）/ build 实跑全绿；三探针回归待真机（`probe:worktree` 沙箱 15/16，大小写项 Windows 专属）
...
- 验收：
  ...
  - [~] `pnpm typecheck` + desktop test + build + 三探针回归全绿 → typecheck/test（desktop **216/216**）/build 实跑全绿；三探针回归待真机
- 验证方式：无窗探针 `--workspace-scope-probe`（2 项目 + 同项目两 worktree，交叉调 fs/git/snapshot/memory/term 断言归属）+ 真机目检

### [ ] T8.8 多对话 UI（标签条 / 进度徽章 / 未读）+ 并发门禁与回归（⏳ 2026-09-04：UI+键盘+退出确认+探针骨架完成，单测/typecheck/build 全绿；**真机探针与目检待跑**——见逐项标注与 §8）
...
- 步骤：
  ...
  5. → [~] 探针骨架+可行断言（①②③⑥⑦）已写；真机跑 + ④⑤⑧定档待
  6. → [~] worktree-probe 沙箱 15/16（Windows 大小写项专属）；其余四探针需 electron，真机回归待
- 验收：
  - [~] `--concurrency-probe` 8 条全 PASS、EXIT=0 → 骨架就绪（④⑤⑧显式 SKIP），真机跑待
  - [~] 五探针回归 + `pnpm -r typecheck`/`test` + build → typecheck（**221/221**）/build 实跑全绿；electron 探针回归真机待
  - [ ] 性能红线表逐条实测达标 → 待真机 --ui-chat/--concurrency-probe 定档
  - [ ] 并发数=1 时行为与改造前逐位一致 → 待真机 A/B 对照目检
  - [ ] 真机 4 对话并行改代码+回流端到端目检留证 → 待（密钥+GUI）
  - [ ] 打包版冒烟 → 待（T8.1 已证 packaged 可从 asar 载 child，交互未验）
