...
- 验收：
  ...
  - [ ] **renderer 那一跳与「切换首屏 ≤100ms」「3 路并发前台体感/CPU」仍未有数** → 需要真窗口与真模型流量，本探针按设计不合成渲染层（打 SKIP）；出口已就位（`engineLatencySummary()` 可被 IPC/面板读取），**待接进 `--ui-chat` 与 EnvironmentPanel**
...
> 一次性跑法：`bash scripts/t88-verify.sh`（macOS 本机；`SKIP_UI=1` 跳真模型对话、`SKIP_PACKAGE=1` 跳打包段）。证据统一落 `apps/desktop/docs/proofs/T8.8/`（**本轮起 proofs 下 `.log` 已解除忽略，可入库**）。细节与踩坑全在 §8 的 2026-09-05 行。
...
| T8.9 红线度量出口 | ✅ `--latency-probe` **7/7 EXIT=0**：rpcRtt p95 0.093ms / eventHop p95 2.292ms / approvalRtt p95 0.087ms，统计按对话独立且 close 后出清 | **main→renderer 那一跳**与切换首屏 / 3 路体感 / 主进程 CPU（需真窗口）；出口已就位，待接进 `--ui-chat` 与 EnvironmentPanel |

> **本批全 ✅ 的剩余条件**：①一轮「GUI 人手 + 真模型」验收（4 对话并行改代码→审 diff→回流、并发审批、切换手感、并发=1 A/B 对照）；②**度量缺口经 T8.9 后只剩**：3 路并发前台体感、主进程 CPU ≤60%、切换首屏 ≤100ms（三者都要真窗口/真渲染层）+ 在飞 prompt/子代理峰值计数（需真模型灌流）——RPC 往返、事件单跳、审批往返**已有口径与无负载实测**，不再算「无对象」；③`--workspace-scope-probe` 与 deny-by-default 探针断言补齐；④Windows 侧零残留硬断言（⑦ 只在 win32 有效）与 T5.3 的安装/签名/三平台（环境阻塞）。
