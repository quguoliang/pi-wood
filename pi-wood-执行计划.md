...
- **刻度条（定稿）**：行高 12px 等距、**常态每根完全一样**（16×1px 发丝线、同色，`data-active` 与加粗规则整个删除）；hover 才动——以目标为中心做**正态衰减展开**（`tickWidthAt`：目标 30px、邻居按 σ=1.6 高斯渐次变长、远端回 16px，200ms ease），**长度与颜色同一条衰减曲线**（`color-mix(foreground r%, muted-foreground)`，r=衰减比）；容器预留 30px 满宽，展开不推动布局。阅读锚点降级为**只做长对话内的刻度条跟随滚动**（手动改 `scrollTop`，不用 `scrollIntoView` 以免带着消息列表一起滚），不再产生任何视觉高亮。
...
- **位置（定稿）**：**竖向居中**浮在消息列左缘（撤销第一轮的「贴顶」）；轮次过多时刻度条内部滚动且隐藏滚动条，摘要浮层是滚动容器的兄弟节点、不被裁剪。窄窗 <720px 自动隐藏、轮末刷树（供 fork 对齐）等 v2.1 行为原样保留。
