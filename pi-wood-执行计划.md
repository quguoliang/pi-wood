...
### [ ] T8.4 审批与 ctx.ui 按对话归属 + PromptTray 显示来源（⏳ 2026-09-04：代码+单测+typecheck+build 全过，真机探针/目检待跑——见下方逐项标注与 §8）
...
- 验收：
  - [x] A 对话的 bash 审批卡显示来源且只能被 A 的应答放行；在 B 对话里应答 A 的请求被拒（单测/探针断言 owner 校验）→ 单测断言完成；真机探针随下一条一并补
  - [~] 两对话同时待审批 → 队列两条、来源可分辨、各自超时互不影响 → 机制（分组/来源行/超时分档互不计时）已实现并单测；**真机两对话目检待跑**
  - [~] **后台对话的审批不会因用户看不见而被 120s 静默拒**（探针：非 active pending 在 120s 后仍在、红点在、切过去可应答）→ `shouldArmTimeout`+`armPendingTimeoutsFor` 单测已断言机制；120s 真机探针待跑
  - [~] child 无本地放行路径：不发 `approval:respond` 时该工具必被拒（探针断言 deny-by-default + 重放 token 失效）→ ticket 一次性消费有单测；deny-by-default 探针断言待跑（可并入 `--conversation-probe` 回归）
  - [x] `acceptAll` 只清该对话 pending，别对话不受影响
  - [x] 审批仍在 Composer 上方 PromptTray 呈现（无新增浮窗/模态，符合用户偏好）
  - [~] `pnpm typecheck` + desktop test + build 全绿 → typecheck（node+web 两份 tsconfig）绿；desktop test **165/165** 绿；**`electron-vite build` EXIT=0**（沙箱按 T8.P 配方补 linux 原生二进制实跑）+ 产物断言（新符号进 main/child/preload、ESM 形态不变）全过；真机探针目检项见上两条 [~]
...
| T8.4 审批归属 | 机制 + 单测 + 探针回归 | **两对话并发审批目检**、后台 pending 120s 不静默拒的**长计时真机验证**、deny-by-default 探针断言并入 `--conversation-probe`（未做） |
...
> **本批全 ✅ 的剩余条件**：①一轮「GUI 人手 + 真模型」验收（4 对话并行改代码→审 diff→回流、并发审批、切换手感、并发=1 A/B 对照、真模型三路体感）；②**度量缺口经 T8.9+T8.10 后只剩两条**：在飞 prompt/子代理峰值计数（需真模型灌流，`--concurrency-probe` ④⑤ 设计上不做）与真模型负载下的体感复核——**红线数字一律要绑负载形状**（同一份代码 16ms 节流下第二跳 p95 1.87ms、无间隔突发下 261ms，后者是排队延迟不是产品慢）；③`--workspace-scope-probe` 已于 09-05 补齐（14/14），**只剩 deny-by-default 探针断言并入 `--conversation-probe`** 未做；④Windows 侧零残留硬断言（⑦ 只在 win32 有效）与 T5.3 的安装/签名/三平台（环境阻塞）。
