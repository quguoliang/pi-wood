...
> **本批全 ✅ 的剩余条件**：①一轮「GUI 人手 + 真模型」验收（4 对话并行改代码→审 diff→回流、并发审批、切换手感、并发=1 A/B 对照）；②RPC p95/切换首屏/CPU 三条**先补度量出口再谈定档**（现在探针没有 p50/p95 与 rAF 掉帧统计）；③`--workspace-scope-probe` 与 deny-by-default 探针断言补齐；④Windows 侧零残留硬断言（⑦ 只在 win32 有效）与 T5.3 的安装/签名/三平台（环境阻塞）。
