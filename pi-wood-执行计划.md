...
- **消息级分叉**：`AssistantRow` 动作排（复制/重试旁）加「分叉」= 把**该回复之前**的全部对话分叉成新会话——行↔树用「第 N 条 user」序号对齐取 entryId → `engine:forkToNewConversation`（`createBranchedSession(leafId)` 生成只含 root→该提问路径的新 jsonl，header 自带 `parentSession`）→ 新对话（独立 worktree）`switchSession` 接上 → 写 session-meta `alias="Fork of <源首问>"` + `forkedFrom=<源文件>` → refresh+switchTo。源对话零扰动。
