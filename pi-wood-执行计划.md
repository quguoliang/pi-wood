...
>
> **T8.0 回填状态（2026-09-04）**：已用真机实测定档 4 行——每对话净增 RSS、对话冷启动、worktree 创建/回流、残留进程数。**其余 6 行（RPC 两跳延迟、后台事件流量、3 路并发体感、切换首屏、休眠→恢复、在飞 prompt/子代理上限）T8.0 测不了**：它们要求 EngineHost 已接线（T8.1/T8.3/T8.5 的 `--concurrency-probe` 才有对象可测），预算数值先按原候选保留、由对应任务落地时实测定档，不在这里假装已测。
>
> **2026-09-05 真机门禁日复测（macOS arm64，dev + packaged 双形态）**：新增定档 **2 行**——**后台事件流量**（1000 token/秒 → **5 帧/秒**，红线 ≤6；`in=1001 out=6 merged=995`，合帧不丢内容/不改序 C8.2/C8.4 同过；**packaged 同数**）与**休眠→恢复**（LRU 腾位置+新建一路 **794ms**、唤醒读回同一 sessionId、全量关停 **161~165ms**，均在 ≤4s 内）；并把「每对话 RSS」行的口径修正为 child 自报 **184~185MB/对话、每多一对话净增 ≈1MB**。**仍无对象定档 5 行**：RPC 两跳 p95（探针当前只在事件里打时序、**没有 p50/p95 统计出口**，要补度量而非补预算）、3 路并发流式前台体感 与 主进程 CPU ≤60%（同时只有 1 路真流，stub 并发不上真渲染层）、切换首屏 ≤100ms（需要真 GUI 打点）、在飞 prompt/子代理峰值上限（④⑤需真模型灌流，`--concurrency-probe` 设计上不做）。这 5 行留「人工 GUI 目检 + 真模型」那一轮，**不拿单路数据或 SKIP 冒充已测**。
>
> **同日 T8.9 补度量后的修订（2026-09-05 晚）**：新增 `--latency-probe` 后，**「RPC 往返」「事件到达单跳」「审批往返」三行已有可自动判定的口径与无负载实测**（rpcRtt p95 0.093ms / eventHop p95 2.292ms / approvalRtt p95 0.087ms，7/7 EXIT=0）→ 上面的「5 行无对象」**缩减为 3 行**：3 路并发前台体感、主进程 CPU、切换首屏（三者都要真窗口/真渲染层），外加「在飞峰值计数」需真模型灌流。**注意别读错**：`eventHop` 只是 child→main 那一跳，main→renderer 一跳仍待接进 `--ui-chat`/面板，那半条没闭合就不许把整行标 ✅。
>
> **同日 T8.10 带窗探针后的再修订（2026-09-05 午后）**：`--ui-latency-probe`（可见窗口 + 三路 16ms 节流合成负载 + 点真标签切换）把剩下三行也定了档——**第二跳 p95 1.872ms、帧间隔 p95 9.5ms、主进程 CPU p95 3.683%、切换首屏 p95 8ms**（5/5 EXIT=0），EnvironmentPanel 同时有了红线行（无样本显示「待采样」）。于是「仍无对象」只剩两条：**① 在飞 prompt/子代理峰值计数**（需真模型灌流）；**② 真模型负载下的三路体感**（合成事件不含 token 生成与真实高亮成本，这一条不许拿合成数据宣布达标）。**新增一条纪律**：红线数字必须绑定负载形状——同一份代码在 600 帧无间隔突发下第二跳 p95 从 1.87ms 变成 261ms，那是排队延迟，把它当产品慢就是量错东西。

| 维度 | 预算 | 度量口径（怎么自动判） | 保障手段 |
|---|---|---|---|
| 每对话净增 RSS（含引擎子进程 + 其扩展/子进程） | **≤ 300MB/对话、3 路 ≤ 1GB**（T8.0 实测 child 自报 RSS **198~225MB/对话**，含 5 个已装社区包 + MCP 适配器；3 路合计 ≈600~675MB，预算有余量。**09-05 复测 dev**：child 自报 **184~185MB/对话、每多一对话净增 ≈1MB**——两数差在装配口径（09-05 探针 stub caps 不载社区包），均远在线内） | 探针轮询 `utilityProcess` 与主进程 RSS，取「起第 N 个对话前后的进程树 RSS 差」→ **实测口径修正**：tasklist 按 PID 取 RSS 在本机解析不出值（探针记 `pidRSS=0` 属口径失效、非无占用），改**以 child 自报 `process.memoryUsage().rss` 为准** | 活跃进程硬上限（默认 3，可配 1~4）+ LRU 关停 |
| 对话冷启动（fork + 载 SDK + 载扩展 + `bindExtensions`） | **≤ 3s**（T8.0 实测 **≈1.1~1.2s**：fork→ready 75~88ms + 装配 1089~1147ms，其中 SDK import ≈590ms、services ≈445ms、session 16ms、bindExtensions 5ms；首帧引擎事件 @1078ms，一轮真对话 settle 2872ms。**09-05 复测 dev**：fork→ready **60~68ms** + child 装配 **378~387ms**（services 15 / session 10 / bind 0，同探针不载社区包口径）） | 探针打时间戳：fork→`ready` 帧→首个 `agent_start` | 引擎惰性起、worktree 后台预建、启动期显「正在准备引擎…」 |
| RPC 往返延迟（child→main→renderer 两跳） | 事件到达 ≤ 20ms p95；审批往返 ≤ 40ms p95（预算不变） | **T8.9 已落地口径**（`electron . --latency-probe`，环形窗口 + 最近秩 p50/p95，**count=0 一律不判达标**）：`ping` 空操作量 main→child→main 纯通道；事件帧带 `sentAt`（`globalThis.performance.timeOrigin + now()`）量 child→main 单跳；审批往返由 **child 单时钟自计**并只取 `auto:true`（未走用户卡）样本——人的思考时间不进通道红线；只读命令才有样本（`prompt/start/compact` 等含模型耗时的不入表）。**09-05 真机实测**：`rpcRtt` p95 **0.093ms**（n=300，max 0.275）、`eventHop` p95 **2.292ms**（800 帧突发灌 512 窗口，max 2.462）、`approvalRtt` p95 **0.087ms**（n=60）、脏样本 invalid=0/512 ⇒ 三条均在预算内。**⚠ 半量已由 T8.10 补齐**：`eventHop` 只是「两跳」的第一跳，第二跳（main→renderer）由带窗探针 `--ui-latency-probe` 量得 **p95=1.872ms**（n=94，16ms 节流负载）⇒ 整行闭合。**顺带一条口径教训**：同一份代码在 600 帧无间隔突发下第二跳 p95 会飙到 261ms——那是**排队延迟不是产品慢**，红线数字必须绑定负载形状（`debugEcho.gapMs`）。 | 批流（多事件一帧）、结构化克隆而非 JSON、高频 delta 走合并通道（T8.3 已落 200ms+32KB 合帧） |
| 后台对话事件流量 | **已定档 ≤ 6 帧/秒/对话**（**09-05 真机 dev + packaged 双形态同数：后台 1000 token/秒 → 5 帧/秒**，`in=1001 out=6 merged=995`，IPC 降幅 ≈99%；同探针 C8.2 合帧不丢内容、C8.4 里程碑前缓冲已冲净 ⇒ 顺序不变量成立） | 主进程 IPC 计数器（每对话每秒帧数、每帧聚合条数） | text/thinking delta 按 **200ms 时间轴 + 32KB 字节兜底**聚合（T8.3 实测把计划原写的「4KB」否掉：帧 ≈90B 时 4KB 会在窗口内反复提前出帧 ≈22 帧/秒，直接破红线）；active 对话完全旁路逐 token |
| 3 路并发流式时前台对话体感 | **已定档（合成负载）** 无可感知掉帧（rAF 帧间隔 p95 ≤33.3ms）、主进程 CPU ≤ 单核 60%。**09-05 带窗探针实测**：三路 × 16ms 节流（≈180 帧/秒合计）下 `frameGap` **p95=9.5ms**（n=987，max 16.8 ⇒ 满帧无掉帧）、**主进程 CPU p95=3.683%**（单核）。**⚠ 口径边界**：负载是 `debugEcho` 合成事件，**不含模型 token 生成与真实 markdown/高亮成本**，真模型三路体感仍归人工那一轮 | `--ui-latency-probe`：渲染层 rAF 帧间隔自测 + 主进程 `process.cpuUsage` 差值 ÷ 墙上时间，**只在有样本的活动窗口里计入**（空闲期计入会把 ≤60% 稀释成好看），批量 2s 上报入环形窗口 | 仅 active 对话逐 token 上屏；后台只更摘要徽章（≤2Hz）；帧表空闲 600ms 自动停（不留常驻 60fps 循环） |
| 切换已激活对话首屏 | **已定档（合成负载）** ≤ 100ms。**09-05 实测 p95=8ms**（n=6，负载进行中点真标签切换） | `--ui-latency-probe`：**起点在渲染层 store 的 `setActiveConversation`**（`markSwitchStart`），App 在切换 effect 的下一帧配平——⚠ 主进程自己改 active 测不出（既不记起点、也不改渲染层可见性判定），必须点真标签 | store 分片常驻、不整表重读 jsonl |
| 休眠→恢复（关停进程→重 fork→`switchSession`） | **已定档 ≤ 4s**（**09-05 真机**：LRU 腾位置 + 新建一路 **794ms**（含等 child 优雅退出），唤醒后 `getSessionId()` 读回**同一 sessionId**=上下文接回成立；全量关停 **161~165ms**） | 探针计时 | 只休眠非 streaming；恢复即续 jsonl |
| worktree 创建 / 回流 | 创建 **≤ 1s**（T8.0 实测：小仓库 64~69ms、真实 pi-wood 仓库 **151~180ms**，工作树 3.3MB 且**不复制 node_modules**）；回流 `git apply --3way` **≤ 1s**（实测 36ms） | 探针跑 `git worktree add` 计时（含 node_modules 是否被复制的坑） | 惰性建树、复用 `.pi-wood/worktrees/`、只 diff 不复制依赖。**⚠ T8.0 实测追加口径**：unified diff 的空行上下文=一行只有前导空格 → git 2.54 的 `git apply` 判 trailing whitespace 并**非零退出但内容已落盘**；T8.6 固定用 `--3way --ignore-whitespace`（实测退出码归零）并以内容/diff 复核，不得只看退出码 |
...
| 残留进程数 | 关闭全部对话后回到基线（**MCP 子进程零残留**）——T8.0 实测：4 个 child（含硬崩对照组）全部离场，`node.exe Δ0 / python.exe Δ0`；`electron.exe Δ1` 系硬崩留下的 crashpad 处理进程，属对照组副作用非引擎泄漏。**⚠ 09-05 口径限制**：tasklist 计数是 **Windows 专属**，macOS 本机 `--concurrency-probe` ⑦ 与 `--conversation-probe` C4.2 均显式记 **SKIP**——**本行在 mac 上不算已通过**，只由「起过的 PID 逐个查活 + child 自报退出 + 注册表清空」间接证到（dev 14/14、packaged 21/21 均如此），Windows 那侧的硬断言留给下轮 Windows 门禁 | 探针起/关 3 轮后 `ps`/`tasklist` 计数比对（T8.0 已按此口径实现：另加「我们起过的 PID 逐个查活」，避免总数差被无关进程污染） | `stop()`→`runtime.dispose()` 修复 + 进程退出后看门狗扫残留（T8.5）。**T8.0 佐证**：child 侧走 `runtime.dispose()`（广播 `session_shutdown`）后 1ms 内退出、MCP 子进程无残留 |
| 单对话体感 | **不得回退**（流式延迟、审批往返、`ctx.ui` 阻塞往返） | 并发数=1 跑 `--ui-chat` 门禁并与改造前逐位对照 | D 的最大风险点：多两跳 RPC，必须用并发=1 的 A/B 对照守住 |

### [x] T8.P **全批第 0 步**：主进程产物由 CJS 切 ESM（前置，先于 T8.0）（✅ 2026-09-03：desktop `package.json` 加 `"type":"module"`；`electron.vite.config.ts`/`main/index.ts`/`engine-manager.ts` 全部 `__dirname`→`import.meta.url`；preload 路径改 `../preload/index.mjs`；**验收靶子达成**——`sdk-adapter.ts` Pi 改静态 `import * as PiSdk`（ESM 下合法），余 5 处动态 import 保留并各注理由。⚠ **实测推翻本任务两处假设**：electron-vite@2.3.0 的 `[name].mjs` 改名**只作用于 preload**，main 产物仍是 `out/main/index.js`（format=es，type:module 下 `.js` 即 ESM）→ `package.json.main` **不得**改 `.mjs`；「Pi 5 个入口」实为 6 处源点。门禁 9 项全绿（typecheck/test/build/产物 grep 断言 main 0×"use strict"·0×__defProp·0×require(·静态 Pi=1/三探针/ui-stress/capture/**ui-chat 真机对话**/package:dir 产 `release/win-unpacked`），证据 `docs/proofs/ui-v3/t8p-gate.log` + `t8p-{capture,ui-stress,ui-chat}.png`。**⚠ 打包态仅产 `--dir` 留档，安装运行未验（T5.3 环境阻塞不变）**；packaged 下子代理 entry 相对路径不存在=切换前既有遗留，留 T8.0/P1-d。门禁期间修掉两起环境问题：主机丢 `@pi-wood/plugin-api` 三处 workspace junction（`pnpm install` 复原）、旧包名死链接 `@pidesk/*` 三处令 `@electron/rebuild` ENOENT 挡 `package:dir`（已 `Move-Item` 入 `.trash`，未硬删）。记录见 §8）
...
- 步骤（改动面很小，逐项验）：
  1. `apps/desktop/package.json`：加 `"type": "module"`；~~`main` 由 `out/main/index.js` → `out/main/index.mjs`（因 electron-vite 改产物扩展名）~~ **⚠ 实测作废：`[name].mjs` 改名只挂在 preload preset 插件上，main 侧仅把 `output.format` 设 `es`、入口名仍 `[name].js`（type:module 下 `.js` 即 ESM）→ `main` 保持 `out/main/index.js`，改成 `.mjs` 会指向不存在文件**
  ...
  6. **解禁一处静态 import 作为验收靶子**：`packages/engine/src/sdk-adapter.ts:51` 的动态 import 改静态 `import`（Pi 是 ESM-only，CJS 下必崩、ESM 下合法）——**这一处改动能 build + 真机跑通，才算真的切过去了**；其余 ~~4 处~~ **5 处**动态 import（实测源点：index/extension-probe/project-manager/provider-manager 4 处 `await import` + session-service 1 处 `return import`）保持动态，并在注释里说明为何保留。**⚠ 实测修正保留理由的措辞**：sdk-adapter 静态化后 Pi **已随启动图加载**，故这 5 处的动态写法不再省启动成本、只命中模块缓存——保留是为维持「先取 agentDir 再注册数据域 IPC」等注册时序与既有惰性结构，改动面最小（各注释已按此如实写「非格式原因」）
  7. 文档与记忆同步：~~MEMORY 里「第一方代码别静态 import 进 bundle（esbuild 转 require 即崩）」一条~~ **⚠ 该条已不在 MEMORY.md（历次压缩移除），改为新增一条「T8.P 后主进程=ESM，静态 import Pi 合法；旧禁静态规则仅限 CJS 产物」**，§8 记偏差；`apps/desktop/DESIGN.md`/`PRODUCT.md` 若有构建描述一并对齐（实测 grep 无 CJS/构建描述，无需改）
- 验收：
  - [x] `pnpm -r typecheck` 全绿（TYPECHECK_EXIT=0）；`pnpm build` 三目标成功（BUILD_EXIT=0），且 **`out/main/index.js`（实测 main 不改名，见上）/`out/preload/index.mjs` 内不再出现 `"use strict"` + `__defProp` 的 CJS 序言、含 `import`/`export` 语句**（grep 断言，别靠肉眼）→ 实测 `main_use_strict=0`、`main___defProp=0`、`main_require_paren=0`、`main_static_pi_import=1`、`preload_use_strict=0`、`preload_has_import=True`
  - [x] dev 真机启动闭环不回退：选项目 → 对话 → 工具卡 → 审批 → 终端(node-pty) → 浏览器(playwright) → 代码高亮(shiki) 全部可用 → `--ui-chat` EXIT=0（`t8p-ui-chat.png`：思考折叠+预览、连续工具分组「2 个工具调用·全部成功」、行内代码高亮、右栏联动齐备；node-pty/playwright 的 ESM named-import 互操作经 build+capture+ui-chat 覆盖，未触发 `createRequire` 退路）
  - [x] 探针全绿：`--plugin-probe`/`--goal-probe`/`--memory-probe` 仍 EXIT=0，`--ui-stress 10000`（`t8p-ui-stress.png`）与 `--capture`（`t8p-capture.png`）双布局复验通过
  - [ ] 子代理仍生效：`agent_start` → 社区/自建 agent 委派链可用（验 `pi-wood-subagent-entry.ts` 经 jiti 在新路径基准下仍被加载）——**dev 形态路径基准已由 `--capture`/`--ui-chat` 启动链路证到（`existsSync` 走同一路径表达式），但委派链本身本轮未跑**（需密钥+真人触发），留 T8.0/P1-d 一并复验，不拿相邻证据冒充
  - [x] 打包态至少产一次物并记录：`pnpm package:dir` EXIT=0，产 `apps/desktop/release/win-unpacked/`（含 asar 完整性写入）；**⚠ 安装/运行仍属"打包态未验"**（T5.3 环境阻塞：无干净 VM/签名/三平台）
  - [x] `sdk-adapter.ts:51` 静态 import 后 `git grep -n "await import(\"@earendil-works"` 余 4 处且各有保留理由的注释（实测 4 处 `await import` + session-service 1 处 `return import`，5 处均带「T8.P 保留动态 import（非格式原因）」注释）
...
### [x] T8.0 前置探针：utilityProcess 引擎可行性 + worktree 生命周期（D 方案的 Go/No-Go 门禁）（✅ 2026-09-04：**双探针全绿 → 判 Go，D 方案成立**。P1（`electron . --engine-process-probe`，dev 形态）**14/14**：child 内工具集与主进程内基线**逐个全等**（15 个，含 5 个社区包工具 ask_user_question/todo/web_search/fetch_content/mcp/plan_mode_*、Skill mcp-scripting、`diagnostics` 0 error）→ 「utilityProcess 里扩展加载等价于主进程」得证；双向 RPC 三链路闭环（事件流 290 帧、反向 `tool:execute`、反向 `ui:confirm/select/input`）、`cancel` 帧可中止在跑的一轮、child 硬崩只掉自己（主进程与另一 child 照常响应）；成本 fork→ready 75~88ms、装配 1.1s、child RSS 198~225MB/对话；`runtime.dispose()` 后 1ms 退出、4 个 child 全部离场、`node/python.exe Δ0`（MCP 零残留）。P2（`node worktree-probe.mjs`）**20/20**：worktree 建 64~69ms（真实仓库 151~180ms）、**不复制 node_modules**（工作树 3.3MB）、两树 diff 互不污染、两份改动可先后 `git apply --3way` 回流、同行冲突第二份被拒且原因可读（不自动合）、remove+prune+删分支后彻底回基线、会话目录按 cwd 编码分家、非 git/脏树/detached/submodule/超长路径/大小写六类降级均有显式文案。**⚠ 三条实测发现（写进下游任务，不许当噪音）**：① `git apply --3way` 对含空行上下文的补丁会**非零退出但内容已落盘**（git 2.54），T8.6 固定加 `--ignore-whitespace`（实测归零）并以内容复核；② **打包态 child 入口缺席**（`files` 只有 `out/**`），而直接往 `files` 加源码路径会让 electron-builder 把工作区依赖映射算错（`packages/engine/package.json must be under apps/desktop`，`package:dir` 直接失败）→ **T8.1 前置改动**：构建时把 `engine-child.mjs` 拷进 `out/main/` 并对其 `asarUnpack`（不能用 `extraResources`——落到 app 树外解析不到 `@earendil-works/*`）；③ `utilityProcess.fork` 的 `env` 值含 `undefined` 会直接 `Invalid value for env`（已修并注释）。另：`Electron proc.pid` 在本机取到 `undefined`，改由 child 自报。顺带得到 T8.P 的打包侧证据：**packaged 主进程 ESM 能正常起**（读设置、注凭据、落日志）。证据 `docs/proofs/T8.0/{engine-process-dev,worktree-probe,packaged-rerun,runner}.log` + `release/win-unpacked/T8.0-proofs/engine-process-packaged.log`。记录见 §8）
...
- 验收：
  - [x] `electron . --engine-process-probe` **EXIT=0**，打印 P1-a~d 全部数据（ms/RSS/进程数/残留计数）→ dev 形态 14/14 全过、`ENGINE_EXIT=0`，数据见 `docs/proofs/T8.0/engine-process-dev.log`。**packaged 形态按设计不过**（P1-d child 入口未随包，见发现②），且 GUI 子系统 exe 的退出码 PowerShell 取不到（`$LASTEXITCODE` 不可信）→ 已改为**读 exe 同目录落盘日志**判定，并在日志里显式声明「打包形态失败=资源缺口，不改写 Go/No-Go」
  - [x] child 内扩展工具集与主进程一致（社区包工具出现、`diagnostics` 无 error），否则判 **No-Go → 走退路** → 实测 15 个工具与主进程内基线**逐个全等**（比对时同口径剔除探针自注册的 `probe_host_echo`）、5 个社区包工具 + Skill `mcp-scripting` 均在、`diagnostics` 0 error ⇒ **判 Go，不退路**
  - [x] 双向 RPC 三条链路闭环（事件流、`tool:execute`、`ui:confirm`）+ child 崩溃不影响主进程 → 事件流 290 帧、反向 `tool:execute` 与 `ui:confirm/select/input` 全闭环、`cancel` 帧可中止在跑的一轮；child `process.crash()` 后主进程与另一 child 照常响应 RPC
  - [x] **零残留**：3 轮起停后进程树与子进程数回到基线（这是 `session_shutdown` 修复是否有效的硬证据）→ 4 个 child（含崩溃对照组）全部离场、`node.exe Δ0 / python.exe Δ0`；`electron.exe Δ1` 为硬崩的 crashpad 副作用（单独记录，非引擎泄漏）。child 侧走 `runtime.dispose()` 广播 `session_shutdown` 后 1ms 退出
  - [x] `--worktree-probe` **EXIT=0**：两对话同项目并行改同文件互不干扰、`git apply` 回流成功、清理彻底、非 git/脏树/submodule 均有显式降级提示 → 20/20 全绿（`node worktree-probe.mjs`）；另加测 detached HEAD、Windows 超长路径、大小写折叠三类
  - [~] 性能红线表的 ⏳ 占位全部回填真数 → **实测定档 4 行**（每对话 RSS 198~225MB、冷启动 ≈1.1~1.2s、worktree 建 151~180ms/回流 36ms、残留 Δ0）；**RPC p95 / 后台事件流量 / 3 路并发体感 / 切换首屏 / 休眠恢复 / 在飞上限 6 行 T8.0 无对象可测**（要等 T8.1/T8.3/T8.5 接线），已按 §0 规则在表头与本行如实标注为「待对应任务实测定档」，不拿占位当已测。「活跃进程上限默认 3」按 RSS 实测**维持不变**（3 路 ≈600~675MB < 1GB 预算）
  - [x] Go/No-Go 结论 + 全部偏差写入 §8 决策日志（No-Go 时按退路重写本节，不许就地妥协）→ 见 §8 2026-09-04 T8.0 行（含 3 条实测发现 + 2 条度量口径修正）
- 验证方式：`pnpm --filter @pi-wood/desktop probe:engine-process` + `probe:worktree`，退出码即判据；数据以表格回填本节

### [x] T8.1 EngineHost 子进程化 + 对话注册表（ConversationRegistry）
...
- 验收：
  - [~] 单项目 2 对话 + 跨项目 1 对话并存：三边各自流式跑完、互不打断 → **跨项目 3 对话并存已证**（`--conversation-probe` C1：三路独立 pid、工具集逐个全等、三路装配 3.9~4.1s）；**同项目 2 对话已于 2026-09-05 真机证到并存**（`--concurrency-probe` ①：同项目 ×2 + 跨项目 ×1，三句柄同时 alive、② 切换不打断 `dead=0`）；**⚠ 仍半闭**：探针走 stub，「三边各自**真模型流式**跑完互不打断」需真流并行 → 留人工那一轮
  - [x] 同项目两对话的 **cwd 不同**（worktree 生效），扩展模块态互不可见 → **2026-09-05 真机证到**：`--concurrency-probe` ① 断言三对话 cwd 去重=3（`[b35ef76b/bbe22d72/8ffe484d]`，两棵同项目树 + 一棵跨项目树），EXIT=0；模块态隔离由「每对话独立进程 + 独立模块图」（C1.2/C1.3 + child 内 `globalThis.__piwoodSubagentBridge` 天然每进程一份）+ 本条 cwd 互异共同闭合
  - [x] 超过 `MAX_LIVE_ENGINES` → 最旧非在跑对话被关停，唤醒后上下文/模型/thinking 档位完整恢复 → C2（上限 2 时挤掉 2 条 idle、活跃数回 2、被休眠者 child 确认退出、唤醒后读到同一 sessionId）；档位重放挂在注册表的 `onConversationReady` 钩子（首建/唤醒/崩溃重启都走），用户显式选过的模型+思考档位按对话记在 `perConversationPrefs` 并重放（模型不支持该档位时回落最低档并 toast，不静默）
  - [x] `suspend` 只在 `agent_settled` 边界发生；streaming 中的对话永不被自动关停 → `conversation-core.isSuspendSafe`（仅 `idle && !inFlightPrompt && pendingApprovals===0`）+ 38 例穷举单测覆盖；全在跑时按 `planAdmit` 拒绝新建并给原因，不静默杀任务
  - [x] **反复 suspend/resume/close 不积累资源** → C4（全量关停后 `node.exe/python.exe` 计数 Δ0、注册表无活跃句柄）；`stop()→runtime.dispose()` 修复有独立回归单测 `packages/engine/src/runtime-dispose.test.ts`（4 例：先 abort 再 dispose、无 runtime.dispose 时退回并可检出未广播、abort 抛错不阻断、未启动不抛）
  - [x] child 崩溃只砸坏该对话（其余对话与主进程存活、可恢复）；主进程退出不留孤儿 → C3（kill 一个 → 退避重启换新 pid、其余 alive、主进程帧管道仍可用）+ C4；孤儿侧双保险：child 监听 `parentPort close` 自清，主进程 `window-all-closed` 广播 `shutdown` 后再退（强制退出窗口 1.5s→4s）
  - [x] 9 个代理工具在 child 侧可被模型调用且结果正确回传；工具名全部过 OpenAI 兼容正则 → C1.5 证 9 个代理工具进了 child 的 session 工具集；**真机端到端**由 `--ui-chat` 实跑证到（`docs/proofs/T8.1/t81-ui-chat.png`：思考+2 个工具调用全部成功+完整回答）；名字校验前移到主进程 `hostToolMap()`（启动即失败）
  - [x] `conversation-core` 状态机 + LRU + seq 对账 + 帧编解码纯函数 `node --test` 穷举 → `conversation-core.test.ts` 38 例（含 7×7 迁移矩阵、丢帧/重复帧、退避上限）+ `engine-rpc.test.ts` 26 例（含跨方向帧互拒、热路径 loose 解码的畸形输入表）
  - [x] `pnpm -r typecheck` + `pnpm -r test` + `pnpm build` 全绿；探针回归 EXIT=0 → 主机门禁：typecheck EXIT=0、test EXIT=0、build EXIT=0、`probe:conversation` EXIT=0（17/17）、`probe:engine-process`（T8.0 回归）EXIT=0、`probe:worktree` EXIT=0、`package:dir` EXIT=0、**packaged 形态 `--conversation-probe` 17/17 EXIT=0**
- 验证方式：`pnpm --filter @pi-wood/desktop probe:conversation`（新增，dev 与 `:packaged` 两条）+ 单测 + `--engine-process-probe` 复跑；真机对话 `electron-vite dev -- --ui-chat`
- **实测数据（dev / packaged 各一轮）**：单路装配 461~574ms（services ≈450 / runtime ≈456~480 / bind 5ms）、三路并存总装 3.9~4.1s、child RSS **197~216MB/对话**、腾位置+新建一路 1.6~2.3s、全量关停 193~198ms、残留 Δ0
- **三条偏差（已按实改文档，不留计划与实现打架）**：① 计划写「child 入口拷进 `out/` 并 `asarUnpack`」→ 实际改成 **main 的第二 rollup 入口**（`out/main/engine-child.js`，随 `out/**` 进 asar），**不需要 asarUnpack**：packaged 探针实证 `utilityProcess.fork` 可直接从 `app.asar` 内加载 ESM 入口并按 app 根解析 `@earendil-works/*`（`extraResources` 反而会把依赖解析弄丢）；② 计划写「高频 delta 在 child 侧按 50ms/8KB 合帧」→ **推迟到 T8.3**（那才有可见性口径与流量度量对象），本轮 child 逐帧上抛，热路径只做 `decodeFrameLoose`（不做逐帧 zod）；③ 计划写「上行 `event{conversationId,…}`」→ 改为 **child 只报 `seq`，会话归属由主进程按 handle 附加**（不信任 child 自报身份，deny-by-default 的前置），`inject-tool-result` 帧取消（宿主工具走反向 RPC 请求/应答即可）

### [x] T8.2 IPC 契约加 conversationId + 事件按对话路由
...
- 验收：
  - [~] 两对话同时流式，各自 store 只收到自己的事件（探针逐条断言 sessionId/conversationId 匹配、零串台）→ **归属链路与路由判定已实现并有纯函数单测**（`engine-envelope.test.ts` 7 例：`active` 权威信号、legacy 裸事件、跨对话判外）；主进程侧每条对话的事件都带自己的 `conversationId`+`seq`（T8.1 C1/C5 已证不混流）。**两路真模型同时流式的端到端串台断言**要等 T8.3 store 分片后由 `--concurrency-probe`（T8.8）补，本轮 `--ui-chat` 只证单对话全链路 envelope 不破
  - [x] 未传 `conversationId` 的旧调用路径行为与现状逐位一致（回归不破）→ 命令侧 `conversationId` 全部可选、缺省=active；推送侧 `unwrapEnginePayload` 认旧裸事件并标 `legacy`，`routeForConversation` 对 legacy/未选定对话一律 `apply`
  - [x] envelope schema 校验失败不丢事件（丢事件要计数 + warn，别静默）→ 三层各计数：主进程 `sendDrops`（无窗口/已销毁/发送抛错，首条 + 每 200 条 warn）、preload `engineEventDropped`（载荷非法，带 reason）、渲染层 `foreignEventCount`（判给别家，可观测不静默）
  - [x] child 上报的 `seq` 断号可检测并显式告警 → T8.1 的 `acceptSeq`/`applyEventSeq` 已把断号计入 `record.droppedEvents` 并 warn + toast，本批把 `seq` 随 envelope 透传到渲染层（`meta.seq`），为 T8.3 的对账留口
  - [x] `pnpm -r typecheck` + build 全绿 → 见本节末「门禁」
- 验证方式：契约纯函数单测（`engine-envelope.test.ts`：归一/退化/路由）+ `--ui-chat` 真机全链路 + `--conversation-probe` 回归；`--concurrency-probe` 串台断言随 T8.3/T8.8
- **两条偏差（按实记录）**：① 非事件通道里 `subagentRuns` 的载荷是**数组**，加 `conversationId` 字段就得改形状、会立刻破渲染层现有消费者 ⇒ 本批只给对象载荷（`subagentEvent`/`assistResult`）追加 `conversationId`+`projectDir` 字段（多余字段被旧消费者忽略），数组那条统一等 T8.3 多对话标签条一起换 envelope；② `EngineEventSchema` 原本**漏了两条宿主自造事件**（`user_message` 压测回显、`model_changed` 选模型通知）——它们一直在被推送和消费，只是没进契约，envelope 严格校验一上就把它们判非法 ⇒ 已补进契约（preload 的 lite 判定当时能吃是因为它不校验事件 type，权威版必须修，属 T8.2 顺带纠出的既有契约缺口）

### [x] T8.3 渲染层 store 按对话分片 + 可见性驱动节流（性能主战场）
...
- 验收：
  - [x] 3 对话并发流式：后台对话 IPC ≤6 条/秒/对话（探针计数）→ 机制判据实测达标：`--conversation-probe` C8.1 以「1 秒 1000 token」合成流跑真节流器 ⇒ **5 帧/秒**（`in=1001 out=6 merged=995`，IPC 降幅 ≈99%），红线 ≤6 通过；**真机三路同时流式的条数**要等 T8.8 的 stub 流并发探针（同项目多对话也还没开，见 T8.6）
  - [ ] 3 对话并发流式时前台无可感知掉帧、主进程单核 CPU ≤60% → **本批无对象可测**（同时只有 1 路真流），归 T8.8 用 stub adapter 造 3 路交错流后实测定档，不拿单路数据冒充
  - [~] 切到后台对话 → 首屏 ≤100ms 且内容完整 → 分片常驻内存，切换是本地 map 取值 + `flushOutbound` 冲掉缓冲（不等下一个 200ms 窗口），首屏成本与对话数无关；**但「期间增量与 jsonl 历史对账去重」未做**——后台对话的历史目前只来自事件流累积，未整读 jsonl（`mergeHistory` 的对账纯函数与单测已就位），真接线随 T8.6/T8.8 的对话标签条一起做
  - [x] 后台对话渲染不引发 active 组件重渲染（渲染计数断言）→ `session-store.test.ts` 断言：前台选定后，后台 100 条事件（含 50 token + 50 工具里程碑）让读前台 `items` 的订阅者 **0 次**回调，而前台自己 1 条新条目恰好 1 次（防「反向 bug：切过去看不到更新」）
  - [x] 各对话滚动位置独立保持；未读计数在切过去时清零 → `scrollTop`/`followBottom`/`unreadCount` 都在切片内；`MessageList` 滚动时写回、切换时按该对话的 `followBottom` 决定是否跟底；未读只在里程碑累加（token 不算），`setActiveConversation` 即清零（两条 store 单测覆盖）
  - [x] `pnpm -r typecheck` + `pnpm -r test`（含 store 归约纯函数穷举）+ build 全绿 → 主机门禁全 EXIT=0：`conversation-slice.test.ts` 17 例（交错/乱序/重复帧/工具生命周期/合帧顺序/历史对账/纯性）、`session-store.test.ts` 7 例、`engine-throttle.test.ts` 8 例、`--conversation-probe` **22/22**；`--ui-chat` 真机对话 + `--capture` 空态截图均绿（15 个消费文件迁移后 UI 无回归）
- 验证方式：`node --test`（归约/store/节流三组纯函数）+ `--conversation-probe`（含 C8 节流度量）+ `--ui-chat`/`--capture`；三路真流并发体感与 CPU 归 `--concurrency-probe`（T8.8）
- **实测数据**：后台 1000 token/秒 → 5 帧/秒（合并 995 条，IPC 降幅 ≈99%）；active 对话 200/200 原样透传（完全旁路）；全量关停 237ms、残留 Δ0（T8.1 口径不变）
- **三条偏差（按实改文档）**：① 计划写的「200ms 或 **4KB**」合帧阈值实测不成立——`text_delta` 帧约 90B，1000 token/秒 ⇒ 每 200ms 窗口 ≈18KB，4KB 会在窗口内反复提前出帧（实测 ≈22 帧/秒，直接破 ≤6 条/秒红线）⇒ 默认字节阈值改 **32KB**，只当突发大输出（bash 长输出）的兜底，常规切窗交给时间轴；② 里程碑出帧前必须**先把该对话缓冲按原顺序冲净**（否则渲染层先收到「工具结束」再收到它前面的 token ⇒ 内容错序），这条顺序不变量计划没写，已实现并被 C8.4/单测盯住；③ 步骤 2 说「16 个消费文件」，实测 **15 个需改**（`TerminalPanel.tsx` 只读项目级 `activeProject`，本就不该进分片选择器）；另 `handleEvent` 改为自带归属采纳（漏调 `noteEventOwnership` 不该导致静默错路由）

### [ ] T8.4 审批与 ctx.ui 按对话归属 + PromptTray 显示来源（🔜 2026-09-05：`--approval-probe` 真机 9/9 断言安全底线（deny-by-default 端到端 / 票据一次性 / 跨对话应答被拒 / 后台不被静默拒），四条验收里三条已闭；**只剩 PromptTray 来源行的 GUI 目检**）

> **实现摘要（2026-09-04）**：纯函数模块 `electron/main/engine/approval-ownership.ts`（`pendingKey` / `canRespond` / `shouldArmTimeout` / `checkApprovalTicket` / `acceptAllScope` / `groupOrderByConversation`，**18 例单测**）；契约 `packages/ipc-schema/src/engine.ts` 新增 `ApprovalRequestPayload/UiRequestPayload/ApprovalRespondPayload/UiRespondPayload` 四 schema（request 带 `conversationId`+`projectName`，null=插件全局项；respond 带 `conversationId`=应答者所处对话）+ `host:subagent guard-tool` 补一次性 `ticket`（child 侧 `randomUUID()`）。`engine-manager.ts`：pending 表 key 改 `${conversationId}:${seq}`、entry 记 owner+armed；**超时分档**——仅发起对话是 active 时起 120s 表，后台对话 pending 常驻，`setActiveConversation` 与新增 `approval:focus-requested` handler 都会 `armPendingTimeoutsFor()` 恢复计时；`approval:decide`/`ui:respond` **双保险归属校验**（canRespond + 主进程 active 比对，拒绝时 ui:notify 提示）；`acceptAll` 走 `acceptAllScope` 只放行 active 对话（无归属的插件 confirm 不再被连带放行）；`decideApproval`/`guard-tool` 消费 `consumedApprovalTickets`（同 ticket 二次使用=拒绝，拒绝也是裁决）；新增 `dropPendingFor()` 在 `closeConversation` 时回收未应答 pending。`capabilities.requestUi`/`EngineHostDeps.requestUi` 签名加 `ctx{conversationId,projectDir}`（归属由 host 按 deps 构造，child 不可自报）。PromptTray：来源行「来自对话：对话 N · 项目名」、队列按对话分组、**非当前对话的请求不抢焦点不自动展开**、动作按钮禁用 + 「去应答」切对话（store→IPC→主进程恢复计时）、折叠态红点标注外部请求。
> **门禁状态（2026-09-04 实跑）**：`tsc -p tsconfig.node.json`/`tsconfig.web.json` 双绿；desktop `node --test` **165/165 绿**（含新增 18 例）；`pnpm exec electron-vite build` **EXIT=0**（按 T8.P 沙箱配方补 linux 原生二进制：rollup/esbuild/@tailwindcss-oxide/lightningcss 落 `.pnpm/node_modules` + `ESBUILD_BINARY_PATH`）；产物断言——`out/main/index.js` 含 `approval:focus-requested`=1、`consumedApprovalTickets`=5、「跨对话审批」=2、`"use strict"`=0、`require(`=0（ESM 形态不变），`out/main/engine-child.js` 含 `ticket`=2，`out/preload/index.mjs` 含 `approval:focus-requested`=1。**仍待真机（本会话无 electron/GUI）**：`--conversation-probe` 回归、真机两对话并发审批目检。

- 来源：§8（T6.1 改造范围 ③）+ 现状核实「审批与 ctx.ui 队列按全局数字 id、无会话归属」行 + 用户 UI 偏好（交互统一贴 Composer，禁浮窗/模态）
- 前置：T8.2
- 步骤：
  1. `pendingApprovals`/`pendingUiRequests`（`engine-manager.ts:88-93`）key 由裸 `number` 改 `` `${conversationId}:${seq}` ``，并记 owner；`approval:respond`/`ui:respond` 校验「应答者必须是发起对话」（否则跨对话误放行＝安全旁路）→ [x] 单测 `canRespond` 5 例 + handler 双保险（自报归属 + 主进程 active 比对）
  2. `confirmViaRenderer`/`requestUi` 载荷加 `conversationId` + 对话标题/项目名；子代理 child 审批沿用 `guardChildTool` 路径但同样带归属 → [x]（对话标题暂以「对话 N」短标签代，正式标题随 T8.8 标签条）
  3. `PromptTray` 头部加来源行「来自对话：<标题> · <项目名>」（点击跳到该对话再应答）；队列按对话分组显示，**仍贴 Composer 正上方**、不加浮窗/模态 → [x]
  4. **⚠ 超时按可见性分档（现在的 120s 一刀切在多对话下会把用户没看到的任务静默判死）**：active 对话保持 120s 默认拒绝；**非 active 对话的 pending 不计时**（或降到小时级 + 到点自动暂停该对话而非直接拒），标签红点常驻，用户切过去后才恢复 120s 计时。`approval:respond` 之外新增 `approval:focus-requested`（点徽章直达该对话并滚到对应审批卡）→ [x]（取「不计时」档；红点落在 PromptTray 折叠态与来源行徽标，对话标签条本身归 T8.8）
  5. `approval:acceptAll`（T7.2）改为**只放行该对话**的 pending（现遍历全局会跨对话放行）→ [x] `acceptAllScope` 穷举单测；无归属的插件 confirm 一并不再被连带放行
  6. 标签徽章：该对话有待审批 → 红点；非 active 对话来审批时不改用户当前视图、不抢焦点（只在标签与徽章提示）→ [~] PromptTray 已做（折叠红点+不自动展开+不抢焦点）；对话标签条徽章归 T8.8
  7. **跨进程底线**：审批/`ctx.ui` 的结论**只能由主进程经渲染层应答注入 child**，child 侧 `guardChildTool`/`permissionGateExtension` 拿不到任何本地兜底放行路径（deny-by-default：无应答 = 拒）；`approval:confirm` RPC 帧带一次性 token 防重放 → [x] `host:approval`/`guard-tool` 均带 ticket 且主进程消费（重放=拒绝）；deny-by-default 结构未动
- 验收：
  - [x] A 对话的 bash 审批卡显示来源且只能被 A 的应答放行；在 B 对话里应答 A 的请求被拒（单测/探针断言 owner 校验）→ 单测 + **2026-09-05 `--approval-probe` D4b 真机断言**（拿别人的审批 id 从另一对话应答 → `respondApprovalFor` 返回 false 且 pending 保留给发起对话）
  - [~] 两对话同时待审批 → 队列两条、来源可分辨、各自超时互不影响 → **归属与超时独立已真机断言**（D4a 后台对话确有一条在飞 pending、D5a 它不被别人的超时窗口牵连），**只剩 PromptTray 来源行的视觉目检**（需 GUI）
  - [x] **后台对话的审批不会因用户看不见而被 120s 静默拒** → **2026-09-05 `--approval-probe` D5a/b/c 真机断言**：后台 pending 活过 2×超时窗口仍在（`pendingMs=1995ms`、`armed=false` 未起表）→ 切到该对话 `armPendingTimeoutsFor` 后 `armed=true` → 起表后超时落 `deny`（reason「用户拒绝该操作」）且 pending 出清。超时窗口经 `PIWOOD_APPROVAL_TIMEOUT_MS` 测试缝压到 900ms（**生产不设 = 120s，行为不变**）
  - [x] child 无本地放行路径：不发 `approval:respond` 时该工具必被拒（deny-by-default + 重放票据失效）→ **2026-09-05 `--approval-probe` 真机断言**：D1 反向通道清单封闭（四条，无任何「读策略/自批」方法）；**D2 端到端**——真 child 发 3 次 `host:approval`、无渲染层应答，**child 侧确实收到 3 个 `allow:false`**（回执随 `debugEcho` 结果带回主进程，不再只信主进程日志），2729ms 收敛不挂死；D3 同一 ticket 第二次被拒（「审批票据已消费（疑似重放）」）
  - [x] `acceptAll` 只清该对话 pending，别对话不受影响
  - [x] 审批仍在 Composer 上方 PromptTray 呈现（无新增浮窗/模态，符合用户偏好）
  - [~] `pnpm typecheck` + desktop test + build 全绿 → typecheck（node+web 两份 tsconfig）绿；desktop test **165/165** 绿；**`electron-vite build` EXIT=0**（沙箱按 T8.P 配方补 linux 原生二进制实跑）+ 产物断言（新符号进 main/child/preload、ESM 形态不变）全过；**真机侧已由 `--approval-probe` 补齐并入 `t88-verify.sh` 第 4 步**（原「并入 --conversation-probe」的建议改为独立探针——它要动 settings/env，混在既有探针里会污染 C 组的 stub 前提）
- 验证方式：归属校验纯函数单测 + `electron . --approval-probe`（真裁决路径，9 条断言）+ PromptTray 两对话目检（剩一项）

### [ ] T8.5 并发闸门：prompt 队列 / 子代理全局上限 / 第二运行时 per-对话化（⏳ 2026-09-04：代码+单测+typecheck+build 全过，真机探针/目检待跑——见下方逐项标注与 §8）

> **实现摘要（2026-09-04）**：纯函数模块 `electron/main/engine/concurrency-gates.ts`（`clampLimit` / `effectivePromptLimit` / **`SlotGate`**（FIFO 槽位闸：`wouldQueue`/`acquire` 回执是否排过队/`setLimit` 放宽唤醒等待者）/ 子代理闸状态机（`planChildRunAdmit`/`reserveChildRun`/`releaseChildRunReservation`/`applyRunsSnapshot`/`dropChildRunState`，**预约-快照对账**防跨对话竞态）/ `planGoalBegin` / `normalizeQuotaAction`+`quotaGuardEffect`，**19 例单测**）。接线：① **prompt 闸门**——`engine:prompt` handler 先 `setLimit(effectivePromptLimit(settings.engineMaxPrompts ?? 3, maxLiveEngines, {rateLimited}))`，`wouldQueue()` 时 `markPromptQueued`（注册表新 action，状态机 `queued`）+ 推 `queue_update` 事件，`acquire()` 后撤标继续执行（prompt 不丢），finally `release()`；② **子代理全局闸**——`decideApproval` 里 `toolName==="agent_start"`（vendor 事实：每 call 恰产 1 run）在策略 deny 之后、confirm 之前排队：`planChildRunAdmit` 超限则等 runs 快照唤醒（`childRunWaiters`），放行前 `reserveChildRun`，审批被拒/异常显式退还；`host:subagent runs` 快照到达时 `applyRunsSnapshot`（running 增量把 reserved 折算为 running）+ 唤醒排队者；`closeConversation` 出清该对话占用；③ **第二运行时**——assist 单飞改 per-对话（`generateAssist(conversationId,…)`，同对话跳过、跨对话经共享单槽 SlotGate(1) 排队不吞），采集缓冲改 per-对话 `assistBufs`（后台也采集），**后台 settled 只暂存 `pendingAssist`、切回该对话时补生成一次**；review/goal-audit 的「后到吞掉」改 SlotGate(1) 排队（⚠ 计划写 ≤2，但共享 adapter/buf 单槽真实并发 2 会互污染缓冲 ⇒ 实取 1，上界内）；④ **goal 互斥**——`listActiveGoalSessions`+`planGoalBegin`，`beginGoal`/`resumeGoal` 冲突即抛错（goal.ipc 捕获 → `ui:notify` toast + 返回当前状态；`force:true` 可强制接管=自动暂停前者，IPC 参数已备、正式冲突 UI 随 T8.8）；⑤ **btw per-对话化**——`btwAdapter` 单槽改 `btwByConversation` Map（修隐性串台：B 的 /btw 会拿到 A 的 cwd 上下文），随 `closeConversation` 释放，换项目不再统一关停；⑥ **残留看门狗**——T8.1 已落地（child exit 后 2s 扫描 + toast 只暴露不偷杀），本批零新增；⑦ **成本护栏**——settings `quotaOverLimitAction`（warn 缺省/throttle/block）：throttle → prompt 限临时降 1，block → `createConversation` 拦新建并提示，用量页告警复用 T7.12。子代理上限 settings：`subagentMaxTotal`（6）/`subagentMaxPerConversation`（4）；prompt 上限 `engineMaxPrompts`（3，与 maxLive 取 min）——三者均 clampLimit 归一（非法值回落默认，0/空串不静默变 1）。
> **门禁状态（实跑）**：desktop typecheck（node+web）双绿；`node --test` **184/184**（新增 19 例）；`electron-vite build` **EXIT=0**（T8.4 沙箱配方复用）；产物断言——`out/main/index.js` 含 `agent_start 排队`=1、`quotaOverLimitAction`=1、`engineMaxPrompts`=1、`阻止新建对话`=1、「强制接管」=2、`queue_update`=3，`"use strict"`=0、`require(`=0（ESM 不变）；ipc-schema / engine / plugin-api 三包测试全绿（≈`pnpm -r test`）。**仍待真机**：`--concurrency-probe` 计数断言（第 4 个 prompt 排队 / 双对话子代理闸 / 切回补生成 assist / 双 goal 互斥 toast / btw 隔离 / 零残留）——本会话无 electron/GUI，如实标注。

- 来源：现状核实「子代理运行时单槽 + 全局桥」「btw 单槽」「三个第二运行时 module 单例 + 单飞」「子代理并发无上限」四行 + 性能红线「在飞 prompt / 子代理 run」
- 前置：T8.1、T8.2
- 步骤：
  1. **prompt 闸门**：全局 `inFlightPrompts ≤ 3`（设置项 1~6），**并与「活跃引擎子进程数 ≤ `MAX_LIVE_ENGINES`」取交集**（进程数才是真正的资源上限，prompt 闸是成本/限流闸），超限的 prompt 入该对话队列并显「排队中」；与 provider 429/退避联动——连续限流时临时降到 1 并发并在用量页提示（复用 T7.12 `quotaWarnings`）→ [x]（交集=min；「连续限流」信号本轮取 quotaWarnings 非空 + throttle 档 ⇒ 降 1，429 事件级探测留后续）
  2. **子代理全局闸**：跨对话共享 `MAX_TOTAL_CHILD_RUNS`（默认 6）+ per-对话 ≤4，在 `agent_start` 接缝（child 侧 `subagent/bridge.ts` / `guardChildTool` 同层）**排队而非直接 spawn**；不改 vendored 内核语义、也不违反其 ADR 0001（它是「无上限」的设计，我们在宿主侧加闸门即兼容）。`__piwoodSubagentBridge`（`engine-manager.ts:210`）与 `subagentRuntime` 单槽（`:79`）随进程化天然变成**每 child 一份**，主进程只做跨进程聚合（成本汇总由「对 `runs.list()` 全量求和」改为「对各对话上报的 per-child run 求和」，T6.6 语义不变）→ [x]（接缝落在 child 主会话审批门 `host:approval`：`toolName==="agent_start"` 排队；bridge/runs 镜像 T8.1 已 per-child，成本汇总 T6.6 已是跨对话求和，本批未再改）
  3. **第二运行时策略（⚠ 别把修 bug 做成成本放大器）**：`assist-service`/`review-service`/`goal-audit` 的 `inFlight` 从「全局一个、后到静默跳过」改为 **per-conversation 单飞**（修隐性坏掉），但**默认只为 active 对话生成会话辅助**（T7.9 每轮 settle 一次＝N 倍成本），后台对话在**被切回时补生成一次**；AI 审查/goal 审计保持「谁点谁触发 + 全局并发 ≤2 + 排队可见」，UI 必须有明确状态而非 null → [x]（审查/审计实取并发 1——共享单槽缓冲的硬约束，见摘要⚠；「排队可见」本轮=调用挂起等待 + 既有 spinner，显式队列深度 UI 随 T8.8）
  4. **goal 模式互斥**：`settings`/`goal-runtime` 加「同时只允许一个对话处于 goal active」（目标模式本就每轮自动续跑，×N 对话＝成本乘积失控），第二个对话开 goal 时给出显式提示（可强制接管并暂停前者）→ [x]（提示=ui:notify toast；`force` 参数已通到 IPC，接管按钮随 T8.8）
  5. **btw 并入注册表**：`btwAdapter` 作为 `kind:"btw"` 的对话句柄，随所属对话 close 一起释放（现 `:201` 只在换项目时 `closeBtw`，多对话下会漏）；进程化后 btw 的 denyAll + 静默 uiBridge 语义保持不变（子进程天然隔离，比今天更强）→ [x]（以 `btwByConversation` Map 挂对话 id，close 连带释放；未做成独立 `kind:"btw"` 注册表句柄——btw 无独立 utilityProcess，语义等价）
  6. **残留看门狗**：对话 close/suspend 后 2s 扫该 child 派生的进程树，发现残留（MCP/子代理 shell）→ 记录 pid + toast 暴露（先可见，再谈自动收），并把计数交给探针断言「零残留」→ [x]（T8.1 已落地：`onExit` → `residueCheck` 2s 窗口、只暴露不偷杀、C4 断言 Δ0；本批零新增）
  7. **成本护栏**：对话标签悬浮/右键显示该对话 token/费用（`usage-tracker` 已按 sessionId 分键，零改造复用）；超月配额时按设置「降并发 / 阻止新建对话 / 仅告警」→ [~] 配额三动作已接线；**按对话显示 token/费用依赖标签条（T8.8）**，本批未做 UI
- 验收：
  - [~] 第 4 个并发 prompt 被排队（探针断言在飞数 ≤ 上限、被排队的最终执行、不丢）→ 机制+单测完成（SlotGate FIFO / effectivePromptLimit / markPromptQueued / queue_update 事件）；探针待真机
  - [~] 两对话各起子代理：全局 child run ≤6 且 per-对话 ≤4、归属对话正确、`runs` 事件不串台 → 预约-快照对账纯函数穷举 + agent_start 接缝接线；探针待真机
  - [~] 两对话各自点 AI 审查 → 两个结果都返回（不再互相吞）；后台对话 settled 不产生 assist、切回后补生成 → 排队语义 + per-对话单飞 + 切回补生成已接线并单测；真机双审查目检待
  - [~] 同时刻最多 1 个 goal active；强行开第二个有明确提示且前者被暂停（不静默双跑）→ `planGoalBegin` 单测 + begin/resume 接线 + toast；真机目检待
  - [~] `/btw` 在两个对话各开一个，关一个不影响另一个，且对话 close 后第二运行时被回收（无进程/会话残留）→ per-对话 Map 接线完成；真机目检待
  - [x] **零残留断言**：close 全部对话后进程树回到基线（含 MCP 类扩展子进程）→ T8.0 P1-e / T8.1 C4 已证（本批无新进程面）
  - [~] 用量视图能按对话归属显示 token/费用，月配额超限动作生效 → 配额动作（throttle/block/warn）已接线；按对话显示归 T8.8 标签条
  - [x] `pnpm -r typecheck` + `pnpm -r test` + build + 三探针回归 EXIT=0 → typecheck / test（desktop 184/184 + 三包）/ build **实跑全绿**；**三探针回归 2026-09-05 真机全绿**（plugin ALL PASS、goal 7/7、memory 7/7，另 engine-process 14/14、conversation 21/21、packaged 21/21 同轮复跑不破）。**⚠ 不由本条代证**：闸门**在飞峰值计数**（prompt ≤3 / 子代理 ≤6&≤4）需真模型灌流，`--concurrency-probe` ④⑤ 至今 SKIP → 见本节「性能红线」一条
- 验证方式：`--concurrency-probe` 计数断言（在飞 prompt / child run / 辅助运行时并发 / 残留 pid）；单飞、互斥与配额纯逻辑单测（✅ 19 例）

### [ ] T8.6 git worktree 隔离与变更回流（第二轮拍板 ③ 的核心）（⏳ 2026-09-04：代码+单测+typecheck+build 全过，真机探针/目检待——见逐项标注与 §8）

> **实现摘要（2026-09-04）**：新建 `electron/main/worktree/` 两件（**electron-free**，可真 git 仓库单测）：① `worktree-naming.ts` 纯函数（`conversationShortId`/`sanitizeSegment`（Windows 非法字符/结尾点/统一小写防大小写不敏感 FS 冲突）/`worktreePathFor`（`<project>/.pi-wood/worktrees/<shortId>`，保留项目原分隔符）/`branchFor`（`piwood/<shortId>`）/`isPathLengthOk`/`isManagedWorktree`（孤儿对账判据：只管自己建的树））+ ② `worktree-service.ts`（`detectWorktreeFeasibility`：not-git/detached/submodule/path-too-long 四类降级各带显式 reason；`ensureWorktree`：`worktree add -b piwood/<id> <path> HEAD` 惰性建（T8.0 实测 151~180ms 不卡 UI）+ 已存在复用 + **node_modules 策略=共享软链**（junction/dir 链接主树依赖，不复制，链接失败静默——树内可手工安装）；`removeWorktree`：脏树（含 untracked）拒绝→先回流/丢弃，force 显式丢弃；remove+prune+删分支幂等；`mergeBackWorktree`：`git -C wt diff <baseRef>`（建树时 HEAD，含已提交+未提交；baseRef 缺失退回 merge-base）→ 主树 `apply --3way --ignore-whitespace`（T8.0 发现③的固定缓解）→ **内容复核**（`--diff-filter=U` 判冲突、name-only 判落盘，不只看退出码）→ **冲突不自动合并**：`apply -R --3way` 整体退掉主树的冲突态（绝不 reset --hard 毁用户改动），改动保留在树供手工处理；untracked **显式列出不自动搬**；`parseWorktreeList`+`reconcileOrphans`（只报告不偷删，沿 T8.1 看门狗哲学））。**接线**：registry `spawnHandle` 建 EngineHost 前先 ensure worktree → `EngineHost.cwd`/`startEngine.projectDir`/onEvent ctx.projectDir 全部=树路径（Pi 会话目录、bash cwd、快照/diff 归集全落树上）；`ConversationHandle` 增 `worktreeBaseRef`；`ensureConversation(opts.newConversation)` 支持同项目**再开一条**（`createConversation` IPC 传入——同项目多对话解锁前提）；`closeConversation` 随对话回收树（settings `worktree.keepAfterClose` 留树；脏树保留+notify 不阻塞关闭）；每项目首次建对话时孤儿对账一次（toast）；engine-manager 的 git 域（collectGitInfo/diffRevert/RuntimeInfo.cwd）改走 `activeGitDir()`=active 对话树。**契约**：ENGINE_CHANNELS 增 `worktreeList/worktreeMergeBack/worktreeRemove` 三通道（engine-manager 已接 handler；设置「工作树」页与 DiffPanel 回流按钮的 UI 随 T8.8）。
> **门禁状态（实跑）**：desktop typecheck 双绿；`node --test` **208/208**（新增 24：naming 13 + service 11——service 用真 git 仓库 fixture 覆盖回流成功路径/冲突不自动合并且主树回退干净且树内改动保留/untracked 显式列出/脏树拒绝/孤儿对账/porcelain 解析）；`pnpm probe:worktree`（T8.0 P2）**15/16**——唯一失败「路径大小写不敏感」是 Windows 专属检查（探针用 `/TMP` 大写路径，Linux 沙箱大小写敏感 FS 下 `/TMP` 是不存在的只读路径；T8.0 在真机 20/20）；`electron-vite build` EXIT=0 + 产物断言（worktreeMergeBack/degraded-shared/共享主工作树/回流全进 main 产物、ESM 不变）。**仍待真机**：同项目两对话 `--conversation-probe` 断言（两引擎 bash cwd 不同）、大仓库建树计时复核、设置「工作树」页 UI（随 T8.8）。

- 来源：本节「方案选型 D」+ 现状核实「Pi 会话目录按 cwd 编码」行 + 风险 6
- 前置：T8.1（句柄含 `worktreePath`）；实测判据来自 T8.0 P2
- 步骤：
  1. → [x] `worktree-naming.ts` 13 例单测
  2. → [x] `ensureWorktree`/`removeWorktree`/`reconcileOrphans`（对账挂每项目首次建对话 + `worktree:list` IPC）
  3. → [~] 服务级回流完成（真 git fixture：成功+冲突+untracked 全覆盖）；DiffPanel「审→回流」按钮 UI 随 T8.8（`worktree:mergeBack` IPC 已就位，冲突 toast 已接）
  4. → [x] child `projectDir`/`cwd` = `worktreePath`；**node_modules 策略=共享软链**（T8.0 实测不复制 + junction 链接，失败静默不挡建树）
  5. → [x] not-git/detached/submodule/path-too-long 各有显式 reason 且经 `caps.notify` 呈现（P2 降级类探针全过）；非 git → `degraded-shared` 显式共享主树
  6. → [~] `settings.worktree.{enabled, keepAfterClose}` 已实现；**maxTotalBytes 未实现**（诚实标注）；Settings「工作树」页 UI 随 T8.8（list/mergeBack/remove 三 IPC 已就位）
- 验收：
  - [~] 同项目两对话各改同一文件不同行 → 两边 `git diff` 互不含对方改动、两引擎 bash cwd 不同（探针断言）→ 机制完成（每对话一树 + `newConversation`）；探针断言待真机
  - [x] 回流：`git apply --3way` 成功路径 + 冲突时不自动合并且有明确出口（含单测 fixture）→ **真 git 仓库单测**（冲突时主树回退干净、树内改动保留）
  - [~] close 后 worktree 与分支按设置被清理；孤儿树在重启后被对账恢复 → 服务级+接线完成（真 git 单测）；真机目检待
  - [x] 非 git / 脏树 / submodule 三种降级各有显式提示文案且不静默共享主树 → not-git 显式共享；detached/submodule 显式提示；脏主树 git 本身允许建（P2 实测「允许但提示」口径）
  - [x] `node_modules` 策略由 T8.0 实测选定并在文档/设置里写明（不复制依赖）→ **共享软链**（junction），注释与本节写明；设置页 UI 随 T8.8
  - [~] 大仓库（≥1 万文件）下建树耗时不阻塞 UI（探针计时 + 后台异步）→ T8.0 P2 实测真实仓库 151~180ms（同步建树不卡 UI 的量级）；「后台异步预建」未做（诚实）
  - [x] `pnpm -r typecheck` + `pnpm -r test` + build 全绿 → typecheck / test（desktop **208/208**）/ build 实跑全绿；**三探针回归 2026-09-05 真机全绿**（`probe:worktree` 本机 **18/18**——此前沙箱 15/16 的唯一失败项正是 Windows 大小写口径，真机可复现并已通过；conversation 21/21、engine-process 14/14 同轮复跑不破）。**⚠ 不由本条代证**：「同项目两对话各改同一文件**不同行** → 两边 `git diff` 互不含对方改动」中，**引擎 cwd 互异已真机证**（concurrency-probe ①），但「双向改动 → 人审 diff → 回流」的端到端仍需 GUI 人工目检（见 T8.8 对应验收项）
- 验证方式：`--worktree-probe`（T8.0 P2 复用，真 git 仓库上跑全生命周期）EXIT=0 + 纯函数单测

### [ ] T8.7 工作台与项目级资源作用域（⏳ 2026-09-04：代码+单测+typecheck+build 全过，真机探针/目检待——见逐项标注与 §8）

> **实现摘要（2026-09-04）**：① **作用域唯一入口**——engine-manager 新导出 `getActiveWorkspaceDir()`（= `activeConversation().worktreePath ?? activeProject`，注释锁死「活动项目语义一律按对话树解析」），`initFileIpc`/`initDataIpc` 注入点、`review.ipc` 的 git diff、`plugins.ipc` 的 `getProjectDir`（插件无对话上下文 → 取 active 对话，语义已在注释写明）全部换源；git 域（collectGitInfo/diffRevert/RuntimeInfo.cwd）T8.6 已走 `activeGitDir()`。② **记忆 scope 归一（补 T7.10 偏差 b）**——`worktree-naming.ts` 增纯函数 `mainProjectRootOf()`（`<proj>/.pi-wood/worktrees/<id>` → `<proj>`，统一分隔符+大小写不敏感匹配），`memory.ipc` 的 `getProjectDir` 过它 ⇒ worktree 对话的 project scope 记忆落到主项目同一份 project.json（memory-tools 走同一单例自动生效）。③ **写并发保护**——新模块 `workbench/write-queue.ts`（`FileWriteQueue.withLock(path, fn)`：同路径 FIFO 串行、异路径并行、临界区抛错不阻塞后续；4 例并发单测，含「两并发 save 不丢条目」），接入 `memory.ipc` 全部写 handler（lock key "memory"）与 `settings:set`（lock key "settings.json"）；快照写入本为同步单线程无交错，未包（诚实标注）。④ **渲染层 workbench per-对话**——`workbench-store.ts` 重构为 `slices: Record<convId, Slice>`，顶层 `diffs/openTabs/activeTab/requestedFile` 保持为**当前对话切片的投影**（RightPane/FilesPanel/DiffPanel 读路径零改动）；`addDiff` 等写 action 带可选 `conversationId`（后台对话的 diff 按归属路由），`switchConversation` 由 App 订阅 `activeConversationId` 驱动（切片常驻内存，切回原样恢复）；diff 载荷（main）带 `conversationId`，后台对话差异静默入切片、不抢开 diff 面板。⑤ **终端 per-对话**——TerminalPanel 改 `Map<convId, {term,pty}>` 实例集（全局上限 8、LRU kill），切对话保留各自 shell 与滚动缓冲（非活跃实例 DOM 摘除但 xterm 常驻）；`term:create` 增 `conversationId` 参数，主进程解析 cwd=该对话 worktree。⑥ **浏览器 per-对话**——browser-service 全局单 page 改 `Map<convId, {page?, url}>` 共享一个 browser 进程，**上限 2 活跃 page、LRU 挂起**（close page 只留 URL，再访问自动重开）；归属由 `configureBrowserScope(getActiveConversationIdSafe)` 注入（agent 工具执行时该对话即 active，语义注释写明）。⑦ **会话聚合**——`data.ipc` 的 `sessions:list` 改为聚合主项目 + `<proj>/.pi-wood/worktrees/*` 全部树（按 modified 新→旧、同 id 去重），左栏 sidebar 与 `pi --resume` 互通不回退；「项目→对话两级树」UI 随 T8.8。⑧ **草稿**——T7.11 已按 sessionId 分键，多对话下天然成立，零改动。
> **门禁状态（实跑）**：desktop typecheck 双绿；`node --test` **216/216**（新增 8：write-queue 4 + mainProjectRootOf 4）；`electron-vite build` EXIT=0 + 产物断言（工作区作用域/记忆归一/withLock/MAX_ACTIVE_PAGES 全进 main 产物、switchConversation 进 renderer、ESM 不变）。**仍待真机**：`--workspace-scope-probe` 双项目交叉断言、同项目两 worktree 的文件树/diff/终端目检、终端切回缓冲与浏览器 page 上限目检——本会话无 electron/GUI，如实标注。
...
- 步骤：
  1. → [x]（`getActiveWorkspaceDir` 唯一入口 + file/data/review/plugins 四处换源；git 域 T8.6 已改）
  2. → [x]（`mainProjectRootOf` 4 例单测 + memory.ipc 过滤；global scope 不受影响）
  3. → [x]（write-queue 4 例单测 + memory/settings 写 handler 接入；快照写入为同步单线程无交错未包——诚实标注）
  4. → [x]（切片投影重构，5 个消费者读路径零改动；写入按对话路由；切回恢复=内存级，跨启动持久化仍只存最后活跃对话的标签——如实标注）
  5. → [x]（per-对话 Map + 上限 8 LRU + `term:create` 对话化 cwd；切回保留 shell 与缓冲）
  6. → [x]（per-对话 page 共享 browser + 上限 2 LRU 挂起留 URL）
  7. → [~] 会话聚合已完成（sidebar 零改动获得聚合视图）；「项目→对话两级树 + 运行态徽章」UI 随 T8.8；`activateProject` 不再隐式切换对话在 T8.6 已成立（ensureConversation 复用语义）
  8. → [x] T7.11 草稿按 sessionId 已成立，零改动（核实非假设：`use-composer-controller` 的 readDraft/writeDraft 键=sessionId）
- 验收：
  - [x] 两个不同项目的对话并存：文件树、git 分支/变更数、快照、记忆 project scope 各自正确、互不覆盖（探针断言）→ **2026-09-05 `--workspace-scope-probe` 真机 14/14**：W1 `getActiveWorkspaceDir()` 随 active 走且三棵树互异、W2 fs 写盘只落 active 那棵树（其余树与主项目都不见）、W3a 分支名 === `branchFor(id)`、W3b A1 改文件不污染 A2/主树（`git status` 互不含）、W4 快照按树归集且 A2 树内容原样、W9 非 git 目录显式降级共享主树不悬空
  - [x] 同项目两对话（两个 worktree）：文件树/diff/终端各自指向自己那棵树；`memory_save` 两条都落到**主项目**同一文件、都不丢 → W1b（projectDir 相同、树不同）+ W5a `mainProjectRootOf(树)===主项目` + W5b 两条记忆同文件都在 + **W6 八路并发 save 一条不丢**（10 条齐全，写队列真串行）+ W8 终端 cwd 解析（含「无对话 id / 查不到树 → 保留请求值」的回归）
  - [~] **CLI 互通不回退**：桌面在 worktree 建的会话，在**主项目**目录 `pi --resume` 可选到（左栏聚合的硬证据）→ 聚合语义已探针断言（W7a 覆盖主项目+全部树、W7b 同 id 去重 + modified 新→旧；实现抽成 `listSessionsAcrossTrees(projectDir, lister?)` 供应用与探针共用同一份）；**真机 `pi --resume` 的人眼确认仍缺**（要 CLI 侧手工跑一次）
  - [~] A 对话的终端 shell 与滚动缓冲在切到 B 再切回后仍在；未被 kill → 已实现（xterm 实例常驻 + LRU 8）；真机目检待
  - [~] A/B 各自浏览器页面互不导航覆盖；活跃 page 数不超上限 → 已实现（per-对话 page + 上限 2 挂起）；真机目检待
  - [~] 右栏标签集随对话独立（开/关/激活态各自持久化恢复）→ 切片内存级恢复完成；跨启动持久化仅最后活跃对话（settings.workbench 键未按对话拆——诚实标注）
  - [x] 并发写队列有单测（两对话同时 memory_save / settings patch，无丢更新）
  - [x] `pnpm typecheck` + desktop test + build + 三探针回归全绿 → typecheck/test（desktop **216/216**）/build 实跑全绿；**三探针回归 2026-09-05 真机全绿**（worktree 18/18、plugin/goal/memory ALL PASS/7/7/7/7、engine-process 14/14、conversation 21/21）；**本任务自己的 `--workspace-scope-probe` 也已于 09-05 实现并真机 14/14 EXIT=0**（fs/git/快照/memory/终端/sessions 交叉归属，见上两条）。**只剩 GUI 目检**：终端切回缓冲未被 kill、两对话浏览器 page 互不覆盖、`pi --resume` 人眼确认可选到 worktree 会话
- 验证方式：无窗探针 `--workspace-scope-probe`（✅ 已实现：2 项目 + 同项目两 worktree + 非 git 降级，交叉断言 fs/git/snapshot/memory/term/sessions 归属；`pnpm --filter @pi-wood/desktop probe:workspace-scope`）+ GUI 目检（剩三项）

### [ ] T8.8 多对话 UI（标签条 / 进度徽章 / 未读）+ 并发门禁与回归（🔜 2026-09-05 真机门禁日：**五探针回归 + `--concurrency-probe` + `--ui-chat` + macOS 首次打包/packaged 探针全绿**，T8.4~T8.7 的真机欠账一并还清；**验收未全闭的剩 3 件**——性能红线 5 行需真模型灌流、并发=1 A/B 人工对照、4 对话并行改代码+回流 GUI 目检，全部过程见 §8 2026-09-05 行）

> **实现摘要（2026-09-04）**：① **对话标签条** `ConversationTabs.tsx`（ConversationHeader 同排）：标题（首条用户消息，与会话列表同源，纯函数 `tabTitle` 截断/回落「项目名 · 对话 N」）、状态徽章（纯函数 `badgeFor` 优先级：待审批红点 > 在飞转圈 > 排队 > 启动中虚线 > 关停灰 > 未读蓝点；5 例单测）、「树」角标 + 悬浮显示 worktreePath、关闭按钮、「+」同项目再开一条（T8.6 语义）；数据源=`listConversations` 轮询 1.5s（载荷已含 `capacity`）+ session-store slices 的 unreadCount 合成。② **键盘可达**：`Ctrl/Cmd+Shift+[` / `]` 循环切对话、`Ctrl+Shift+N` 新建、`Ctrl+W` 关闭当前（组件内 keydown 注册）。③ **关闭在跑对话二选一**：内联卡（贴标签条，无全局模态）——「关停引擎但保留可恢复」=suspend（sessionFile 留，切回自动接回）/「中止任务并关闭」=abort+close（worktree 未回流改动保留并 toast 提示）。④ **退出确认**：main `before-quit` 查 `busyConversations()`（registry 新导出）→ `showMessageBoxSync` 列清单，取消则 preventDefault 留在应用。⑤ **EnvironmentPanel 资源行**：`summarizeCapacity` 纯函数（conversation-core）+ registry `engineRssTotal()` + listConversations 载荷增 `capacity{对话 N/M · prompt X/Y · 子代理 Z/W · RSS 合计}`，面板 4s 轮询展示。⑥ **门禁探针** `electron . --concurrency-probe`（`concurrency-probe.ts`，stub caps + 真 child，index.ts 注册模式旗标）：断言 ①三对话并存且 cwd 各不相同（worktree 隔离）②切换不打断 ⑥审批门在位 ③close 清空 ⑦零残留（Windows tasklist 口径）；④⑤⑧ 输出 SKIP 标记（prompt/子代理闸计数与性能红线需真渲染层/真模型流量——SlotGate 与预约-快照对账已由单测穷举，红线定档随 --ui-chat 真机）。
> **门禁状态（实跑）**：desktop typecheck 双绿；`node --test` **221/221**（新增 5：badge 3 + tabTitle 2）；`electron-vite build` EXIT=0 + 产物断言（标签条/关闭确认/退出确认/concurrency-probe 全进产物、ESM 不变）。**待真机（T8.8 收口欠账，集中列出）**：`--concurrency-probe` 真机跑 EXIT=0；五探针回归（plugin/goal/memory/engine-process 全需 electron；worktree-probe 沙箱 15/16）；性能红线 6 行定档（RPC p95/三路真流体感/CPU/切换首屏/休眠恢复/在飞上限）；真机带密钥「2 项目 + 同项目双 worktree 共 4 对话并行改代码+回流」目检留证；打包版冒烟（packaged child 载 SDK）。**本批明确不做（按计划边界）**：多窗口、回流自动化、引擎预热池、worktree 依赖共享、跨端同步。**轻量未做（诚实标注）**：「+」新建的选项目/fork 高级形态、命令面板聚合项、标签跨启动 per-对话持久化、用量页按对话细分。

- 来源：本节「第一轮拍板 ①」（用户指定单窗口多对话标签）
- 前置：T8.1~T8.7
- 步骤：
  1. → [x] 标签条（标题/徽章/树角标/关闭/+）
  2. → [~] 键盘四组快捷键已接；命令面板聚合项未做
  3. → [x] 关闭二选一内联卡 + before-quit 清单确认框
  4. → [~] 树角标+悬浮路径+资源行已接；**用量页按对话细分未做**（usage-tracker 落盘粒度是 provider|model，改造留后续）
  5. → [x] 探针骨架+可行断言（①②③⑥⑦）已写，**2026-09-05 真机跑通 4/4 EXIT=0**（⑦ 零残留为 Windows tasklist 口径 → macOS SKIP）；④⑤⑧ 归下方「性能红线」一条，需真模型灌流
  6. → [x] **2026-09-05 真机五探针回归全过**：worktree **18/18**（含沙箱不可复现的 Windows 大小写项）、plugin ALL PASS、goal 7/7、memory 7/7、engine-process 14/14、conversation 21/21
- 验收：
  - [x] `--concurrency-probe` 可行断言全 PASS、EXIT=0（**2026-09-05 真机 4/4**）→ ① 三对话并存且 cwd 各不相同 = **同项目 ×2 + 跨项目 ×1**（`[b35ef76b/bbe22d72/8ffe484d]`，T8.6「同项目两对话 cwd 不同」的真机硬证）、② 切换不打断 `dead=0`、③ close 后注册表清空、⑥ 审批门在位；⑦ 零残留计数是 Windows tasklist 专属口径 → macOS 显式 SKIP。**⚠ 本条原写「8 条全 PASS」是计划期口径写错**：探针按设计只含 5 条可行断言，④⑤⑧（prompt/子代理在飞计数、性能红线逐条）必须真模型灌流，属下面「性能红线」一条、不由本探针代证
  - [x] 五探针回归 + `pnpm -r typecheck`/`test` + build → **2026-09-05 真机全绿**：typecheck EXIT=0；`pnpm -r test` desktop **221/221**（补登 `conversation-badge.test.ts` 后才名副其实——09-04 记录的 221 实为手工数，门禁跑的是 216）、ipc-schema 41/engine 12/plugin-api 10；build EXIT=0 + 产物断言；worktree **18/18**、plugin ALL PASS、goal **7/7**、memory **7/7**、engine-process **14/14**、conversation **21/21**；`--ui-chat` 真模型闭环 EXIT=0
  - [~] 性能红线表逐条实测达标 → 本轮**新定档 4 行**：后台事件流量 **5 帧/秒**（红线 ≤6，dev + packaged 双形态同数）、每对话 RSS/净增（child 自报 184~185MB、**每多一对话净增 ≈1MB**）、休眠→恢复（腾位置+新建 794ms、唤醒读回同一 sessionId、全量关停 161~165ms）、冷启动（fork→ready 60~68ms + 装配 378~387ms）。**仍无对象 5 行**：RPC 两跳 p95（探针未打 p50/p95 统计）、3 路并发流式前台体感、主进程 CPU ≤60%、切换首屏 ≤100ms、在飞 prompt/子代理峰值上限——**不拿单路或 SKIP 冒充已测**
  - [ ] 并发数=1 时行为与改造前逐位一致 → 待人工 A/B 对照目检（需 GUI）
  - [ ] 真机 4 对话并行改代码+回流端到端目检留证 → 待人工（密钥已备，缺人手在 GUI 里跑 4 对话并审 diff→回流）
  - [x] 打包版冒烟 → **2026-09-05 macOS arm64 首次打包成功**（`electron-builder --dir --config.electronDist=<本地 dist>` 绕开重下 Electron 发行包的网络失败）+ **packaged `--conversation-probe=packaged` 21/21 EXIT=0**（child 从 asar 载 SDK/社区扩展、崩溃隔离与自动恢复、合帧节流同数）。**⚠ 口径限定**：证的是「打包产物内引擎子进程链路可用」，**安装/签名/公证/三平台仍属 T5.3 环境阻塞**（无干净 VM/证书）
- 验证方式：探针退出码 + 门禁数据 + 真机目检截图（`docs/proofs/T8.8/`）

### [x] T8.9 红线度量出口（`--latency-probe`）：把「有预算、无口径」的三行红线变成可自动判定（✅ 2026-09-05：探针 7/7 EXIT=0，实测数见下；renderer 那一跳与切换首屏/CPU 仍显式 SKIP）

> **为什么立它**：§7.9 性能红线表里「RPC 往返延迟」「事件到达」「审批往返」三行长期只有预算数字、没有统计出口——探针不输出 p50/p95，就永远只能写「待真机」，红线沦为愿望。本任务**只做度量，不改任何性能路径**（不做批流、不动合帧阈值），先让达标/不达标可判。
> **来源**：09-05 真机门禁日结论「性能红线剩 5 行无对象定档」→ 用户拍板「先补度量出口，再排人工目检」。
> **前置**：T8.1（EngineHost/帧协议）、T8.8（探针 harness 与 stub caps 范式）。

- 步骤：
  1. → [x] **纯模块 `packages/ipc-schema/src/latency-stats.ts`**（electron-free、可穷举单测）：`LatencyRecorder`（固定容量环形窗口 + `dropped`/`invalid` 计数）、`percentile`（最近秩）、`summarize`、`mergeRecorders`（跨对话合并**样本**而非合并快照平均——平均的平均会掩盖单路抖动）、`LATENCY_BUDGETS`（红线预算常量，改它=改红线）、`budgetVerdict`（**count=0 一律不通过**并写明未触发）、`RPC_LATENCY_SAMPLED_METHODS` 取样域。+23 例单测
  2. → [x] **协议加法（向后兼容，不升 `ENGINE_RPC_VERSION`）**：下行方法 `ping`（空操作，量纯通道）与 `debugEcho`（探针门控，经真通道合成事件帧/审批往返）；`EventUp.sentAt?`（跨进程 epoch 戳）；上行新帧 `metric {name, ms, n?}`（child 单时钟自计，fire-and-forget）；`HostApprovalResult.auto?`（未走用户卡的标记，供审批样本过滤）
  3. → [x] **child 侧**：事件帧发 `sentAt`（`nowEpochMs()`）；`rpc()` 支持 `{name, when}` 度量参数——审批只在 `auto===true` 时上报（**人的思考时间绝不进通道红线**）；`ping`/`debugEcho` 处理器，后者非 `PIWOOD_ENGINE_PROBE=1` 一律抛（不给生产留后门）
  4. → [x] **主进程侧**：`EngineHost` 三个窗口（`rpcRtt` 256 / `eventHop` 512 / 按名归档的 metrics 256），只在 respond 到达时记**只读取样域**命令、在 event 帧记单跳、在 metric 帧按名记；`latencyReport()` / `latencyRecorders()`；注册表 `engineLatencySummary()` 跨活跃对话聚合（**统计不跨会话污染**由 L5 断言）
  5. → [x] **探针 `electron . --latency-probe`**（无窗、真 child、真 IPC）：L1a ping 往返 p95、L1b 每对话统计独立、L2 事件单跳 p95、L2b 跨进程时钟脏样本占比 ≤10%、L3 审批往返 p95、L4 聚合视图、L5 close 后窗口出清；**切换首屏 / 3 路真流体感 / 主进程 CPU 显式打 SKIP**
  6. → [x] 接进门禁：`t88-verify.sh` 第 4 步 `probe-latency` + 产物符号断言加 `debugEcho`；`package.json` 加 `probe:latency`
- 验收：
  - [x] 三行红线有可自动判定的口径与退出码 → **09-05 真机 7/7 EXIT=0**：`rpcRtt` p50=0.048 / **p95=0.093ms** / max=0.275（n=300，预算 ≤40ms）；`eventHop`（child→main 单跳，800 帧突发灌 512 窗口）p50=1.842 / **p95=2.292ms** / max=2.462（预算 ≤20ms）；`approvalRtt`（自动裁决路径）p50=0.053 / **p95=0.087ms**（n=60，预算 ≤40ms）；脏样本 invalid=0/512
  - [x] 度量不污染判定路径：统计只在帧处理里做加/减法，不参与任何 allow/deny、休眠或节流决策（代码审阅 + 五探针回归 09-05 复跑不破）
  - [x] 取样域排除模型耗时（`prompt/steer/followUp/start/compact` 不入 `rpcRtt`）→ 由 `RPC_LATENCY_SAMPLED_METHODS` + 单测钉住（含 `isRpcLatencySampled("prompt")===false`）
  - [x] 无样本不判达标 → `budgetVerdict` 对 count=0 返回不通过并写明「本轮未触发该路径」；探针里 `hostToolRtt n=0` 如实展示为 0 而非省略
  - [x] 统计随句柄出清（不跨会话污染）→ L5：close 全部后 `hosts=0`、`rpcRtt n=0`、`eventHop n=0`
  - [x] **renderer 那一跳与「切换首屏」「3 路并发体感/CPU」** → 同日晚些由 **T8.10 带窗探针**闭合（第二跳 p95 1.872ms、首屏 p95 8ms、帧间隔 p95 9.5ms、主进程 CPU p95 3.683%，5/5 EXIT=0；EnvironmentPanel 红线行同步上线）。**仍留一条**：合成事件不含模型 token 生成与真实高亮成本 ⇒ **真模型负载下的体感复核**归人工那一轮
  - [x] 门禁全绿：`pnpm -r typecheck` EXIT=0；`pnpm -r test` desktop **221/221** + ipc-schema **64/64**（+23）+ engine 12 + plugin-api 10 = **307**；`electron-vite build` EXIT=0 + 产物断言（`debugEcho` 进 main 与 engine-child bundle）
- 踩坑（已写进代码注释）：**`ipc-schema` 被渲染层共用**，在 `engine-rpc.ts` 里 `import { performance } from "node:perf_hooks"` 会让 renderer 构建立刻失败（`"performance" is not exported by "__vite-browser-external"`）→ `nowEpochMs()` 一律读 `globalThis.performance`，缺失时退回 `Date.now()`
- 验证方式：`pnpm exec electron . --latency-probe`（或 `pnpm --filter @pi-wood/desktop probe:latency`）退出码 + 打印的分位数；证据已落 `docs/proofs/T8.8/probe-latency.log`（本轮手工跑，下轮起由 `t88-verify.sh` 第 4 步自动覆盖）

### [x] T8.10 带窗红线度量出口（`--ui-latency-probe` + EnvironmentPanel 红线行）：补齐 T8.9 测不到的三行（✅ 2026-09-05：探针 5/5 EXIT=0；负载是合成事件，不含模型生成成本）

> **为什么立它**：T8.9 收口的同时留下三行——**事件两跳的第二跳（main→renderer）**、**切换已激活对话首屏**、**流式期间掉帧与主进程 CPU**。它们必须有真渲染层，无窗探针造不出来（造出来也不是那个东西）。
> **来源**：T8.9 验收最后一条「renderer 那一跳与首屏/CPU 仍未有数」→ 同轮续做。
> **前置**：T8.9（recorder/预算/判据三件套）、T8.3（可见性节流与切片）、T8.8（标签条 = 可点击的真实切换入口）。

- 步骤：
  1. → [x] envelope 加 `tPush?`（主进程推送时刻，`sendFrame` 每帧一次 `nowEpochMs()`）；**preload 的 lite 解包同步透传**（那份复制是刻意的，漏透传 = 第二跳永远没样本，已在注释钉住同步义务）
  2. → [x] 渲染层 `src/renderer/src/lib/latency-outlet.ts`：第二跳/首屏/帧间隔三项自测，**只在「最近 600ms 内来过事件」且窗口可见时开 rAF 帧表**（空闲即停——拿常驻 60fps 循环去测卡不卡，自己先把 CPU 吃出红线）；样本本地攒批 **2s 一次**上报（每帧一次 IPC 会让度量本身变成性能问题）
  3. → [x] 主进程 `renderer-latency.ts`：四个环形窗口汇聚渲染层样本 + **主进程 CPU 只在有样本的活动窗口里算**（`process.cpuUsage` 差值 ÷ 墙上时间，空闲期计入会把 ≤60% 稀释成好看）；载荷非法只计数不抛（度量不许影响功能）
  4. → [x] 带窗探针 `--ui-latency-probe`：三路对话（同项目 ×2 + 跨项目 ×1）按 **16ms 间隔**灌合成事件（≈60 token/秒/对话，合计 ~180 帧/秒），**负载进行中点真标签切换 6 次**，断言 U1 第二跳 / U2 帧间隔 / U3 主进程 CPU / U4 切换首屏 / U5 上报链路健康（`reportedSamples>0 && rejectedBatches===0`）
  5. → [x] `EnvironmentPanel` 新增红线行：六项 p95 + CPU 一行展示，**无样本显示「待采样」而不是 0ms**，超预算标红；预算数字从 `LATENCY_BUDGETS` 取（渲染层不自己抄数字）
  6. → [x] 接进门禁：`t88-verify.sh` 新增 4b) `probe-ui-latency`（`SKIP_GUI=1` 可跳，它要可见窗口会抢焦点）；头部用法说明同步
- 实测（09-05 macOS 真机，5/5 EXIT=0）：`rendererHop` p50=0.149 / **p95=1.872ms**（n=94，max 3.767）；`frameGap` p50=8.3 / **p95=9.5ms**（n=987，max 16.8 ⇒ 60fps 无掉帧）；**主进程 CPU p95=3.683%**（单核，预算 60%）；`firstPaint` **p95=8ms**（n=6，预算 100ms）；`eventHop` 节流后 p95=0.422ms。截图 `docs/proofs/T8.8/ui-latency-probe.png`（三标签 + 各带「树」角标 + 合成流已上屏 = 渲染路径真的走通）
- **两条关键教训（比数字更值钱）**：
  1. **首跑用 600 帧无间隔突发 → `rendererHop` p95 = 261ms「爆表」**。那不是产品慢，是**负载模型错了**：突发测的是「管子塞多快」（排队延迟），真实流式是每秒几十帧。改 16ms 节流后同一台机器同一份代码 p95=1.87ms。**红线必须先写清负载形状，否则数字毫无意义**——已把这点写进 `DebugEchoParamsSchema.gapMs` 的注释与探针头部注释。
  2. **主进程自己调 `setActiveConversation` 测不出首屏**：首屏起点在渲染层 store 的 `setActiveConversation` 里（`markSwitchStart`），主进程改 active 既不产生样本、也不会让渲染层的可见性判定跟上 → 探针改成 `executeJavaScript` **点真标签按钮**（`[role="tab"] > button`），走的就是用户那条路径。
...
> - **项目移除 ≠ 会话删除**：ChatGPT「删除项目连带删会话」是公认反面教材；移除项目只解除关联；
> - **有状态资产的删除有前置条件**：VS Code 明确「先整合 worktree 变更再删会话，否则拒绝执行」——与 pi-wood 每对话一棵 worktree 的架构完全同构，是现成先例；
> - **永远不静默清列表**：Windsurf 只显 ~20 条且旧的自动消失、无归档无导出，大量用户投诉并催生第三方备份工具；
> - 能力最全的参照是 VS Code Copilot Agents 面板：Archive / Mark as Done / Pin / 分组（workspace/时间/自定义）/ 筛选（活跃/完成/已归档）/ 状态徽标 / 右键菜单末位 Delete。
>
> **pi-wood 的映射口径（本任务的实现基线）**：
> 1. **元数据以会话文件路径为键**，存 pi-wood 侧 `~/.pi-wood/session-meta.json`（`archived/pinned/alias` 三字段），**不动 Pi 会话文件本身**——CLI resume / T1.4 互通零影响；
> 2. **三层语义分开**：「关闭对话」= 结束引擎进程（既有）；「归档」= 从列表移除（可恢复）；「删除」= 销毁会话文件（CLI 亦不可 resume）；
> 3. **项目「移除」只解除注册**（复用 `project:remove` 既有语义，只从 projects.json 摘除），确认框明说不删磁盘；
> 4. **删除守卫**：会话正被活跃对话认领 → 拒绝并提示先关闭；UI 删除必须二次确认（内联条，与 pendingClose 同款交互）。

- 步骤：
  1. → [x] 主进程 `session-meta-service.ts`：`~/.pi-wood/session-meta.json` 注册表类（read/write/setMeta/clearMeta，纯逻辑可单测）（✅ `applySessionMetaPatch` 清键不留垃圾；单测 6/6）
  2. → [x] ipc-schema：`SESSION_META_CHANNELS`（get/set/delete）+ `SessionMetaSchema` + `project:rename`（✅）
  3. → [x] `project-manager.rename(id, name)`（显示别名，只改注册表）；`data.ipc` 注册四个新 handler（删除含「被活跃对话认领」守卫 + `.jsonl` 形态守卫）（✅；**顺手修掉 remove 既有 bug**：IPC 传注册表 id 时被再哈希一次永远匹配不上 → 改为「规范化 id / 原始 id / path 三者任一」，由新单测暴露）
  4. → [x] 渲染层 `session-meta-store`（zustand，乐观写回失败回滚）+ preload 面补齐（projectRename/projectRemove/sessionsMeta/sessionsSetMeta/sessionsDelete）（✅）
...
- **第二轮（2026-09-06，用户真机反馈四问题，全部修复并 GUI 实测）**：
  ...
  3. → [x] **归档移出目录树（问题 3，用户裁定 R2）**：左栏不再展示「已归档」组；设置新增「归档」页（ArchiveSettingsPanel）统一管理——跨项目扫描 archived 会话，恢复=清标记、删除=二次确认（sessionsDelete 复用）
  4. → [x] **「设置 → 工作树」页补齐（问题 4）**：T8.0 起 toast 一直指向不存在的页面。新增 WorktreeSettingsPanel：worktree.enabled/keepAfterClose 两开关（settings 新增 worktree 段）+ 当前项目孤儿树列表（engine:worktreeList）+ 回收（`removeManagedWorktreeByPath`：realpath 守卫托管目录，脏树拒绝后升级「强制回收」；worktreeRemove handler 增 path 口径）
  5. → [x] **GUI 实测（dev 实例，computer-use 驱动）**：切既有对话后 Composer 五控件全亮；「+」正确进草稿态并聚焦；设置「工作树」页列出 openchamber 7 棵孤儿树且**实际回收一棵成功**、列表自动刷新；「归档」页正确列出 1 条归档会话。typecheck 全绿、240/240
...
11. **T8.9 红线度量出口（`--latency-probe`）**（0.5~1 天，✅ 2026-09-05）——排在最后是因为「先让功能跑通再补度量」，但它决定红线表能否收口：**没有度量出口的预算不算验收标准**
12. **T8.10 带窗红线度量出口（`--ui-latency-probe` + 面板红线行）**（0.5 天，✅ 2026-09-05）——与 T8.9 合起来把红线表从「多数行无对象」压到只剩两条需真模型：在飞峰值计数、真模型负载下的体感复核
...
| 任务 | 真机已闭 | 仍欠（闭不掉的原因） |
|---|---|---|
| T8.P / T8.0 / T8.1 / T8.2 / T8.3 | ✅ 复跑不破（engine-process 14/14、conversation 21/21、packaged 21/21、`--ui-chat` EXIT=0） | 同项目两对话的**真模型并行流式**手感（并发=1 A/B 对照 + 3 路体感/CPU/切换首屏）——要 GUI 人手 + 真流量 |
| T8.4 审批归属 | ✅ **`--approval-probe` 真机 9/9**：D1 反向通道清单封闭无自批路径、D2 端到端 deny-by-default（真 child 收到 3×`allow:false`，2729ms 收敛不挂死）、D3 票据重放被拒、D4b 跨对话应答被拒、D5a/b/c 后台 pending 不被静默拒且切过去才起表落 deny | ① PromptTray 来源行的视觉目检（GUI）；② 生产 120s 档（探针经 `PIWOOD_APPROVAL_TIMEOUT_MS` 压到 900ms，真值行为等价但没人盯过 120s 的实际体验） |
| T8.5 并发闸门 | 探针回归 + 41 例穷举单测 | ④⑤ **在飞 prompt/子代理峰值计数**需真模型灌流（`--concurrency-probe` 设计上 SKIP） |
| T8.6 worktree | ✅ **同项目两对话 cwd 互异真机证到**（concurrency-probe ①）、worktree-probe 本机 18/18 | **双向改同文件 → 人审 diff → 回流**端到端目检留证（GUI 人工） |
| T8.7 作用域 | ✅ **`--workspace-scope-probe` 已实现并真机 14/14**（fs/git/快照/memory/终端/sessions 交叉归属；含八路并发 save 不丢、非 git 显式降级、close 后作用域不悬空） | 只剩三项 GUI 目检：终端切回 shell 缓冲未被 kill、两对话浏览器 page 互不覆盖、`pi --resume` 人眼确认可选到 worktree 会话 |
...
- **验收（✅ 2026-09-06）**：`context-outline` 纯函数单测（user 分节/工具归组/序号/截断/空输入）；`--capture` 空态不回归；真机点节点跳转+scroll spy 高亮目检。
- **v2（✅ 2026-09-07 落地，见下节 T9.2）**：接 pi `SessionManager` 树结构（leaf 切换/`forkFrom` RPC，对应 §7.9 fork 语义与 T7.10 备选）→ 真分叉图（纵深横叉布局）+ transcript 按路径过滤 + 双击切分支。

### T9.2 v2：真分支树（navigateTree 切 leaf + 路径过滤 transcript + 双击切分支）（✅ 2026-09-07）

- **SDK 语义核实（0.84.4 源码级）**：会话树 API 齐全——`getTree/getBranch/getLeafId` + 高层 `AgentSession.navigateTree(targetId, {summarize?})`：**只挪 leaf、不截断、零写盘**；流式进行中直接抛；目标是 user 条目时 leaf=其**父**并把原文作为 `editorText` 回填（检查点式「改这问重发」）；完成后自动刷新 agent 状态并广播 `session_tree{newLeafId, oldLeafId}`。⚠ 只调 `sessionManager.branch()` 不刷 agent 状态——v2 一律走 `navigateTree`。leaf 指针**不持久化**（重开会话回到时间戳最新末梢）。`entry_appended` 只覆盖扩展自定义条目，普通消息不带 entry id ⇒ 树数据不靠事件增量，走「轮末重读 jsonl」。
- **引擎/主进程**：`session-tree.ts` 新增纯函数 `pathToLeafIds`（leaf→root 回溯、防环、leaf 不存在返回 null 降级不过滤）；`EngineAdapter` 链补 `navigateTree`（ipc-schema RPC 表+参数 schema、pi-types、sdk/remote adapter、engine-child 分发表）；`ENGINE_CHANNELS.navigateTree` handler 按 conversationId 取引擎、休眠/dead 显式拒绝（**不惰性拉起**——激活该走对话激活链路）；`EngineEventSchema` 补 `session_tree`；`sessions:tree` 行投影加 `role/textHead`（分支题面）；`sessions:messages` 加 `leafId` 路径过滤；`engine:switchSession` 把新会话文件同步回注册表（续写后树/历史取数一致性）。
- **渲染层**：`lib/context-tree.ts` 纯函数（`deriveContextTree`：路径回溯+旁支根折叠+挂载序号；`expandBranch`/`navigateLandsOn`/`defaultLeafId`/`branchTitle`）+10 例单测；`context-tree-store`（每对话 file/rows/leafId/手选标记；**叶保持规则**：手选叶只有当默认尾相对手选时确实前移（用户在这条分支上又聊了）才自动跟随，纯浏览不被刷新打掉）；`ensureHistoryLoaded` 叶感知 + 新 `rebaseToBranch`（`conversation-slice.rebaseFromHistory` 整体换底、不与旧增量对账）；`ContextTree.tsx` v2：旁支虚线行挂分叉点下（折叠一行、可展开列分支内每一问）、**双击切分支**（换底+原文回填输入框+toast）、头部「⑂N」计数、底部「正在查看历史分支 · 回到最新」条；v1 大纲跳转/scroll-spy 原样保留。
- **顺带修掉两处既有 bug（带窗探针首捕，都是「静默没跑」型）**：① `ensureHistoryLoaded` 把 `engine:listConversations` 的 `{conversations, capacity}` 载荷当数组、按不存在的 `r.conversationId` 查 → 每切后台对话都 TypeError 被 void 吞，「切到后台对话整读对账」自 T8 ⑤ 起实际从未生效；② `noteEventOwnership` 采纳 active 戳直接置 `activeConversationId`、不走装载分支，随后 `switchTo(同 id)` 早退 → 该对话永远空壳停在 Onboarding。修复=按真实形状解包 + 同 id 早退分支补兜底装载。
- **验收**：`--context-tree-probe`（无窗、无模型）**11/11**——A 组树投影/按叶过滤/坏叶降级/RPC 帧契约，B 组真 child 活链路 `switchSession→navigateTree`（editorText 回填/assistant 目标不回填/零写盘）；`--context-tree-ui-probe`（带窗、对人元素派发真 dblclick）**8/8**——旁支可见、默认叶 transcript、双击换底（公共前缀在、两支线文本消失、原问题回填、「回到最新」出现、旁支反向可见）、回到最新复原；截图 `docs/proofs/ui-v3/context-tree-ui.png`。两探针已进 `t88-verify.sh`（UI 版受 SKIP_GUI 控制）。单测：engine +4、desktop +10（已登记 test 清单）。
- **偏差（记录在案）**：布局用「纵列时间轴 + 虚线旁支行」而非 awb 的 d3 二维图（w-56 窄列下二维挤压严重，纵深=序号、横叉=缩进+虚线，够读）；`summarize`（弃枝摘要）恒 false 不做（模型成本，留后续）；leaf 不落盘是 SDK 语义，桌面「上次停留分支」不跨重启保留；双击切分支只在非流式可用（SDK 硬约束，UI 置灰+tooltip 说明）。

### T9.2 v2.1：形态改判——minimap 刻度条 + 消息级分叉（✅ 2026-09-07，同日用户反馈）

> 用户看过 v2 侧栏后判定**形态错误**：「缩略树做太重了，不该单独弄侧栏」——参考 dsh-desktop 的刻度条形态重做；fork 也不该长在树上，应该是**每条 agent 回复底部的「分叉」按钮**（附截图为证）。以下按改判落地：

- **拆掉（UI 层）**：`ContextTree.tsx` 侧栏整件（旁支虚线行/双击切分支/回到最新条/⑂N 计数）、ConversationHeader 的 list-tree 开关按钮、`settings.ui.contextTreeEnabled` 与 `contextTreeSummarizeOnSwitch` 两个设置项（旧 settings.json 残留键无害）。**保留（引擎层备用，无 UI）**：`navigateTree` 全链 RPC、`session_tree` 事件契约、`sessions:messages` 的 leafId 过滤、`context-tree-store`（叶感知历史装载 + fork 序号对齐仍依赖它）、`lib/context-tree.ts` 纯函数与单测、`rebaseFromHistory`。
- **新 `MessageMinimap`**（消息列左缘浮层，不占布局宽度）：每条 user/assistant 消息一个刻度（工具/思考/系统不成刻度），**常态全部等长 14px**；hover 以目标为中心做**正态分布重排**（目标 30px、相邻按高斯衰减渐长，150ms ease，只变宽度、列高稳定不跳），右侧浮「角色+首行摘要」tooltip；点击复用 `piwood:outline-jump`（scrollToIndex+行 flash）；阅读区高亮复用 `piwood:outline-active` 的 user 锚点（同轮刻度点亮）；宿主 <720px 自动隐藏（visibility 占位不重排）；随 items 自动重建；轮末顺带强刷会话树（供 fork 对齐）。消息 <2 条不渲染。
- **消息级分叉（同日二次改判）**：`AssistantRow` 动作排（复制/重试旁）加「分叉」= **新对话含被点回复本身**——精确语义：fork 到「被点回复所在轮的结束」（下一问之前）。用户实测旧语义（停在触发提问、不含该回复，且渲染层直传 entryId 在多气泡轮易错位）反馈后改判：渲染层只传 `userOrdinal`（该回复前第几条 user 行，user 行↔user 条目天然 1:1）+ 视图 `leafId`，**轮末条目由主进程 `pathToLeafIds` 同源定位** → `engine:forkToNewConversation`（`createBranchedSession(轮末条目)` 生成只含 root→轮末路径的新 jsonl，header 自带 `parentSession`）→ 新对话（独立 worktree）`switchSession` 接上 → 写 session-meta `alias="Fork of <源首问>"` + `forkedFrom=<主进程回传的源文件>` → refresh+switchTo。源对话零扰动。
- **「⑂ 从对话中派生」chip**：新对话消息列底部居中显示（数据源=session-meta.forkedFrom），点击 `switchTo` 回跳源对话；源未打开时 toast 指引左栏恢复。左栏对话行标题走既有 `alias ?? 首条消息` 渲染路径，显示「Fork of …」。
- **踩坑两则**：① **ResizeObserver 挂早了**——组件在 `ticks<2` 时 return null，`useEffect([])` 首跑没有 DOM，observer 永远没挂上（窄窗隐藏失效）；改 **callback ref** 在元素真 attach 时建观察。② 探针断言「Fork of」文本在 body 里查不到——临时项目**默认折叠**、对话行不渲染；别名断言改走数据契约（`sessions:meta` 落盘值），渲染路径 T8.11 已证。
- **验收**：`--context-tree-ui-probe` 重写为 **10/10**（刻度数=路径消息数、hover 高斯重排 30>27>21px+tooltip 角色摘要、点刻度 flash、分叉后两对话+视图停在触发提问+chip+别名/forkedFrom 落盘、点 chip 回跳、窄窗隐藏/恢复）；`--context-tree-probe` **13/13** 原样过（引擎层语义未动）；全量门禁 **21/21 EXIT=0**；截图 `docs/proofs/ui-v3/context-tree-ui.png`（刻度条+tooltip 同帧）。
- **形态教训（记档）**：导航类附加物**先问「值不值得占布局宽度」**——常驻侧栏挤压正文，引出宽度自适应/开关/禁用三件套补丁，本质是形态选错的连锁补偿；浮层刻度把这条链整个消掉。

### T9.2 v2.2：刻度条按 OpenWood 参考重做 + 用户三轮改判定稿（✅ 2026-09-08）

> 第一轮：用户判 v2.1「这个对话缩略图不对，完全参考 OpenWood 的实现和样式」。参考源＝`OpenWood/.cache/opencode-v1.18.21/packages/session-ui` 的 `message-nav.tsx`/`message-nav.css` + `diff-changes.tsx` 的 `variant="bars"`（**OpenWood 自身 src 里没有这个组件，别去 `apps/desktop/src` 找**）。按源码逐值搬（24/16/12/200/8px/120ms）。
> 第二轮：用户看过实现图后判「更丑了」，明确三处覆盖参考——**竖向居中**、**不 hover 时每根一样长（hover 才动态变长）**、**hover 只展示当前这一条（不要其他上下文）**。
> 第三轮（定稿）：用户再指两处——**不要默认高亮**（当前阅读轮也不许比别的粗/长，撤销参考与第二轮的 active 加粗）；**hover 要有正态分布动画**（不是只长目标那一根，邻居要按高斯衰减跟着变长，即 v2.1 那味）。

- **刻度语义收窄**：只有 **user 轮次**成刻度（参考 `messages: UserMessage[]`）——assistant/thinking/tool/system 不再各占一格；`ticks<2` 不渲染（参考 `messages.length > 1`）。
- **刻度条（定稿）**：行高 12px 等距、**常态每根完全一样**（10×1px 发丝线、同色，`data-active` 与加粗规则整个删除）；hover 才动——以目标为中心做**正态衰减展开**（`tickWidthAt`：目标 20px、邻居按 σ=1.6 高斯渐次变长、远端回 10px，GSAP 0.25s），**长度与颜色同一条衰减曲线**（hover 变亮 = 前景色叠加层按衰减比调 opacity，等效原 `color-mix(foreground r%, muted-foreground)` 且可被 GSAP 插值）；容器预留 20px 满宽，展开不推动布局。阅读锚点降级为**只做长对话内的刻度条跟随滚动**（手动改 `scrollTop`，不用 `scrollIntoView` 以免带着消息列表一起滚），不再产生任何视觉高亮。hover 动画由 GSAP 驱动（`scaleX`+`autoAlpha` 合成层、`overwrite:"auto"` 抢占、reduced-motion 归零），React state 只管浮层内容——旧「setState 驱动 inline width + CSS transition」在快速平移时逐帧 reflow，是"卡"的根源。
- **摘要浮层（定稿）**：hover 某根刻度 → 右侧 8px 处只浮出**这一条**的摘要（diff bars + 首行标题，单行省略、`max-w min(24rem,38vw)`），按实测行中心与该刻度竖向对齐；离开 120ms 后收（沿刻度条平移不闪断）。参考的 200px 全轮次列表（normal 变体 + HoverCard）**不采纳**。
- **diff bars 数据源**：该轮内所有 tool 行 `diffStat` 汇总的增/删行数 → 18×14 内 5 根 2px 竖条（绿=增 `--success`、红=删 `--destructive`、灰=中性），分档规则逐条搬运参考的 `DiffChanges.blockCounts`（极小改动各 1 块、<20 或比例 <4 只上 4 块、≤5/≤10 行封顶 1~2 块、超额按主导方回退）。
- **纯逻辑下沉**：新增 `lib/message-nav.ts`（`buildNavTicks`/`firstLine`/`tickWidthAt` 正态衰减/`diffBarCounts`）与 `lib/outline-bus.ts`（阅读锚点总线），各带单测并登记进 `package.json` 的 test 清单（门禁 ①「不登记=静默漏测」）。
- **修一处静默 bug（新探针 U3.1 首捕）**：scroll-spy 锚点原先走 `window` CustomEvent，而 MessageList 与 MessageMinimap 是同层兄弟——**effect 按挂载顺序跑，首帧锚点在刻度条挂上监听之前就发完**；短对话（内容不足一屏）不再触发 scroll 事件 ⇒ active 态长期空着。改 `outline-bus` 缓存最后一个值 + 订阅即回放（顺序无关），MessageList 另加「行表变化/切对话也重发锚点」的 effect。
- **位置（定稿）**：**竖向居中**浮在消息列左缘（撤销第一轮的「贴顶」）；轮次过多时刻度条内部滚动且隐藏滚动条，摘要浮层是滚动容器的兄弟节点、不被裁剪。窄窗 <720px 自动隐藏、轮末刷树（供 fork 对齐）等 v2.1 行为原样保留。**留白带（同日用户补充）**：中栏所有内容块（Header/消息列表/Composer/PromptTray/Assist）横向 padding 统一 `px-8`（最小 32px），刻度条距左缘 8px 且满宽 20px（8+20=28 ≤ 32）恰好落在留白带内——任何窗口宽度下都不与正文重叠；<720px 时整条隐藏。
- **验收**：`--context-tree-ui-probe` 按定稿重写 → **14/14 ALL PASS EXIT=0**（U1.1b 常态两根都 16×1px 完全一样、U2 实测目标 30px / 邻居 27.5px / 常态 16px 的正态衰减且粗细不变 + 浮层贴右与该行中心对齐 ±3px、U2.1 浮层含被 hover 的标题且**不含**另一轮标题 + 5 根 bars、U3.1 指针离开后整列回到 16×1px 且浮层收起、U4.x 分叉链原样、U5 窄窗）；`--context-tree-probe` **13/13** 未受影响；desktop 单测 **273** 全绿（+13 例，含 `tickWidthAt` 三例）、typecheck 全绿；证据图 `docs/proofs/ui-v3/context-tree-ui-hover.png`（hover 展开 + 单条摘要态）。
- **教训（记档）**：① 「参考 X 的实现和样式」= 先读 X 源码拿确切数值与状态机（24/16/12/200/8/120ms 照搬），别凭印象猜。② 但**参考只管几何，动效与强调的口味不在参考里**：本轮参考的 active 满宽、200px 全轮次列表、贴顶定位三处被否，而 v2.1 被批「自造」的正态分布展开恰恰是用户想要的。**结论：形态类改动先出一张实现图过一眼再铺开**，三轮返工能压成一轮。
...
| 日期 | 任务号 | 类别 | 内容 | 影响 |
|---|---|---|---|---|
| 2026-09-08 | T9.2 v2.2（§7.10） | 形态改判+修既有 bug+完成 | **用户判「这个对话缩略图不对，完全参考 OpenWood 的实现和样式」**。定位参考源＝`OpenWood/.cache/opencode-v1.18.21/packages/session-ui`（`message-nav.tsx`/`message-nav.css` + `diff-changes.tsx` bars 变体），按源码逐值改写 MessageMinimap：① 刻度只按 **user 轮次**（v2.1 是每条 user/assistant 一格）；② 行高 12px、**常态每根完全一样**（16×1px 发丝线、同色，`data-active`/加粗规则删净）；③ hover 才动：以目标为中心**正态衰减展开**（目标 30px、邻居按 σ=1.6 跟着变长、远端回 16px，200ms），长度与颜色走同一条衰减曲线（`color-mix`），容器预留 20px 满宽不推动布局；阅读锚点降级为只做长对话内的刻度条跟随滚动；④ hover 某根刻度 → 右侧 8px 只浮出**这一条**的摘要（diff bars + 首行标题、按该行中心竖向对齐、离开 120ms 后收）——参考的 200px 全轮次列表被用户否掉（「不要展示其他上下文」），增删数取该轮 tool 行 `diffStat` 汇总、分档规则照搬 `DiffChanges.blockCounts`；⑤ 位置**保持竖向居中**（撤销第一轮的贴顶），轮次过多内部滚动且藏滚动条；**不做任何默认高亮**（第二轮残留的「当前轮加粗 2px」也删掉，锚点只用于跟随滚动）。纯逻辑下沉 `lib/message-nav.ts` + `lib/outline-bus.ts`（各带单测、登记 test 清单）。**顺带闭一枚静默 bug（新用例 U3.1 首捕）**：锚点走 window CustomEvent + 兄弟组件 effect 挂载顺序 ⇒ 首帧锚点在刻度条挂监听前发完，短对话不再有 scroll 事件补发，active 态长期空着；改总线缓存末值 + 订阅即回放，并让 MessageList 在行表变化/切对话时重发锚点。验收：UI 探针重写 **14/14 EXIT=0**、headless 13/13、desktop 单测 **273**、typecheck 全绿，新增 `docs/proofs/ui-v3/context-tree-ui-hover.png`（正态展开 + 单条摘要同帧）。 | 缩略导航定稿为「常态完全等长发丝线 + hover 正态展开 + 单条摘要浮层」：几何数值取自参考、动效与强调由用户拍板；锚点总线让「晚挂载的导航消费者」不再依赖 effect 顺序；**教训入档两条**：①「参考 X 的实现和样式」= 先读 X 源码拿确切数值与状态机；② 参考只覆盖几何，**动效/强调的口味不在参考里**（本轮 active 满宽、全量列表、贴顶三处被否，而被批自造的高斯展开恰是用户要的）——形态类改动先出实现图过一眼再铺开 |
| 2026-09-07（晚） | T9.2 v2.1（§7.10） | 语义改判+完成 | **消息级分叉语义按用户实测改判：「停在触发提问」→「含被点回复所在轮结束」**。用户报「4 条消息点第 4 条 fork 只有 2 条」——现场取证（其 openchamber/OpenWood 真实会话文件）证实引擎落盘无误、是语义与直觉不符 + 旧实现渲染层直传 entryId 在「一轮裂多气泡」（tool-call 轮次）下存在错位风险。改造：`engine:forkToNewConversation` 入参 `{conversationId, userOrdinal, leafId?}`，**轮末条目定位收进主进程**（pathToLeafIds 取路径 → 第 N 个 user 条目到第 N+1 个之前 = 轮末），渲染层只数 user 行（1:1 天然稳）；回执补 `sourceFile` 供写 `forkedFrom` 谱系（不再依赖树缓存）。探针 U4 重写 + 新增 U4.3「轮中分叉」用例 → **11/11**；headless 13/13、单测 260、typecheck 全绿。**运维坑记档**：带窗探针静默 EXIT=0 零输出 = 被用户在线实例的单例锁挡下（boot.log 可见 gotLock 时间戳），先停残留实例再跑。 | fork 语义=「复制到这里为止，从这里继续」，与「换分支（navigateTree，备用无 UI）」彻底分工；按轮定位消除了多气泡错位的整类隐患 |
| 2026-09-07（下午） | T9.2 v2.1（§7.10） | 形态改判+完成 | **用户改判缩略树形态，同日重做**：① 侧栏（w-56 aside）整件撤除——「做太重了，不该单独弄侧栏」；换成 **MessageMinimap 刻度条**：消息列左缘浮层不占布局宽度，每条 user/assistant 一个刻度、常态等长 14px，hover 正态分布重排（目标 30px、邻居高斯衰减）+「角色+首行摘要」tooltip，点击复用 `piwood:outline-jump` 跳转+flash，阅读区高亮复用 `piwood:outline-active` 锚点，<720px 自动隐藏，Header 树开关/两个 settings 项/弃枝摘要 UI 一并撤除；**引擎层 navigateTree/session_tree/leafId 过滤/树 store/纯函数全部保留备用**（无 UI，随时可接）。② fork 从「树上的按钮」改为**消息级**：AssistantRow 动作排加「分叉」（复制/重试旁）= 把该回复之前的全部对话经 `engine:forkToNewConversation`（`createBranchedSession` 只含 root→触发提问路径、header 带 parentSession）分叉成**新对话**（独立 worktree），别名「Fork of <源首问>」+ session-meta 新字段 `forkedFrom` 落谱系；新对话底部「⑂ 从对话中派生」chip 点击回跳源对话；行↔树 entryId 用「第 N 条 user」序号对齐。③ 踩坑两则：(a) **ResizeObserver 挂早**——组件 `ticks<2` 时 return null，`useEffect([])` 首跑无 DOM 后永不再挂（窄窗隐藏失效），改 callback ref 在真 attach 时建观察；(b) 探针「Fork of」body 文本断言不成立——临时项目默认折叠对话行不渲染，改断 `sessions:meta` 落盘值（渲染路径 alias??首条消息 T8.11 已证）。④ 验收：UI 探针重写 **10/10**（刻度数/高斯重排 30>27>21/tooltip/点刻度 flash/分叉两对话+停在触发提问+chip+谱系落盘/回跳/窄窗隐藏）、headless 探针 13/13 原样过、**全量门禁 21/21 EXIT=0**、单测 desktop 260（+forkedFrom 语义 1 例）；截图 `docs/proofs/ui-v3/context-tree-ui.png`。 | 缩略导航不再挤压正文；「沿某条回复岔开再试」成一键盘动作；**形态教训入档**：导航类附加物先问值不值得占布局宽度；v2 侧栏的宽度自适应/开关/摘要开关三件套随形态撤销（引擎能力未浪费，留作分支 UI 备用底座） |
| 2026-09-07 | T9.2（§7.10） | 新功能+修既有 bug+完成 | **上下文缩略树 v2：真分支树（navigateTree 切 leaf + 路径过滤 transcript + 双击切分支）**。① **SDK 源码级核实**（0.84.4）：`AgentSession.navigateTree(targetId,{summarize})` 只挪 leaf、不截断、零写盘，流式中抛错；目标是 user 条目时 leaf=其父并回填 `editorText`（检查点式「改这问重发」），完成后自动刷 agent 状态并广播 `session_tree{newLeafId,oldLeafId}`——⚠ 裸 `sessionManager.branch()` 不刷 agent 状态，必须走 navigateTree；leaf 指针**不落盘**（重开回时间戳最新末梢）；`entry_appended` 只覆盖扩展自定义条目、普通消息不带 entry id ⇒ 树数据走「轮末重读 jsonl」，不靠事件增量。② **引擎链**：`session-tree.ts` 纯函数 `pathToLeafIds`（防环、坏叶返回 null 降级不过滤）；EngineAdapter 链补 navigateTree 六件套（ipc-schema RPC 表+参数 schema / pi-types / adapter 接口 / sdk+remote 实现 / engine-child 分发表）；`engine:navigateTree` handler 按 conversationId 取引擎、休眠显式拒不惰性拉起；`EngineEventSchema` 补 `session_tree`（envelope 回归例同 entry_appended 钉法）；`sessions:tree` 行投影加 `role/textHead`（分支题面）；`sessions:messages` 加 `leafId` 过滤。③ **渲染层**：`lib/context-tree.ts` 纯函数（deriveContextTree/expandBranch/navigateLandsOn/defaultLeafId/branchTitle）+10 例单测；`context-tree-store` 叶保持规则（**手选叶只有当默认尾相对手选时确实前移才自动跟随**——否则纯浏览旧分支会被轮末刷新打掉，规则迭代了两版）；`rebaseToBranch`+`rebaseFromHistory`（切分支整体换底、不与旧增量对账，旁支条目本就该消失）；ContextTree v2 虚线旁支行/展开列问/双击切换/回填/「回到最新」条/头部 ⑂N 计数。④ **带窗交互探针首捕两枚既有 bug**（都是「静默没跑」型）：(a) `ensureHistoryLoaded` 把 `engine:listConversations` 的 `{conversations,capacity}` 载荷当数组、按不存在的 `r.conversationId` 查 → 每次装载都 TypeError 被 void 吞，**「切到后台对话整读对账」自 T8⑤ 起从未真正生效**（当时症状被 epoch 修复掩盖）；(b) `noteEventOwnership` 采纳 active 戳直接置 id、不走装载分支 → 同 id `switchTo` 早退，对话空壳停在 Onboarding。均修 + 探针断言钉住。⑤ **顺带补洞**：`engine:switchSession` 后把新会话文件同步回注册表（此前 HistoryPane 续写不更新 `record.sessionFile`，渲染层按文件取树/取历史会拿旧值）；侧栏续写改为「先切引擎→刷树拿叶→按叶过滤 loadHistory」。⑥ **验收**：`--context-tree-probe`（无窗无模型）**11/11**（A 数据面 + B 真 child 活链路：editorText 回填/assistant 目标不回填/零写盘 1538→1538 字节）；`--context-tree-ui-probe`（带窗、对真 DOM 元素派发 dblclick）**8/8**；两枚新探针进 `t88-verify.sh`（UI 版受 SKIP_GUI 控制）→ **全量门禁 21/21 EXIT=0**（含真模型 ui-chat + mac 打包 + packaged 探针 21/21）；单测 desktop 258/engine 16/ipc-schema 71 全绿；截图 `docs/proofs/ui-v3/context-tree-ui.png`。**踩坑三则**：(a) UI 探针首跑忘 `electron-vite build`，`electron .` 跑旧产物=普通启动（表现为「进程活着没输出」），带窗探针必须先重建；(b) 双击 user 条目的期望写错——SDK 语义是 leaf=其父，transcript 该只剩公共前缀（问题在输入框里），断言按语义重写；(c) macOS 无鼠标注入下渲染层交互的可脚本化正道 = `webContents.executeJavaScript` 对人元素派发 `MouseEvent("dblclick",{bubbles:true})`，React 根委托正常接收。 | T9.1 的 v2 承诺兑现：分叉会话在桌面「看得见、切得动」；两枚静默 bug（后台对话整读对账从未生效/启动期采纳空壳）闭掉；`engine:switchSession` 注册表一致性补洞；剩 v2.1 候选：宽度自适应收起、弃枝摘要（summarize）、forkFrom 另开新对话入口（§7.9 语义） |
...
| 2026-09-05（同日第五轮） | T8.4（§7.9） | 补验证工具+完成+踩坑 | **`--approval-probe`：把审批安全底线从「单测 + stub 恒拒」升级到「真裁决路径真机断言」**。**为什么要单独立探针**：`--conversation-probe` 里的 `decideApproval` 是**探针自己装的假实现**，它绿了只证明「宿主说拒、child 就拒」，**没有证明真主进程在没人应答时会自动拒**；而 T8.4 的原计划是「并入 --conversation-probe」——实际做不到：本探针要动 settings（钉策略）与 env（压超时），混进去会污染 C 组的 stub 前提。① **九条断言真机 9/9 EXIT=0**：D0 前置（把策略钉成 `allAsk` 并确认 `decide(bash)==="ask"`，钉不住就直接失败——否则后面全是空转）；D1 反向通道清单封闭（`host:tool-execute/ui/approval/subagent` 四条，正则扫不到任何 policy/allow/grant/approve 类自批入口）；**D2 端到端 deny-by-default**——真 child 发 3 次 `host:approval`、无渲染层应答，**child 侧确实收到 3 个 `allow:false`**（2729ms 收敛，不挂死）；D3 同一 ticket 第二次裁决被拒（「审批票据已消费（疑似重放）」）；D4a/D4b 后台对话确有一条在飞 pending，且**拿别人的审批 id 从另一对话应答被拒、pending 保留给发起对话**；**D5a/b/c 可见性分档**——后台 pending 活过 2×超时窗口仍在（`pendingMs=1995ms`、`armed=false` 未起表）→ 切过去 `armPendingTimeoutsFor` 后 `armed=true` → 起表后超时落 `deny`（reason「用户拒绝该操作」）且 pending 出清。② **为可测性做的抽取与一处测试缝**：`decideApprovalFor(ctx,p)`（从 caps 字面量里搬出，caps 委托调用，行为逐字不变）、`respondApprovalFor(p)`（从 `approval:decide` handler 搬出）、`pendingApprovalSnapshot()`（只读，带 `id/armed/createdAt/pendingMs`——为此给 pending 条目补了 `id` 与 `createdAt` 两个字段）、`PIWOOD_APPROVAL_TIMEOUT_MS` 测试缝把 120s 压到 900ms。③ **⚠ 测试缝差点写成死的**：第一版把超时提成模块常量 `const APPROVAL_TIMEOUT_MS = approvalTimeoutMs()`——**`index.ts` 静态 import 本模块，常量会在探针设置 env 之前就被固化，缝会静默失效**（表现是探针慢到 120s 或直接超时判错）。改成**起表时调用** `approvalTimeoutMs()`（两处 arm 定时器），并在注释里钉住这条因果。④ **⚠ D2 的关键改法**：原先只能看主进程日志推断「child 应该收到拒了」；现在让 `debugEcho` **把宿主回执收进返回值带回主进程**（`verdicts[]`，上限 20 条），断言打在 child 实际拿到的东西上——**安全底线的证据要落在被保护方的观察上，不是落在守门人的日志里**。⑤ **探针会临时改用户设置，必须可恢复**：`replaceSection("approval", {mode:"allAsk"})` → `finally` 里恢复原值并打印「已恢复审批策略为 X」，恢复失败打 ⚠ 提示手工检查（本轮实测：跑完 `~/.pi-wood/settings.json` 仍是 `{"mode":"allAsk","rules":[]}`，与跑前一致）。⑥ **门禁**：`SKIP_UI=1 bash scripts/t88-verify.sh` **全 18 步 EXIT=0**（第 4 步新增 `probe-approval`）；typecheck 绿、`pnpm -r test` 320 全绿、其余探针复跑不破；`package.json` 加 `probe:approval`。⑦ **仍欠（不掩饰）**：PromptTray 来源行的视觉目检（GUI）；生产 120s 档的真实体验没人盯过（探针压到 900ms，行为等价但「等两分钟才自动拒」的手感是否可接受属产品判断）。 | T8.4 三条验收（跨对话应答被拒 / 后台不被静默拒 / deny-by-default+票据重放）**由 `[~]` 升 `[x]`**、「两对话同时待审批」拆半（归属与超时独立已断言，只剩来源行目检）；§7.9 状态表 T8.4 行改写、「剩余条件 ③」→**探针类工具已全部齐备**（作用域 14/14、安全底线 9/9、度量 7/7 + 5/5，均在门禁第 4 步）；证据 `docs/proofs/T8.8/probe-approval.log` |
| 2026-09-05（同日第四轮） | T8.7（§7.9） | 补验证工具+完成+踩坑 | **`--workspace-scope-probe` 落地**——T8.7 立项时写明「验证方式：无窗探针，2 项目 + 同项目两 worktree 交叉调 fs/git/snapshot/memory/term 断言归属」却从未实现的工具，本轮补上并真机跑通。① **14 条断言**（`electron/main/engine/workspace-scope-probe.ts`，stub caps + 真 child + 真 git 树）：W1 `getActiveWorkspaceDir()` 随 active 对话走且三棵树互异；W1b 同项目两对话 projectDir 相同而工作树不同（隔离前提）；W2 fs 写盘只落 active 那棵树（其余两树 + 主项目都查无此文件）；W3a 分支名 === 纯函数 `branchFor(id)`；W3b A1 改 README 不污染 A2/主树（`git status --porcelain` 互不含对方）；W4 快照按树归集（A1 服务出 1 条、A2 服务 0 条、**且 A2 树上的 README 内容原样**）；W5a `mainProjectRootOf(树)===主项目`；W5b 两条对话的 project 记忆落**主项目同一份** project.json 且都在；**W6 八路并发 `memory_save` 一条不丢**（最终 10 条，`FileWriteQueue.withLock("memory")` 真串行）；W7a 聚合树集合含主项目 + 每条对话的树；W7b 同 id 去重 + modified 新→旧；W8 终端 cwd 解析（含「无 conversationId / 查不到树 → 保留请求值」的回归语义）；W9 非 git 目录**显式降级**共享主树、作用域不悬空；W10 close 一条后 active 不指向已回收的树。② **为可测性做的两处抽取重构**（不是为探针而扭曲产品代码，是把判定收进单一实现）：`session-service.listSessionsAcrossTrees(projectDir, lister?)` + `worktreeTreesOf()`（`data.ipc` 的 `sessions:list` handler 改调它，行为逐位一致；**lister 可注入**让探针不必依赖 Pi 的会话目录编码规则就能断言聚合语义）、`terminal-service.resolveTermCwd(requestedCwd, conversationId, worktreeOf)`（handler 改调它）。理由同一条：**探针不许复制第二套判定逻辑**——复制品测绿了不等于应用没串。③ **⚠ 一次探针自身写错判据（记下来免得重犯）**：首跑 W4 假失败——我按 `chA1[0].file.startsWith(dirA1)` 断言，而 `SnapshotService.collectChanges()` 返回的 `file` 是 `relative(this.projectDir, full)` **相对路径**。读实现后改成断言 `=== "README.md"` 并**加一条更硬的证据**（A2 树上的文件内容真没被改）。教训同 T8.9 那条同源：**断言要贴着实现的事实写，失败先怀疑判据再怀疑产品**。④ **顺带被真机证实的一处设计**：探针收尾时主进程打出「对话 conv-1 的工作树未回收：工作树有未提交改动（未跟踪 1 项）。请先回流，或确认丢弃后强制删除」——T8.6 的**脏树拒绝回收**在真机上按预期生效（探针故意改了 README 没回流），不是 bug、是证据。⑤ **门禁**：`SKIP_UI=1 bash scripts/t88-verify.sh` **全 17 步 EXIT=0**（新增 4) `probe-workspace-scope`）；typecheck 绿、`pnpm -r test` 320 全绿、`--conversation-probe` 21/21 / `--concurrency-probe` 4/4 / `--latency-probe` 7/7 / `--ui-latency-probe` 5/5 / packaged 21/21 复跑不破；`package.json` 加 `probe:workspace-scope`。⑥ **T8.7 仍欠（不掩饰）**：终端切回 shell 缓冲未被 kill、两对话浏览器 page 互不覆盖、`pi --resume` 人眼确认可选到 worktree 会话——三项都要 GUI/CLI 人手，探针替代不了。 | T8.7 两条核心验收（跨项目并存 / 同项目两 worktree 归属）**由 `[~]` 升 `[x]`**、「CLI 互通不回退」半闭（聚合语义已断言、CLI 侧目检待）、门禁验收行改写；§7.9「真机验证状态」表 T8.7 行与「剩余条件 ③」同步（度量/作用域类工具已齐，只剩 deny-by-default 探针断言与 GUI 人工轮）；证据 `docs/proofs/T8.8/probe-workspace-scope.log` |
| 2026-09-05（同日第三轮） | T8.10（§7.9） | 新增任务+完成+踩坑 | **带窗红线度量出口：把 T8.9 剩下的三行（第二跳 / 切换首屏 / 掉帧+主进程 CPU）也变成可自动判定**。① ** plumbing 四层**：envelope 加 `tPush?`（`sendFrame` 每帧一次 `nowEpochMs()`）→ **preload 的 lite 解包同步透传**（那份复制是 T8.2 刻意留的，漏透传第二跳就永远 0 样本，注释已钉同步义务）→ 渲染层 `src/renderer/src/lib/latency-outlet.ts`（第二跳/首屏/帧间隔自测，**rAF 帧表只在「最近 600ms 来过事件」且窗口可见时开**、空闲 600ms 自动停；样本本地攒批 **2s 一次**上报——每帧一次 IPC 会让度量本身变成性能问题）→ 主进程 `electron/main/engine/renderer-latency.ts`（四个环形窗口汇聚 + **主进程 CPU 用 `process.cpuUsage` 差值 ÷ 墙上时间、只在有样本的活动窗口里计入**，空闲期计入会把 ≤60% 稀释成好看；载荷非法只计数不抛）。② **带窗探针 `--ui-latency-probe`**：三路对话（同项目 ×2 + 跨项目 ×1，各占一棵 worktree）按 **16ms 间隔**灌 `debugEcho` 合成事件（≈60 token/秒/对话、合计 ~180 帧/秒），**负载进行中用 `executeJavaScript` 点真标签切换 6 次**，五条断言 U1 第二跳 / U2 帧间隔 / U3 主进程 CPU / U4 切换首屏 / U5 上报链路健康（`reportedSamples>0 && rejectedBatches===0`）→ **真机 5/5 EXIT=0**：`rendererHop` p50=0.149/**p95=1.872ms**（n=94，max 3.767）、`frameGap` p50=8.3/**p95=9.5ms**（n=987，max 16.8 ⇒ 满帧无掉帧）、**主进程 CPU p95=3.683%**（单核）、`firstPaint` **p95=8ms**（n=6）、节流后 `eventHop` p95=0.422ms。③ **EnvironmentPanel 新增红线行**：六项 p95 + CPU 一行，**无样本显示「待采样」而不是 0ms**，超预算标红，预算从 `LATENCY_BUDGETS` 取（渲染层不自己抄数字）。④ **⚠ 两条教训（比数字值钱）**：(a) **首跑用 600 帧无间隔突发 → 第二跳 p95 = 261ms「爆表」**，那不是产品慢是**负载模型错了**（突发测的是「管子塞多快」= 排队延迟），改 16ms 节流后同一份代码 1.87ms ⇒ **红线数字必须绑定负载形状**，已把 `gapMs` 写进 `DebugEchoParamsSchema` 并在注释里留住这条反例；(b) **主进程自己调 `setActiveConversation` 测不出首屏**——起点在渲染层 store（`markSwitchStart`），主进程改 active 既不产样本也不会让渲染层的可见性判定跟上 ⇒ 探针必须点真标签（`[role="tab"] > button`），走的正是用户那条路径。⑤ **回归**：typecheck EXIT=0、`pnpm -r test` **320**（desktop 228 含 latency-outlet 7 例 + ipc-schema 70 + engine 12 + plugin-api 10）、`--conversation-probe` 21/21、`--concurrency-probe` 4/4、`--latency-probe` 7/7 复跑不破、build EXIT=0；门禁脚本新增 4b) `probe-ui-latency`（`SKIP_GUI=1` 可跳，它要可见窗口会抢焦点）。⑥ **未闭合（诚实标注）**：合成事件**不含模型 token 生成与真实 markdown/高亮成本** ⇒ 真模型三路体感仍归人工轮；面板那行只经构建 + 探针链路验证，**人眼目检留 GUI 轮**；`approvalRtt`/`hostToolRtt` 在本探针恒 0 样本（不判达标，如实显示）。 | **性能红线表「仍无对象」只剩两条**（在飞 prompt/子代理峰值计数需真模型灌流、真模型负载下的体感复核）；§7.9 新增 T8.10 条目 + 状态表行 + 落地顺序第 12 步；RPC 行由「半量」转整行闭合；证据 `docs/proofs/T8.8/{probe-ui-latency.log,ui-latency-probe.png}`（截图里三标签各带「树」角标、合成流已上屏 = 渲染路径真走通）；下一轮 `t88-verify.sh` 会自动跑 `probe-ui-latency` |
| 2026-09-05（同日更晚） | T8.9（§7.9） | 新增任务+完成+踩坑 | **红线度量出口：把「有预算、无口径」的三行红线变成可自动判定**（用户拍板「先补度量出口，再排人工目检」）。**为什么立任务**：性能红线表「RPC 往返 / 事件到达 / 审批往返」三行自 §7.9 立项起就只有毫秒预算，探针**没有 p50/p95 统计出口**——按 §0 规则「没有可自动判定的口径，红线就只是愿望」，这三行永远只能写「待真机」。**只做度量、不改任何性能路径**（不做批流、不动 T8.3 合帧阈值）。① **纯模块 `packages/ipc-schema/src/latency-stats.ts`**（electron-free）：`LatencyRecorder`（固定容量**环形窗口** + `dropped`/`invalid` 计数——长跑进程用全量样本会让早期抖动永久污染结论、且无界吃内存）、`percentile`（最近秩）、`summarize`、`mergeRecorders`（跨对话合并的是**样本**不是快照平均——平均的平均会掩盖单路抖动）、`LATENCY_BUDGETS`（预算常量，改它=改红线）、`budgetVerdict`（**count=0 一律不通过**并写明未触发）、`RPC_LATENCY_SAMPLED_METHODS`（取样域）。+23 例单测（ipc-schema 41→**64**）。② **协议加法（向后兼容，不升 `ENGINE_RPC_VERSION`：两侧同构建产物）**：下行 `ping`（空操作，量 main→child→main 纯通道）与 `debugEcho`（**非 `PIWOOD_ENGINE_PROBE=1` 一律抛**——不给生产留合成流量的后门；events ≤2000/approvals ≤500 由 zod 夹住防灌爆）；`EventUp.sentAt?`（跨进程 epoch 戳）；上行新帧 `metric {name,ms,n?}`（child 单时钟自计、fire-and-forget、不参与 seq 对账）；`HostApprovalResult.auto?`。③ **三条取样纪律（这次收口的真正内容）**：(a) **模型耗时不入通道表**——`prompt/steer/followUp/start/compact` 的往返混着模型与装配时间，塞进同一张直方图会把 p95 抬到秒级、把红线判成不可能达标，那是**量错东西**不是量超了；(b) **人的思考时间不入审批表**——审批只在 `auto===true`（规则直拒/直放、会话自动接受、票据失效，即**没弹用户卡**）时记样本，否则一次 120s 等待就能把 p95 撑爆；(c) **无样本不判达标**——`hostToolRtt n=0` 在探针输出里如实显示为 0 而不是省略。④ **接线**：child 事件帧发 `sentAt`、`rpc()` 加 `{name, when}` 度量参数、`ping`/`debugEcho` 处理器；主进程 `EngineHost` 三个窗口（rpcRtt 256 / eventHop 512 / 按名归档 metrics 256）只在帧处理里做加减法**不参与任何判定**，`latencyReport()`/`latencyRecorders()`，注册表 `engineLatencySummary()` 跨活跃对话聚合。⑤ **探针 `electron . --latency-probe` 真机 7/7 EXIT=0**：L1a `rpcRtt` p50=0.048/**p95=0.093**/max=0.275ms（n=300，预算 ≤40）；L1b **统计按对话独立**（A/B 各 150 样本互不污染）；L2 `eventHop` p50=1.842/**p95=2.292**/max=2.462ms（800 帧突发灌 512 窗口，预算 ≤20）；L2b 脏样本占比（invalid=0/512，>10% 判失败——那说明跨进程时钟口径本身坏了而非网络慢）；L3 `approvalRtt` **p95=0.087ms**（n=60）；L4 聚合视图（hosts=2、合并 n=300）；L5 **close 全部后窗口出清**（`hosts=0`、各 count=0，否则重启应用会读到上一轮的旧分位数）。⑥ **⚠ 两个踩坑（都写进了代码注释）**：(a) **`ipc-schema` 被渲染层共用**，我在 `engine-rpc.ts` 顶部写 `import { performance } from "node:perf_hooks"` 直接让 **renderer 构建立刻失败**（`"performance" is not exported by "__vite-browser-external"`）→ `nowEpochMs()` 改读 `globalThis.performance`（Node ≥16 与浏览器都有），缺失退回 `Date.now()`；**教训：给共享包加任何 `node:` 依赖前先想渲染层**；(b) metric 帧是 child 在 respond **之后**补发的，读快照前不 sleep 一下就会把「还没到」误判成「无样本」。⑦ **没闭上的（诚实标注）**：`eventHop` 只是「两跳」的**第一跳**，**main→renderer 那一跳本探针测不到**（无窗口），切换首屏 ≤100ms、3 路并发前台体感、主进程 CPU 同样需要真渲染层 → 探针显式打 SKIP，出口已就位（`engineLatencySummary()` 可被 IPC/面板读），**待接进 `--ui-chat` 与 EnvironmentPanel**；本探针数据是**无负载下界**（ping 不碰引擎、审批 stub 立即回），不许当「真机用起来就这么快」的承诺值引用。 | 红线表「仍无对象定档 5 行」**缩为 3 行**（3 路体感、CPU、切换首屏）+ 在飞峰值计数需真模型灌流；`t88-verify.sh` 第 4 步新增 `probe-latency`、产物符号断言加 `debugEcho`；`package.json` 加 `probe:latency`；§7.9 新增 T8.9 任务条目与「真机验证状态」表行、落地顺序补第 11 步；门禁全绿：typecheck EXIT=0、`pnpm -r test` **307**（desktop 221 + ipc-schema 64 + engine 12 + plugin-api 10）、build EXIT=0 |
| 2026-09-05 | T8.8（§7.9） | 完成+修复+门禁缺陷+踩坑 | **真机门禁日（macOS 本机）——T8.4~T8.8 的真机欠账一轮集中还清**。① **首跑 fail-fast 在 `--concurrency-probe` 断言①**（`list=2 cwd 去重=2`，探针 11:31 挂）→ 定位链：临时项目 `mkdtemp` 出来**不是 git 仓库** → `detectWorktreeFeasibility` 判 `not-git` → 走 **degraded-shared 共享主树** → 「三对话 cwd 各不相同」这条断言在结构上永不成立（**不是隔离失效，是探针前提缺失**）。修：`makeProject()` 里 `git init`+`config user.*`+`add`+`commit` 造真仓库，并加 `cwdDetail` 输出（`conv-id:shortId` 三棵树可见互异）；另补 `app.exit(code)`——主进程设 `process.exitCode` **不会让 Electron 退出**（与 conversation-probe 同法，此前探针「跑完不退」同源）。② **修 A：`ensureConversation(opts.newConversation)` 泄漏 + 串会话**——原实现把「同项目再开一条」写成「先摘既有句柄再新建」，而摘句柄**不区分活跃与否**：`newConversation=true` 时若既有对话进程还活着，等于把旧对话连 child 进程一起丢表不管（**进程泄漏**），且 `resumeSessionFile` 被无条件接回 → **新对话继承了旧对话的会话文件**（语义正好反了：新对话应当空白）。改：`existingAlive` 先判，**只在已死时摘句柄**；`resumeSessionFile`/`carriedRestarts` 仅在「已死且非 newConversation」时携带。③ **修 B：`reconcileOrphans` 未 realpath 归一**——git `worktree list --porcelain` 返回的是解析后的真实路径，而调用方传入的 `projectDir`/`activePaths` 含软链段（**macOS 临时目录 `/var/folders/...` → `/private/var/...`**），不归一就会把**活跃树误报成孤儿**（用户侧=设置页提示「发现 N 个未回收工作树」纯属假警报，且真去回收会杀掉在跑的对话）。改：新增 `realpathSafe()`（路径不存在时原样返回），`projectDir`/`activePaths`/每条列表项先归一再比。④ **修 C：`scripts/t88-verify.sh` 两处自身缺陷**——(a) `PROOF` 用相对路径，而探针阶段 `cd apps/desktop` 后**证据落盘失效**（改绝对）；(b) ⚠ **`pkill -9 -f "Electron.app/Contents/MacOS/Electron"` 会误杀用户其它 Electron 应用**（本机实测命中 Trae CN / QwenWork / ZCode 的 crashpad 与主进程），模式必须锁 `pi-wood.*` 前缀；(c) build 改 `--filter @pi-wood/desktop` 免被其它包 preflight 拖挂。⑤ **⚠ 门禁漏洞两处（本轮新查出，比探针失败更值得记）**：(a) `apps/desktop/package.json` 的 `test` 是**显式文件清单**，T8.8 新增的 `src/renderer/src/stores/conversation-badge.test.ts`（`badgeFor` 3 + `tabTitle` 2）**从未登记** → 门禁实跑 **216**、而 a80246f 提交说明与 §7.9 正文都写 **221/221**（那 5 例手工跑是过的）→ 补登记，并在脚本加 **0.5) 单测清单覆盖对账**（`find electron src -name "*.test.ts" -not -path "*/vendor/*"` 逐个核对是否在 `scripts.test` 里，缺一个即 fail）；(b) **`*.log` 全局忽略把 `docs/proofs/` 下的门禁证据日志整个吞掉**（typecheck/test/build/probe-* 只留在磁盘、不进 git 历史＝证据丢失）→ `apps/desktop/.gitignore` 加 `!docs/proofs/**/*.log` 豁免（**注意**：更深一层的 `.gitignore` 覆盖根上的反向豁免，豁免必须写在 `apps/desktop/.gitignore` 里才生效），探针 `.started` 半途崩溃标记则新增忽略。⑥ **最终全绿（11:46~11:55 一次跑完 + 打包段复跑）**：`test-coverage` 22 文件全在案；`pnpm -r typecheck` EXIT=0；`pnpm -r test` **desktop 221/221（首次名副其实）+ ipc-schema 41 + engine 12 + plugin-api 10 全绿**；`electron-vite build` EXIT=0 + 产物断言（标签条/`agent_start 排队`/`engineMaxPrompts`/`quotaOverLimitAction`/concurrency-probe 全进 bundle、`"use strict"`=0、`require(`=0）；**五探针回归全过**：worktree **18/18**（沙箱侧此前 15/16，本机含大小写项全过）、plugin **ALL PASS**、goal **7/7**、memory **7/7**、engine-process **14/14**、conversation **21/21**；**`--concurrency-probe` 4/4 EXIT=0**（① 三对话 cwd 各不相同 `[b35ef76b/bbe22d72/8ffe484d]`——**同项目 ×2 + 跨项目 ×1**，即 T8.6「同项目两对话 cwd 不同」的真机硬证、② 切换不打断 dead=0、③ close 后注册表清空、⑥ 审批门在位；⑦ 零残留计数 Windows tasklist 专属 → macOS 记 SKIP）；**`--ui-chat` 真模型闭环 EXIT=0**（DeepSeek，选项目→对话→工具→settle→截图 `docs/proofs/ui-v3/ui-chat.png`。**该图同时是 T8.8 标签条的首张真机目检证据**：「test-project · 对话 1」+「树」角标 + 「+」在位；模型自述工作目录为 `…/test-project/.pi-wood/worktrees/ef71c174` ⇒ **引擎 cwd 落独占树**在 GUI 侧也成立；「2 个工具调用 · 1 个失败」对应 harness 恒拒策略下模型如实转述「用户拒绝该操作」⇒ **deny 路径真机走通**；思考折叠/行内代码/markdown 渲染无回退）。⑦ **macOS 首次打包 + packaged 探针（T8.8 最后一条验收）**：`package:dir` 首跑 **FAIL**——electron-builder 会**重新去 GitHub 下载 `electron-v37.10.3-darwin-arm64.zip`**（本机 `connection reset by peer`），而 node_modules 里早有同版本解包 dist → 改 `--config.electronDist=$(require.resolve("electron") 的 ../dist)` 后 **EXIT=0**，`appOutDir=release/mac-arm64`；**⚠ 产物目录名随平台/架构变**：macOS arm64 是 **`release/mac-arm64/pi-wood.app`**、**不是** 脚本原先硬编码的 `release/mac-unpacked/`（Windows 才是 `release/win-unpacked/pi-wood.exe`）→ 脚本改 glob 探测可执行文件、找不到即显式报错。**packaged 形态 `--conversation-probe=packaged` 21/21 EXIT=0**（7.9s）：child 从 asar 载 SDK+社区扩展、C3 kill→换 pid 自动恢复且其余对话存活、C8.1 后台 1000 token/秒 → **5 帧/秒**（红线 ≤6，**dev 与 packaged 双形态同数**）、合帧不丢序（`0 1 2 3 4 …998 999`）、C4.1 全量关停 161ms。⑧ **性能红线新增定档**（其余见本节末）：child 自报 RSS dev **184~185MB/对话**、**每多一对话净增 ≈1MB**（T8.0 记的 198~225MB 系含 5 个社区包 + MCP 的另一口径）；fork→ready **60~68ms**、child 装配 boot **378~387ms**（services 15 / session 10 / bind 0）；LRU 腾位置 + 新建一路 **794ms**（含等 child 优雅退出）、唤醒后读回同一 sessionId；全量关停 **161~165ms**；**仍无对象定档 5 行**：RPC 两跳 p95（探针未打 p50/p95 统计）、3 路并发流式前台体感/CPU ≤60%、切换首屏 ≤100ms、在飞 prompt/子代理峰值上限（④⑤需真模型灌流）——**不拿单路或 SKIP 冒充已测**。⑨ **三条如实标注的观察项（本轮不修，见下方影响列/后续批次）**：(a) child 在 `suspend`/`quit` 路径自行退出时 `EngineHost.exiting` 未置位 → `crashed=true`，日志把 **code=0 记成「子进程异常退出」**（探针断言注册表已清空、`recoverAfterCrash` 有 `handles.get(id)!==h` 护栏，实测无幽灵重启，但口径误导、且在 UI 上会走 warning 通知路径）；(b) 插件 `demo-kitchen` 在**未选项目**时调 `fs:read` → handler 直接抛 `引擎未启动：请先选择项目`，Electron 打成 `Error occurred in handler for 'fs:read'` 噪声（应改为结构化返回/无项目降级），属 T5.2 插件面健壮性；(c) 同一张 `ui-chat.png` 里**标签条已有「test-project · 对话 1」而 Composer 占位仍写「先在上方选择一个项目」**——harness 收尾态下「项目/对话指针」的显示口径不一致（占位判据没跟着 active 对话走），渲染层小问题，登记不修。**⚠ 环境事实（非产品代码）**：本机 shell 带 `NODE_TLS_REJECT_UNAUTHORIZED=0`，被探针子进程继承后 Node 会打印「TLS 证书校验已关闭」警告——**日志里那条警告与 pi-wood 无关**（仓库内 grep 无此赋值），读者勿当产品缺陷。证据：`docs/proofs/T8.8/{test-coverage,typecheck,test,build,artifacts,probe-worktree,probe-plugin,probe-goal,probe-memory,probe-engine-process,probe-conversation,probe-concurrency,ui-chat,package-dir,probe-conversation-packaged}.log`（本轮起 proofs 下日志入库）。 | **T8.8 主体收口**：五探针 + concurrency-probe + ui-chat + 打包冒烟全绿，**§7.9「代码级完成 + 真机待跑」转为「真机已验」**；`--concurrency-probe` ① 把 T8.6「同项目两对话 cwd 不同」从单测层升到真机层；新增两条门禁纪律（**新单测必须登记进 `scripts.test`**、**证据日志必须能入库**）；性能红线剩 5 行与「并发=1 A/B 对照、真机 4 对话并行改代码+回流目检」仍为欠账（需真模型流量 + GUI 人手），T5.3 打包安装/签名/三平台阻塞不变；`--ui-chat` 截图顺带补上 **T8.8 标签条的首张真机目检证据**；观察项 (a)(b)(c) 挂后续小改 |
| 2026-09-04 | T8.8（§7.9） | UI+键盘+退出确认+探针骨架完成，真机探针/目检待跑 | **多对话 UI（标签条/徽章/未读）+ 并发门禁与回归（T8 批代码收口）**。① 对话标签条 `ConversationTabs.tsx`（ConversationHeader 同排）：标题=首条用户消息（纯函数 `tabTitle` 截断/回落「项目名 · 对话 N」）、状态徽章（纯函数 `badgeFor`：待审批红点>在飞转圈>排队>启动中虚线>关停灰>未读蓝点）、「树」角标+悬浮 worktreePath、关闭按钮、「+」同项目再开一条（T8.6 语义）；数据源=listConversations 轮询 1.5s + slices.unreadCount。② 键盘：`Ctrl/Cmd+Shift+[`/`]` 循环切换、`Ctrl+Shift+N` 新建、`Ctrl+W` 关闭当前（组件内注册）。③ 关闭在跑对话二选一内联卡（无全局模态）：「关停引擎但保留可恢复」=suspend vs「中止任务并关闭」=abort+close（worktree 未回流改动保留+toast）。④ 退出确认：before-quit 查 `busyConversations()`（registry 新导出）→ showMessageBoxSync 列清单，取消 preventDefault。⑤ EnvironmentPanel 资源行：`summarizeCapacity` 纯函数 + `engineRssTotal()` + listConversations 载荷增 `capacity`（对话 N/M · prompt X/Y · 子代理 Z/W · RSS 合计）。⑥ 门禁探针 `electron . --concurrency-probe`（concurrency-probe.ts，stub caps+真 child）：①三对话并存且 cwd 各不相同②切换不打断⑥审批门在位③close 清空⑦零残留（win 口径）；④⑤⑧显式 SKIP（需真渲染层/真模型流量，闸门逻辑已由 41 例单测穷举）。**门禁（实跑）**：typecheck 双绿；`node --test` **221/221**（新增 5）；build EXIT=0 + 产物断言（标签条/确认框/probe 全进产物、ESM 不变）。**⚠ 待真机（集中欠账）**：--concurrency-probe 真机跑、五探针 electron 回归、性能红线 6 行定档、4 对话并行改代码+回流端到端目检、打包冒烟。**轻量未做**：「+」选项目/fork 高级形态、命令面板聚合项、标签 per-对话跨启动持久化、用量页按对话细分（usage 落盘粒度 provider|model）。| **T8.0~T8.8 代码级全部完成**；真机欠账集中在 T8.8 收口（一轮真机门禁日：concurrency-probe + 五探针 + --ui-chat 定档 + 4 对话目检）即可全批标 ✅ |
| 2026-09-04 | T8.7（§7.9） | 代码+单测+typecheck+build 全过，真机探针待跑 | **工作台与项目级资源作用域**。① **作用域唯一入口**：engine-manager 新导出 `getActiveWorkspaceDir()`（=active 对话 worktree ?? 主项目），file/data IPC 注入点、review git diff、plugins `getProjectDir`（插件无对话上下文→取 active，注释写明）全部换源（git 域 T8.6 已改）。② **记忆 scope 归一（补 T7.10 偏差 b）**：`worktree-naming.ts` 增 `mainProjectRootOf()` 纯函数（统一分隔符+大小写不敏感匹配 `.pi-wood/worktrees`），memory.ipc `getProjectDir` 过它 ⇒ worktree 对话记忆落主项目同一份 project.json（memory-tools 同单例自动生效）。③ **写并发保护**：新模块 `workbench/write-queue.ts`（`FileWriteQueue.withLock`：同路径 FIFO 串行/异路径并行/抛错不断链，4 例并发单测含「两并发 save 不丢条目」），接入 memory 全部写 handler 与 `settings:set`；快照写入为同步单线程无交错未包（诚实）。④ **workbench per-对话**：store 重构为 `slices: Record<convId, Slice>` + 顶层投影（5 个消费者读路径零改动），写 action 带 conversationId 路由，`switchConversation` 由 App 订阅 activeConversationId 驱动；main 侧 diff 载荷带 conversationId，后台对话差异静默入各自切片不抢开面板。⑤ **终端 per-对话**：TerminalPanel 改 `Map<convId,{term,pty}>`（上限 8 LRU kill，xterm 实例常驻保缓冲，DOM 随切换摘挂），`term:create` 增 conversationId、主进程解析 cwd=对话树。⑥ **浏览器 per-对话**：全局单 page 改 `Map<convId,{page?,url}>` 共享 browser，**上限 2 活跃 LRU 挂起**（close 留 URL 再访问重开），归属经 `configureBrowserScope` 注入。⑦ **会话聚合**：`sessions:list` 聚合主项目+全部 worktree 树（modified 排序+同 id 去重），sidebar 零改动获得，CLI 互通硬验收的桌面侧完成。⑧ 草稿核实 T7.11 已按 sessionId 成立，零改动。**门禁（实跑）**：typecheck 双绿；`node --test` **216/216**（新增 8）；build EXIT=0 + 产物断言（作用域/归一/写队列/浏览器上限全进产物、ESM 不变）。**⚠ 偏差（诚实标注）**：① workbench 标签跨启动持久化仍只存最后活跃对话（settings.workbench 键未按对话拆）；② 「项目→对话两级树+徽章」UI 随 T8.8；③ 快照写未包锁（同步无交错）；④ `--workspace-scope-probe` 与真机目检待跑。 | T8.7 ✅（代码级）；T8.8 可开工（前置全部就绪）；真机探针欠账集中在 T8.8 终验一并还 |
| 2026-09-04 | T8.6（§7.9） | 代码+单测+typecheck+build 全过，真机探针待跑 | **git worktree 隔离与变更回流（同项目多对话解锁前提）**。新建 `electron/main/worktree/`（**electron-free**，真 git 仓库可单测）：`worktree-naming.ts` 纯函数（路径 `<project>/.pi-wood/worktrees/<shortId>`、分支 `piwood/<shortId>`、Windows 非法字符/结尾点/**统一小写**防大小写不敏感 FS 同名冲突、`isManagedWorktree` 管辖判据——孤儿对账只管自己建的树，不碰用户自建 worktree）；`worktree-service.ts`：`detectWorktreeFeasibility`（not-git/detached/submodule/path-too-long 四类降级各带显式 reason，**不许静默退化**）、`ensureWorktree`（惰性 `worktree add -b piwood/<id> <path> HEAD`，T8.0 实测 151~180ms 同步建不卡 UI；分支已存在→复用重试；**node_modules 策略定案=共享软链**：junction 链接主树依赖、不复制、链接失败静默）、`removeWorktree`（脏树含 untracked 拒绝→先回流/丢弃；force 显式；remove+prune+删分支幂等）、`mergeBackWorktree`（`git -C wt diff <baseRef>` 含已提交+未提交，baseRef 缺失退 merge-base → 主树 `apply --3way --ignore-whitespace`（T8.0 发现③固定口径）→ **内容复核**（--diff-filter=U 判冲突 + name-only 判落盘，不只看退出码）→ **冲突不自动合并：`apply -R --3way` 整体退掉主树冲突态**（绝不 reset --hard 毁用户未提交改动），改动保留树内；untracked 显式列出不自动搬）、`reconcileOrphans`（只报告不偷删）。**接线**：spawnHandle 建 EngineHost 前先建树 → `EngineHost.cwd`/`startEngine.projectDir`/onEvent ctx 全部=树路径（Pi 会话目录、bash cwd、快照/diff 全落树上，两对话物理不互踩）；`ensureConversation(opts.newConversation)` 同项目再开一条（`createConversation` 传入）——**同项目多对话解锁**；`closeConversation` 随对话回收树（`worktree.keepAfterClose` 可留树；脏树保留+notify 不阻塞关闭）；每项目首次建对话孤儿对账（toast）；engine-manager git 域改走 `activeGitDir()`=active 对话树。契约：ENGINE_CHANNELS 增 worktreeList/MergeBack/Remove 三通道 + engine-manager handler。**门禁（实跑）**：typecheck 双绿；`node --test` **208/208**（新增 24：naming 13 + service 11 **真 git fixture**——回流成功路径、冲突不自动合并且主树回退干净且树内改动保留、untracked 显式列出、脏树拒绝、孤儿对账、porcelain 解析）；`probe:worktree`（T8.0 P2）**15/16**——唯一失败=「Windows 大小写不敏感路径」项（探针用 `/TMP` 大写路径，Linux 沙箱大小写敏感 FS 不可复现；T8.0 真机 20/20）；build EXIT=0 + 产物断言（worktreeMergeBack/degraded-shared/共享主工作树全进产物、ESM 不变）。**⚠ 偏差（诚实标注）**：① `worktree.maxTotalBytes` 磁盘配额未实现；② Settings「工作树」页与 DiffPanel「审→回流」按钮 UI 未做（三 IPC 已就位，随 T8.8）；③ 「后台异步预建」未做（同步 151~180ms 实测量级不卡）；④ 回流冲突的「退出」= 改动留树 + toast，DiffPanel 内审 diff 的交互留 T8.8；⑤ 同项目两对话 cwd 断言与 close/对账目检待真机。 | **同项目多对话解锁**（T8.8 标签条的后端前提齐了）；T8.7「MemoryService/文件树绑 activeProject」需按 `worktreePath` 复查；性能红线「worktree 创建/回流」两行维持 T8.0 实测数 |
| 2026-09-04 | T8.5（§7.9） | 代码+单测+typecheck+build 全过，真机探针待跑 | **并发闸门：prompt 队列 / 子代理全局上限 / 第二运行时 per-对话化**。新建纯函数模块 `electron/main/engine/concurrency-gates.ts`（19 例单测）：`clampLimit`（非法值回落默认，0/空串不静默变 1——沿用 T8.1 normalize 的教训）、`effectivePromptLimit`（**min(设置上限, maxLive)**：进程数才是资源上限、prompt 闸是成本闸；限流降 1）、**`SlotGate`**（FIFO 槽位闸：`acquire()` 回执「是否排过队」、`setLimit` 放宽时唤醒等待者、release 不跌破 0）、子代理闸状态机（**预约-快照对账**：agent_start 放行前 `reserved++` 占位，runs 快照里 run 转 running 时按**增量**把 reserved 折算成 running，审批被拒/异常显式退还——不预约则同刻跨对话 acquire 会竞态超限）、`planGoalBegin`、`quotaGuardEffect`。接线 7 处：① prompt 闸门（`engine:prompt`：wouldQueue → `markPromptQueued`（注册表新 action，状态机 `queued`）+ 推 `queue_update` 事件 → acquire → 撤标 → finally release；settings `engineMaxPrompts` 缺省 3）；② 子代理闸（`decideApproval` 对 `toolName==="agent_start"` 在 deny 之后 confirm 之前排队等待 runs 快照唤醒；**每 agent_start 恰产 1 run 是 vendor 事实**，闸在宿主侧不碰 vendored/ADR 0001；settings `subagentMaxTotal`=6/`subagentMaxPerConversation`=4）；③ assist：`generateAssist` 加 conversationId 参数——**per-对话单飞**（修隐性坏掉：原全局单飞让两对话轮流 settled 互相吞）+ 共享单槽 SlotGate(1) 跨对话排队不吞；采集缓冲改 per-对话 `assistBufs`（后台也采集）；**后台 settled 只暂存 `pendingAssist`，切回该对话时补生成一次**（默认只为 active 生成，防 N×每轮成本）；④ review/goal-audit：「后到吞掉」改 SlotGate(1) 排队（⚠ **计划写全局 ≤2，实测共享 adapter/buf/resolveCurrent 单槽下并发 2 会互污染缓冲 ⇒ 实取 1**，在 ≤2 上界内；真开 2 须 per-call 会话隔离，随后续批）；⑤ goal 互斥：`listActiveGoalSessions`+`planGoalBegin`，`beginGoal`/`resumeGoal` 冲突抛错，goal.ipc 捕获 → `ui:notify` toast + 返回当前状态（渲染层不 catch invoke 拒绝），`force` 参数已通到 IPC（接管按钮随 T8.8）；⑥ **btw per-对话化**：单槽改 `btwByConversation` Map——**修一个真 bug**：多对话下 B 的 /btw 会复用 A 的 adapter（cwd 上下文串台）；随 `closeConversation` 释放，换项目不再统一关停（旧 `closeBtw` 调用点删除）；⑦ 配额动作：settings `quotaOverLimitAction`（warn 缺省/throttle/block）——throttle ⇒ prompt 限降 1、block ⇒ `createConversation` 拦新建（`engineStart` 选项目入口不拦，首次使用不 break）、warn 仅用量页告警（T7.12 复用）。**门禁（实跑）**：desktop typecheck 双绿；`node --test` **184/184**（新增 19）；`electron-vite build` EXIT=0（T8.4 沙箱配方）+ 产物断言（agent_start 排队/quota/engineMaxPrompts/强制接管/queue_update 全进 main 产物、ESM 不变）；ipc-schema/engine/plugin-api 三包测试绿。**⚠ 偏差与未做（诚实标注）**：① 审查/审计并发 1 而非 2（见上）；② goal 冲突提示=toast 非正式冲突 UI；③ btw 未做成注册表 `kind:"btw"` 句柄（无独立子进程，Map 挂对话 id 语义等价）；④ 「连续限流降并发」的信号取 quotaWarnings 非空（节流级），429 事件级探测未做；⑤ 按对话显示 token/费用归 T8.8；⑥ `--concurrency-probe` 计数断言与真机目检待跑（本会话无 electron/GUI）。 | T8.5 代码就绪；性能红线「在飞 prompt/子代理 run」两行的**机制**就位、实测数待 `--concurrency-probe`；T8.6（worktree）可开工 |
| 2026-09-04 | T8.4（§7.9） | 代码+单测+typecheck+build 全过，真机探针待跑 | **审批与 ctx.ui 按对话归属 + PromptTray 显示来源**。新建纯函数模块 `electron/main/engine/approval-ownership.ts`（`pendingKey`/`canRespond`/`shouldArmTimeout`/`checkApprovalTicket`/`acceptAllScope`/`groupOrderByConversation`，18 例单测）。契约：ipc-schema 新增 `ApprovalRequestPayload/UiRequestPayload/ApprovalRespondPayload/UiRespondPayload` 四 schema（request 带 `conversationId`+`projectName`，**null=插件全局项**；respond 带 `conversationId`=应答者所处对话）；`host:subagent guard-tool` 补一次性 `ticket`（child `randomUUID()`），与 `host:approval` 的 ticket 同款防重放。engine-manager：pending 表 key 改 `${conversationId}:${seq}`、entry 记 owner+armed；**超时分档**（步骤 4 取「不计时」档）：仅发起对话是 active 才起 120s 表，后台 pending 常驻，`setActiveConversation` 与新增 `approval:focus-requested` handler 都会 `armPendingTimeoutsFor()` 恢复计时；`approval:decide`/`ui:respond` **双保险归属校验**（canRespond + 主进程 active 比对，拒绝时 `ui:notify` 提示，不动原 pending 让用户切过去再答）；`acceptAll` 改 `acceptAllScope` 只放行 active 对话（**无归属的插件 confirm 也不再被连带放行**——比原计划更严：原实现全局遍历会把插件 confirm 一起放掉）；`decideApproval`/`guard-tool` 消费 `consumedApprovalTickets`（同 ticket 二次使用=拒绝，**拒绝也是裁决**，防重放绕过）；新增 `dropPendingFor()` 随 `closeConversation` 回收未应答 pending（未计时项无兜底定时器，不回收会占死 tray 与一个永不 settle 的 Promise）。归属来源：`capabilities.requestUi`/`EngineHostDeps.requestUi` 签名加 `ctx{conversationId,projectDir}`，**由 EngineHost 按 deps 构造、child 不可自报**（与 T8.1「不信任 child 自报归属」同源）。PromptTray：来源行「来自对话：对话 N · 项目名」（对话正式标题还没有，用 fork 序号短标签代，随 T8.8 标签条换真标题）、队列按对话分组（渲染层内联排序，与 `groupOrderByConversation` 同语义）、**非当前对话的请求不抢焦点不自动展开**、动作区整体换成禁用态 + 「去应答」按钮（store.setActiveConversation → IPC → 主进程恢复该对话计时）、折叠态红点标注外部请求。**门禁（实跑）**：`tsc tsconfig.node.json`+`tsconfig.web.json` 双绿；desktop `node --test` **165/165**（新增 18 例）；`electron-vite build` **EXIT=0**——按 T8.P 沙箱配方补 linux 原生二进制（rollup@4.63.1/esbuild@0.21.5/@tailwindcss-oxide@4.3.3/lightningcss@1.32.0 的 linux-arm64 包落 `node_modules/.pnpm/node_modules` hoisted 兜底 + `ESBUILD_BINARY_PATH`，⚠ esbuild 二进制须与 build 同一条命令内下载，/tmp 每次 bash 调用即重置）；产物断言全过（`out/main/index.js`：`approval:focus-requested`=1、`consumedApprovalTickets`=5、「跨对话审批」=2、`"use strict"`=0、`require(`=0 ⇒ ESM 形态不变；`engine-child.js` ticket=2；preload focus-requested=1）。**仍待真机**：`--conversation-probe` 回归、真机两对话并发审批目检（本会话无 electron/GUI，诚实标注）。 | T8.4 代码就绪；**本机补跑 build+探针后即可标 ✅**；T8.5 可开工（与 T8.4 真机验收不互斥）；ticket 一次性语义纳入 `--conversation-probe` 回归候选 |
| 2026-09-04 | T8.3（§7.9） | 完成+实测+偏差 | **渲染层 store 按对话分片 + 主进程可见性节流（性能主战场）**。① 归约抽出 `src/renderer/src/stores/conversation-slice.ts`：`(slice, event, ctx{now,nextId,visible,seq}) => {slice, changed, milestone}` 纯函数（原先长在 zustand 闭包里、依赖 `set/get/Date.now/模块级 seq`，根本没法喂事件序列做穷举），切片含 `items/liveText/liveThinking/streaming/queue/engineReady/currentSessionId/unreadCount/hasPendingApproval/scrollTop/followBottom/historyLoaded/lastSeq/droppedEvents/toolCallCount/runningToolCount/lastMilestone/thinkingStartedAt/headTrimmed`——`thinkingStartedAt` 从模块级变量搬进切片（多对话下必须各归各的）。② `session-store.ts` 改 `slices: Record<convId, ConversationSlice>` + `useActiveConversation(sel)` / `useConversationSlice(id, sel)` / `activeSlice()`；`handleEvent(e, meta)` 自带归属采纳（漏调 `noteEventOwnership` 不该导致静默错路由）、`changed:false` 完全不 `set`；内存护栏 `MAX_SLICE_ITEMS=2000` 裁尾并计 `headTrimmed`。③ **15 个消费文件**迁移到选择器（计划写 16，实测 `TerminalPanel.tsx` 只读项目级 `activeProject` 不需改）；`useSidebarProjects` 的裸 `useSessionStore()` 全量订阅改稳定 action 选择器（否则任何后台写入都拉它重渲染）；`loadMessages`→`loadHistory`；`MessageList` 滚动写回 + 按对话 `followBottom` 决定是否跟底。④ 出站节流 `packages/ipc-schema/src/engine-throttle.ts`（纯、可注入阈值）：active 完全旁路逐 token；后台只合增量（`text_delta`/`thinking_delta` 不互合、不同 `toolCallId` 不互合、`text_start/end` 边界不合），到点/到量出帧；App 侧 `foreign` 事件**仍进它自己的切片**（只计数不丢弃——T8.2 那版提前 return 会让后台永不累积，本批纠正）。⑤ 探针新增 C8 五断言、store 新增 7 例、归约 17 例、节流 8 例。**实测**：后台 1000 token/秒 → **5 帧/秒**（`in=1001 out=6 merged=995`，IPC 降幅 ≈99%）；active 200/200 原样透传；后台 100 条事件让前台选择器订阅者 **0 次**回调、前台自己 1 条新条目恰好 1 次；`--conversation-probe` **22/22 EXIT=0**；`pnpm -r typecheck`/`-r test`/`build` 全 EXIT=0；`--ui-chat` 真机对话 + `--capture` 空态截图绿（迁移无 UI 回归）。**⚠ 三条偏差**：① **计划写的「200ms 或 4KB」阈值实测不成立**——`text_delta` 帧 ≈90B、1000 token/秒 ⇒ 每 200ms 窗口 ≈18KB，4KB 会在窗口内反复提前出帧（实测 ≈22 帧/秒，直接破「后台 ≤6 条/秒」红线）⇒ 默认字节阈值改 **32KB**，只当突发大输出兜底，常规切窗交给时间轴；② 补一条计划没写的**顺序不变量**：里程碑出帧前必须先把该对话缓冲按原顺序冲净，否则渲染层先收到「工具结束」再收到它前面的 token ⇒ 内容错序（C8.4 + 单测盯住）；③ 步骤 4「首次激活整读 jsonl 并与增量对账去重」本批**未接线**（`mergeHistory` 对账纯函数与单测已就位），后台对话历史目前只来自事件累积 ⇒ 验收「切过去首屏内容完整」如实标 `[~]`，真接线随 T8.6/T8.8 的对话标签条；「三路真流掉帧/CPU ≤60%」本批无对象可测（同时只有 1 路真流），归 T8.8 的 stub 并发探针，不拿单路数据冒充。**另**：`window.pi` 在 store action 里加了 `typeof window` 守卫，否则 store 无法在 `node --test` 下被测。 | **T8.3 ✅（体感/CPU 两条留给 T8.8 实测定档）**；性能红线「事件流量」行由占位转实测 5 帧/秒达标；T8.4/T8.8 可开工 |
| 2026-09-04 | T8.2（§7.9） | 完成+发现+偏差 | **IPC 契约加 `conversationId` + 事件按对话路由（envelope 层）**。契约（`packages/ipc-schema/src/engine.ts`）：`ConversationEventEnvelopeSchema {conversationId, projectDir, seq?, active?, event}` + 纯函数 `makeEngineEnvelope` / `unwrapEnginePayload`（**同时吃新信封与旧裸事件**，旧路径标 `legacy` 而不是丢）/ `routeForConversation`；`prompt/steer/followUp/setModel/setThinking/fork` 等命令 schema 统一加**可选** `conversationId`（缺省=active，旧调用零改动）；新增 5 条对话域通道常量（`listConversations`/`createConversation`/`suspendConversation`/`closeConversation`/`setActiveConversation`）。主进程：`send(channel, data, conversationId?)` 改走 **`conversationId → webContents` 路由表**（`engine:start`/`setActiveConversation` 用 `event.sender` 登记；单窗口现状保留 `getAllWindows()[0]` 兜底）+ 新增 `pushEngineEvent`（唯一事件出口，盖 `active` 标记）；**不再由主进程替渲染层丢后台事件**（每条对话都推，分片与节流是 T8.3 的活）；自造的 `model_changed`/`thinking_level_changed` 也包信封 ⇒ 撤掉 T8.1 里「只给 active 推」的补丁。注册表补 `setActiveConversation(id)`（只接受存在且未死的对话，切换即刷 `lastActiveAt` ⇒ LRU 永远挑不中正在被看的那条）与 `activeProjectDir()`。preload：`onEngineEvent(cb)` 归一为 `cb(event, meta)`，**故意不复用 `@pi-wood/ipc-schema`**（preload 构建 `externalizeDepsPlugin()` 无 exclude，workspace import 会变成沙箱 preload 的运行时 require、打包后加载失败）⇒ 内联 `unwrapEnginePayloadLite` 并注释指明权威版本位置与同步义务。渲染层：`session-store` 加 `activeConversationId`/`foreignEventCount`/纯函数 `noteEventOwnership(meta)`（唯一写入口=主进程盖的 `active===true` 权威信号），`App.tsx` 订阅点变路由器（foreign → 计数，不进当前转录本）；**不做 slice-per-conversation**（那是 T8.3）。**三层「丢事件不许静默」计数**：主进程 `sendDrops`、preload `engineEventDropped`（带 reason）、渲染层 `foreignEventCount`，各自首条 + 每 200 条 warn。**⚠ 顺带纠出既有契约缺口**：`EngineEventSchema` 一直漏了**宿主自造**的 `user_message`（`debug:stress` 回显）与 `model_changed`（选模型通知）——它们长期在被推送/消费却没进 schema，envelope 严格校验一上就把它们判非法（旧 preload 直传所以从未暴露）⇒ 已补进契约并注释「不是 Pi 事件」；`engine-envelope.test.ts` 专门有一条回归盯这件事。**门禁（主机 PowerShell 跑 `scripts/t82-gate.ps1`）**：`pnpm -r typecheck` EXIT=0、`pnpm -r test` EXIT=0（含新增 7 例 envelope 单测 + T8.1 的 26/38/4 例）、`build` EXIT=0、`probe:conversation` 17/17 EXIT=0（T8.1 回归不破）、**`--ui-chat` 真机对话 EXIT=0**（`docs/proofs/T8.2/t82-ui-chat.png`：思考+2 工具调用+markdown 表格全渲染 ⇒ 主进程包信封 → preload 归一 → store 路由全链路不破）。**偏差**：① `subagentRuns` 载荷是数组，加字段即破现有渲染层消费者 ⇒ 本批只给对象载荷（`subagentEvent`/`assistResult`）追加 `conversationId`+`projectDir`，数组那条等 T8.3 标签条一起换 envelope；② 两路真模型同时流式的端到端串台断言需 T8.3 分片后由 `--concurrency-probe`（T8.8）补，本批以契约纯函数单测 + 主进程归属唯一性（T8.1 C1/C5）覆盖，验收项如实标 `[~]`。**环境事实重现**：主机脚本首跑因**中文注释 + 无 BOM UTF-8 被 PS 5.1 按 ANSI 读**而 ParserError ⇒ 重写为纯 ASCII 后通过（MEMORY 早已记过这条，本轮再次踩实：主机 .ps1 一律 ASCII-only）。 | **T8.2 ✅ → T8.3（渲染层 store 分片 + 可见性节流）可开工**；`engine-envelope.test.ts` 纳入回归；主机脚本 ASCII-only 铁律复申 |
| 2026-09-04 | T8.1（§7.9） | 完成+偏差+发现 | **Pi 引擎从主进程搬进 utilityProcess（一对话一进程）+ 对话注册表落地**。新建 7 件：`engine/engine-child.ts`（**main 的第二 rollup 入口** → `out/main/engine-child.js` 9.7KB + 共享 chunk `host-tool-specs` 36.7KB；child 内跑 `SdkAdapter`、`ctx.ui` 全走反向 RPC、审批门只做「问宿主」、子代理桥每进程一份）、`engine/engine-host.ts`（`EngineHost`＝fork＋帧传输＋反向 RPC 服务端＋seq 对账＋崩溃/退出兜底；命令级超时表：`prompt/steer/followUp` 不限、只读 10~30s、`start` 120s）、`engine/conversation-registry.ts`（句柄表＋`planAdmit` **循环**腾位置＋退避重启≤3＋退出后 2s 残留扫描「只暴露不偷杀」）、`engine/conversation-core.ts`（状态机/LRU/seq/退避**纯函数**）＋38 例穷举单测、`packages/ipc-schema/src/engine-rpc.ts`（帧契约：20 条下行命令＋4 条反向命令＋`ENGINE_RPC_PARAM_SCHEMAS` 校验表＋**热路径 `decodeFrameLoose` 不做逐帧 zod**）＋26 例单测、`packages/engine/src/remote-adapter.ts`（`RemoteEngineAdapter implements EngineAdapter`，传输可注入＝探针回路）、`packages/engine/src/runtime-dispose.ts`（**`stop()` 改走 `runtime.dispose()`**，SDK 实测 `dispose()` 先广播 `session_shutdown` 再 `session.dispose()`，旧写法直接跳过广播 ⇒ MCP/子代理不回收）＋4 例回归单测。**接口变更**（`EngineAdapter`）：`start()` 返回 `EngineStartInfo`（pid/工具集/timings/RSS/diagnostics）、`getAvailableThinkingLevels()` 与 `listCommands()` **改异步**（同步签名等于假设同进程）、会话替换三方法返回 `EngineSessionRef`、`reload()` 进接口。**`engine-manager` 30 个 handler 业务逻辑零改动**，只改「取哪个 handle」（`requireAdapter`→active 对话）＋只读降级口径＋审批/快照/子代理按对话归属；默认模型与思考档位重放挂到注册表的 `onConversationReady` 钩子（首建/唤醒/崩溃重启都走，幂等键＝对话 id＋child pid），`activeSnapshots` 改 `snapshotsByProject`，`subagentRuntime` 改 runs 快照镜像。**⚠ 设计取舍（比计划更严）**：删掉 `host:policy` 通道——**策略判定也留在主进程**，`host:approval` 一次往返直接给 allow/deny，child 拿不到策略原文也没有本地放行路径（超时＝拒）；`inject-tool-result` 帧取消（宿主工具走反向请求/应答即可）；上行 `event` **不带 conversationId**，归属由主进程按 handle 附加（不信任 child 自报身份）。**agent-tools 拆出 electron-free 的 `host-tool-specs.ts`**：TypeBox schema 带 `Symbol.for('TypeBox.Kind')` **不可 structuredClone** ⇒ 跨进程只过工具名，声明在 child 侧本地取（9 个代理工具同名同参数，`execute` 转发 `host:tool-execute`）。**新增 `--conversation-probe`（17 项，dev/packaged 各 17/17 EXIT=0）**：C6 入口解析、C1 三路并存（独立 pid、工具集逐个全等、9 代理工具在列、三路装配 3.9~4.1s、child RSS 197~216MB）、C5 帧往返自洽＋seq 单调、C2 上限 2 时挤掉最旧 idle 且其 child 确认退出＋唤醒后同一 sessionId、C3 kill 一个→换 pid 自动恢复且其余 alive 主进程管道仍可用、C4 全量关停 193~198ms 后 `node/python` 计数 **Δ0**。主机门禁：`pnpm -r typecheck`/`-r test`/`build` 全 EXIT=0，T8.0 两探针回归不破（P1 14/14、P2 20/20），`package:dir` EXIT=0，**`--ui-chat` 真机对话在子进程引擎下跑通**（`docs/proofs/T8.1/t81-ui-chat.png`：思考＋2 个工具调用全部成功＋完整回答）。**⚠ 四条发现**：① **`utilityProcess.fork` 可直接从 `app.asar` 内加载 ESM 入口并按 app 根解析 `@earendil-works/*`** ⇒ T8.0 交给 T8.1 的「拷进 out/ + `asarUnpack`」处置**不需要**（`extraResources` 反而会把依赖解析弄丢），打包态证据 `docs/proofs/T8.1/conversation-probe-packaged.txt`；② 打包态证据目录不存在时 `writeFileSync` 抛错被 catch 吞 ⇒ 出现「跑了但 `proof_exists=False`」假象，先 `mkdirSync(recursive)`；③ **Node 24 的 `node --test x.ts` 类型剥离不支持尖括号断言**（`ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX`）⇒ 测试里一律 `as T`；④ `packages/engine`/`packages/ipc-schema` 原本没有 `@types/node`，一旦 src 用到 `node:` 内建或测试用 `node:test`，`pnpm -r typecheck` 即挂 ⇒ 本轮两处补 devDep；另注：本机 `pnpm install` 因 `[ERR_PNPM_IGNORED_BUILDS] electron@37.10.3` **恒返回 1**（链接与 `install-app-deps` 实际都成功），别把它当门禁失败。**偏差（已同步改 §7.9 T8.1 正文）**：child 侧 50ms/8KB **合帧推迟到 T8.3**（那才有可见性与流量度量对象）；`ENGINE_CHANNELS.fork` 语义暂落「当前会话内 fork」，另开带独立 worktree 的新对话等 T8.6；同项目 2 对话并存同样等 T8.6（注册表按 projectDir 1:1，避免同目录互踩）。**遗留（诚实标注）**：非 active 对话的事件本批**只落快照不进渲染层**（T8.2 envelope 才接）；packaged 安装/交互仍未验（T5.3 环境阻塞不变）；子代理 `agent_start` 委派链仍未实跑（需密钥＋真人触发）；`--ui-chat` 期间 Warp 另一 pane 里有个**旧代码 dev 实例**在刷 `SdkAdapter not started`（切项目后旧实现留了已 stop 的 adapter），属切换前行为、门禁 step 0 已随残留 electron 一并终止。 | **T8.1 ✅ → T8.2（IPC 加 conversationId＋事件按对话路由）可开工**；`probe:conversation`（dev/packaged）纳入回归；性能红线表「休眠恢复」「在飞上限」两行有实测数；MEMORY 新增 asar/TypeBox/Node 断言三条事实 |
| 2026-09-04 | T8.0（§7.9） | 完成+实测+发现 | **双探针跑完 → D 方案判 Go**（§7.9 关键路径门禁解除）。新建 3 件：`electron/main/engine/engine-child.mjs`（utilityProcess 子进程入口，**独立 .mjs 不打进 bundle**；帧协议 `{kind:invoke|respond|event|cancel|control}` 双向、child 侧 `ctx.ui` 反向 RPC、宿主实现的 `probe_host_echo` customTool、关停走 `runtime.dispose()` 而非 `session.dispose()`）、`engine-process-probe.ts`（`electron . --engine-process-probe`，无窗、**跳单例锁**以便与用户开着的 dev 并存、自检 `app.exit`）、`worktree-probe.mjs`（纯 Node，`PI_OFFLINE=1`）。**P1 dev 形态 14/14**：① 装载性——child 内 15 个工具与主进程内基线**逐个全等**（同口径剔除探针自注册的 `probe_host_echo` 后比对）、含 5 个社区包工具（`ask_user_question/todo/web_search/source_check/fetch_content/get_search_content/mcpScript/mcp/plan_mode_question/plan_mode_complete`）+ Skill `mcp-scripting`、`diagnostics` 0 error ⇒ **「utilityProcess 里扩展加载等价于主进程」得证，A/B/C 全否决、D 不必回退**；② 双向 RPC——事件流 290 帧、反向 `tool:execute` 与 `ui:confirm/select/input` 闭环、`cancel` 帧 4s 内中止在跑的一轮、child `process.crash()` 只掉自己（主进程与另一 child 照常响应 RPC）；③ 成本——fork→ready **75~88ms**、装配 **1089~1147ms**（SDK import ≈590 / services ≈445 / session 16 / bind 5）、child RSS **198~225MB/对话**（3 路 ≈600~675MB，落在候选预算内）；④ 残留——`runtime.dispose()` 1ms 返回、4 个 child 全部离场、`node.exe Δ0 / python.exe Δ0`（MCP 零残留），`electron.exe Δ1` 是硬崩对照组的 crashpad 副作用，单独记。**P2 20/20**：worktree add 64~69ms（真实 pi-wood 仓库 151~180ms、工作树 3.3MB、**不复制 node_modules**、`.git` 是文件共享 objects）、两树 `git diff` 互不污染、两份改动先后 `git apply --3way` 均落主树（36ms）、同行冲突第二份被拒且原因可读（**不自动合**）、`worktree remove+prune+branch -D` 后跟踪文件回干净基线、会话目录按 cwd 编码分家（`--C--...-main--` vs `--C--...-wt-s--`）、非 git/脏树/detached/submodule/超长路径/大小写六类降级全部有显式文案。**⚠ 三条发现（写进下游，不当噪音）**：① `git apply --3way` 对含空行上下文的补丁**非零退出但内容已落盘**（git 2.54 判 trailing whitespace；根因是 unified diff 里空行=「一个前导空格」）→ T8.6 固定 `--3way --ignore-whitespace`（实测归零）+ 以内容/diff 复核，禁止只看退出码；② **打包态 child 入口缺席**（`files` 只有 `out/**`），且直接往 `files` 加源码路径会让 electron-builder 把工作区依赖映射算错（`packages/engine/package.json must be under apps/desktop` → `package:dir` 失败，已回滚该改动并留注释）→ **T8.1 前置**：构建时拷 `engine-child.mjs` 进 `out/main/` 并对其 `asarUnpack`（**不能用 `extraResources`**：落到 app 树外解析不到 `@earendil-works/*`）；③ `utilityProcess.fork` 的 `env` 含 `undefined` 值直接 `Invalid value for env`（首轮 P1 全挂在此），已改为过滤非字符串 env 值并注释。**另两条口径修正**：`utilityProcess` 的 `proc.pid` 本机返回 `undefined` → 改由 child 在 `ready` 帧自报；`tasklist` 按 PID 取 RSS 解析不出值（探针 `pidRSS=0` 属口径失效）→ 以 child 自报 `memoryUsage().rss` 为准，性能红线表两行据此改写。**顺带补上 T8.P 的打包侧证据**：packaged exe 无头跑通了主进程 ESM（读 settings、注入钥匙串凭据、落盘日志），但**安装/交互仍未验**（T5.3 阻塞不变）。**性能红线表回填 4 行、其余 6 行如实标注「T8.0 测不了，待 T8.1/T8.3/T8.5 有对象后实测定档」**，不假装已测。 | **T8.0 ✅ → T8.1 可开工**（前置：child 入口随包的构建改动）；D 方案不退路；`--engine-process-probe` / `probe:worktree` 纳入回归 |
| 2026-09-03 | T8.P（§7.9） | 完成+偏差+踩坑 | **主进程产物 CJS→ESM 切换完成（多对话批第 0 步）**。改动 5 处：① `apps/desktop/package.json` 加 `"type":"module"`（`main` 字段**不动**，见偏差①）；② `electron.vite.config.ts` 3 处 `resolve(__dirname,…)`→`fileURLToPath(import.meta.url)` 推 `root`（否则 `type:module` 下 vite 配置直接 `__dirname is not defined`）；③ `electron/main/index.ts` 顶部 `__dirname` shim + preload 路径 `../preload/index.js`→**`.mjs`**（`sandbox:false` 保留并在注释锁死=ESM preload 硬依赖）；④ `engine-manager.ts` 子代理 entry 路径基准改 `import.meta.url`（dev 下 `../../electron/...` 与旧 `__dirname` 解析等价；**packaged 下该相对路径落 asar 之外→`existsSync` 假→子代理禁用**，属切换前即存在的遗留，注释标待 T8.0/P1-d）；⑤ **验收靶子达成**：`packages/engine/src/sdk-adapter.ts` Pi 动态 import 改静态 `import * as PiSdk`（ESM-only 包在 ESM 下合法＝切净的硬证明；渲染层零影响——该文件仅 `@pi-wood/engine/sdk` 主进程子路径可达，根入口仍纯类型）。余 **5 处**动态 import（index/extension-probe/project-manager/provider-manager 4 处 `await import` + session-service 1 处 `return import`）保留并各加「T8.P 保留动态 import（非格式原因）」注释。**⚠ 偏差①（实测推翻计划假设，最重要）**：计划步骤 1/2 写「main 产物→`out/main/index.mjs`、`package.json.main` 同步改」——**错**。electron-vite@2.3.0 的 `[name].mjs` 改名块（`dist/chunks/lib-BmEkZIgk.mjs:410-421`）只挂在 **preload** 的 preset 插件上；main 侧仅把 `rollupOptions.output.format` 设 `es`、入口名仍 `[name].js`（`type:module` 下 `.js` 即 ESM）→ **`main` 保持 `out/main/index.js`**，按计划改 `.mjs` 会指向不存在文件、app 起不来（首版照计划改后已回退）。**⚠ 偏差②**：计划写「其余 4 处动态 import」，实测源点 5 处（旧 bundle 只显 5 处入口是因两处被折叠）；且 sdk-adapter 静态化后 Pi 已进启动图，「避免启动即载重 SDK」这条保留理由**不再成立**，注释与计划正文均按实改成「维持注册时序 + 最小改动面」。**门禁（主机 Warp 跑 `scripts/t8p-gate.ps1`，日志 `apps/desktop/docs/proofs/ui-v3/t8p-gate.log`）9 项全绿**：`pnpm -r typecheck` EXIT=0、`pnpm -r test` EXIT=0、三目标 `pnpm build` EXIT=0；产物 grep 断言 `out/main/index.js`：`"use strict"`=0 / `__defProp`=0 / `require(`=0 / Pi 静态 import=1 / Pi 动态 import=5，`out/preload/index.mjs` 存在且 `"use strict"`=0、含 `import {`；`--plugin-probe`/`--goal-probe`/`--memory-probe` 全 EXIT=0；`--ui-stress 10000` EXIT=0、`--capture` EXIT=0、**`--ui-chat` 真机对话 EXIT=0**（`t8p-ui-chat.png`：思考预览+连续工具分组+行内代码高亮+右栏联动，选项目→对话→工具→settle 闭环在 ESM 主进程下不回退）；`pnpm package:dir` 复跑 EXIT=0 产 `release/win-unpacked/`（204MB exe）。**步骤 5 的 CJS↔ESM 互操作风险未兑现**（node-pty/playwright/zod/diff/ignore/shiki + 4 workspace 包经 build+capture+ui-chat 全通过，**未启用 `createRequire` 退路**）。**⚠ 环境事件两起（非代码缺陷但挡门禁，如实记）**：(a) 主机 node_modules **丢了 `@pi-wood/plugin-api` 三处 workspace junction**（`apps/desktop/node_modules/@pi-wood/`、`packages/ipc-schema/node_modules/@pi-wood/`、`packages/plugin-api/node_modules/@types`）→ 门禁首跑 typecheck/build/engine test **同一根因三处全挂**（`Cannot find module '@pi-wood/plugin-api'` / `node:test`），`pnpm install` 复原链接（其自身 postinstall `install-app-deps` 因 (b) 报红但不影响链接落盘）；(b) 旧包名遗留**死链接 `@pidesk/*`**（apps/desktop、packages/engine、packages/ipc-schema 三处 `node_modules/@pidesk`）令 electron-builder `@electron/rebuild` 在 stat 处 ENOENT → **`package:dir` 首跑 EXIT=1**——**修正 T5.6 §8 那条「install-app-deps 报红无关、不影响 build」的判定**：build 确实不受影响，但**会挡打包步骤**。三处 `@pidesk` 目录 `Move-Item` 入仓库 `.trash/pidesk-{1,2,3}`（未硬删）后复跑 EXIT=0。**遗留（诚实标注）**：① 打包态仅 `--dir` 产留档，**安装/运行未验**（T5.3 环境阻塞不变：无干净 VM/签名/三平台）；② packaged 下子代理 entry 路径失效（见 ④），留 T8.0/P1-d 一并处理；③ 子代理 `agent_start` 委派链本轮**未实跑**（需密钥+真人触发），dev 路径基准仅由 `existsSync` 同表达式间接证到，验收项相应留 `[ ]`；④ `--ui-chat` 截屏右栏显 `browser:navigate TimeoutError（example.com）`=本机网络噪音，与模块格式无关。**沙箱侧环境事实**（本会话 `mcp__workspace__bash`=Linux VM，挂载丢 Windows junction）：tsc 需 `paths` 补 `undici-types`/`@pi-wood/plugin-api` 才能出可信 main 侧结果；`electron-vite build` 需把临时配置放 `.pnpm/node_modules` hoisted 店 + `ESBUILD_BINARY_PATH` 指沙箱本地二进制 + 补 `@rollup/rollup-linux-x64-gnu`；**结论：真机门禁必须走主机**（本轮以 computer-use 驱动 Warp 完成，命令与产物全留档）。 | **T8.P ✅（§7.9 第 0 步收口）**；T8.0 前置满足；§7.9「现状核实」中「主进程产物=CJS」一行作废；MEMORY 新增 ESM 事实条 |
