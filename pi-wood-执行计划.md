...
- 验收：
  - [~] 两个不同项目的对话并存：文件树、git 分支/变更数、快照、记忆 project scope 各自正确、互不覆盖（探针断言）→ 作用域链路已按对话树接线；探针/目检待真机
  - [~] 同项目两对话（两个 worktree）：文件树/diff/终端各自指向自己那棵树；`memory_save` 两条都落到**主项目**同一文件、都不丢 → 归一+写队列+单测完成；真机目检待
  ...
  - [~] **CLI 互通不回退**：桌面在 worktree 建的会话，在**主项目**目录 `pi --resume` 可选到（左栏聚合的硬证据）→ 聚合 IPC 完成；真机 `pi --resume` 目检待
  - [x] 并发写队列有单测（两对话同时 memory_save / settings patch，无丢更新）
  - [x] `pnpm typecheck` + desktop test + build + 三探针回归全绿 → typecheck/test（desktop **216/216**）/build 实跑全绿；**三探针回归 2026-09-05 真机全绿**（worktree 18/18、plugin/goal/memory ALL PASS/7/7/7/7、engine-process 14/14、conversation 21/21）。**⚠ 本任务自身验证仍缺两件（不许被上一句带过）**：`--workspace-scope-probe`（2 项目 + 同项目两 worktree 交叉断言 fs/git/snapshot/memory/term 归属）**至今未实现**、双项目与双 worktree 的**文件树/diff/终端缓冲/浏览器 page 目检未跑**（需 GUI）
...
| T8.7 作用域 | 探针回归全绿 | `--workspace-scope-probe` **至今未实现**；双项目/双 worktree 的文件树·diff·终端缓冲·浏览器 page·`pi --resume` 聚合目检未跑 |
...
> **本批全 ✅ 的剩余条件**：①一轮「GUI 人手 + 真模型」验收（4 对话并行改代码→审 diff→回流、并发审批、切换手感、并发=1 A/B 对照、真模型三路体感）；②**度量缺口经 T8.9+T8.10 后只剩两条**：在飞 prompt/子代理峰值计数（需真模型灌流，`--concurrency-probe` ④⑤ 设计上不做）与真模型负载下的体感复核——**红线数字一律要绑负载形状**（同一份代码 16ms 节流下第二跳 p95 1.87ms、无间隔突发下 261ms，后者是排队延迟不是产品慢）；③`--workspace-scope-probe` 与 deny-by-default 探针断言补齐；④Windows 侧零残留硬断言（⑦ 只在 win32 有效）与 T5.3 的安装/签名/三平台（环境阻塞）。
