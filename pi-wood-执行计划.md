...
- **摘要浮层（定稿）**：hover 某根刻度 → 右侧 8px 处只浮出**这一条**的摘要（仅首行标题，单行省略、`max-w min(24rem,38vw)`），按实测行中心与该刻度竖向对齐；离开 120ms 后收（沿刻度条平移不闪断）。参考的 200px 全轮次列表（normal 变体 + HoverCard）**不采纳**。
- **diff bars（撤销）**：浮层里那 5 根增删竖条被用户否决（「这什么鬼东西」——纯问答轮全是灰条，看起来像无意义的装饰）→ 连同 `diffBarCounts`/`DIFF_TOTAL_BLOCKS` 纯函数与单测一起移除，`message-nav` 收窄为 `buildNavTicks`/`firstLine`/`tickWidthAt`。
- **纯逻辑下沉**：新增 `lib/message-nav.ts`（`buildNavTicks`/`firstLine`/`tickWidthAt` 正态衰减）与 `lib/outline-bus.ts`（阅读锚点总线），各带单测并登记进 `package.json` 的 test 清单（门禁 ①「不登记=静默漏测」）。
