...
> **剩余（后续）**：child 的 **edit/write 尚未包守卫**（只包了 bash，可复用 `guardChildTool` 扩到二者）；子代理进度/状态上屏归 §7.7 T6.3；ApprovalCard 入参原样显示 JSON 偏丑（展示打磨）；完整多代理 fan-out/steer/cancel e2e 可选再补。
...
- [x] 验证（**真机 e2e 已验**，DeepSeek + dev9）：`general` 子代理执行 `git log --oneline -3` 时**弹出桌面 ApprovalCard**（`允许执行 bash?`）。因 child print 会话不触发 `tool_call`，门改由 vendored `createPiSessionOptions` **包 `bash.execute` 调 `guardChildTool`** 生效。⚠ child 的 **edit/write 尚未包**（下一步扩）。
